...
const generalSlice = createSlice({
  ...
  reducers: {
    ...
    setAllUsers(state, action: PayloadAction<User[]>) {
      if (action.payload)
        state.users.all = postProcessUsersList([...action.payload])
      else state.users.all = []
      state.users.engineers = state.users.all.filter(
        (user) => user.role == UserRole.ENGINEER,
      )
      state.users.annotators = state.users.all.filter(
        (user) => user.role == UserRole.ANNOTATOR,
      )
      state.users.experts = state.users.all.filter(
        (user) => user.role == UserRole.EXPERT,
      )
    },
...
export const selectInactiveOrganization = (state) =>
  state.general.organizationInactive
...
export const selectBricksIntegrator = (state) =>
  state.general.bricksIntegrator.config
export const selectBricksIntegratorAttributes = (state) =>
  state.general.bricksIntegrator.attributes
export const selectBricksIntegratorLanguages = (state) =>
  state.general.bricksIntegrator.languages
export const selectBricksIntegratorEmbeddings = (state) =>
  state.general.bricksIntegrator.embeddings
export const selectBricksIntegratorLabelingTasks = (state) =>
  state.general.bricksIntegrator.labelingTasks
export const selectBricksIntegratorLookupLists = (state) =>
  state.general.bricksIntegrator.lookupLists
export const selectBricksIntegratorLabels = (state) =>
  state.general.bricksIntegrator.labels
export const selectDisplayIconComments = (state) =>
  state.general.comments.displayIcon
export const selectRouteColor = (state) => state.general.routeColor

export const selectAnnotatorsDict = createSelector(
  [selectAnnotators],
