...
} from '@/src/types/components/projects/projectId/data-browser/data-browser'
import {
  postProcessDataSlices,
  postProcessRecordComments,
...
const dataBrowserSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromAllDataSlicesById(state, action: PayloadAction<string>) {
      if (action.payload)
        state.all = state.all.filter((slice) => slice.id !== action.payload)
    },
    updateDataSlicesState: {
      reducer(state, action: PayloadAction<any[]>) {
        if (action.payload.length !== 2)
          throw new Error(
            'updateDataSlicesState must be called with exactly 2 arguments',
          )
        const [dataSliceId, changes] = action.payload
        if (state.active && state.active.id === dataSliceId)
          changeAllFor(state.active, changes)

        const dataSlice = state.all.find(
          (lookupList) => lookupList.id === dataSliceId,
        )
    ...
    setSearchRecordsExtended(
      state,
      action: PayloadAction<SearchRecordsExtended>,
    ) {
      if (action.payload) state.searchRecordsExtended = action.payload
      else
        state.searchRecordsExtended = {
          fullCount: 0,
          queryLimit: 0,
          queryOffset: 0,
          recordList: [],
    ...
    updateConfigurationState: {
      reducer(state, action: PayloadAction<any[]>) {
        if (action.payload.length !== 2)
          throw new Error(
            'updateConfigurationState must be called with exactly 2 arguments',
          )
    ...
    updateAdditionalDataState: {
      reducer(state, action: PayloadAction<any[]>) {
        if (action.payload.length !== 2)
          throw new Error(
            'updateAdditionalDataState must be called with exactly 2 arguments',
          )
    ...
    setRecordComments(state, action: PayloadAction<any>) {
      if (action.payload)
        state.recordComments = postProcessRecordComments(action.payload)
      else state.recordComments = {}
    },
    setRecordsInDisplay(state, action: PayloadAction<boolean>) {
      if (action.payload)
        state.similaritySearch.recordsInDisplay = action.payload
    ...
    expandRecordList(state, action: PayloadAction<any>) {
      if (action.payload) {
        ...
        state.searchRecordsExtended.recordList = [
          ...state.searchRecordsExtended.recordList,
          ...action.payload.recordList,
        ]
    ...
    updateFullSearchState: {
      reducer(state, action: PayloadAction<any[]>) {
        if (action.payload.length !== 2)
          throw new Error(
            'updateFullSearchState must be called with exactly 2 arguments',
          )
...
export const selectSimilaritySearch = (state) =>
  state.dataBrowser.similaritySearch
export const selectActiveSearchParams = (state) =>
  state.dataBrowser.activeSearchParams
...
export const selectIsTextHighlightNeeded = (state) =>
  state.dataBrowser.isTextHighlightNeeded
export const selectRecordComments = (state) => state.dataBrowser.recordComments
export const selectUniqueValuesDict = (state) =>
  state.dataBrowser.uniqueValuesDict
...
export const selectDataSlicesAll = createSelector(
  [selectDataSlices],
  (d): any => (d ? d.filter((slice) => slice.id != '@@NO_SLICE@@') : null),
)
export const selectDataSlicesDict = createSelector(
  [selectDataSlicesAll],
  (a): any => (a ? arrayToDict(a, 'id') : null),
)
export const selectStaticSlices = createSelector(
  [selectDataSlices],
  (a): any =>
    a ? a.filter((slice) => slice.sliceType == Slice.STATIC_DEFAULT) : null,
)
