    ...
    reducers: {
        ...
        updateLookupListState: {
            reducer(state, action: PayloadAction<any[]>) {
                if (action.payload.length !== 2) throw new Error("updateLookupList must be called with exactly 2 arguments");
                const [lookupListId, changes] = action.payload;
                if (state.active && state.active.id === lookupListId) changeAllFor(state.active, changes);

                const lookupList = state.all.find((lookupList) => lookupList.id === lookupListId);
...
export const { updateLookupListState, setActiveLookupList, setAllLookupLists, extendAllLookupLists, removeFromAllLookupListById, setCheckedLookupLists } = lookupListsSlice.actions;
