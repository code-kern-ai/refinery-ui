...
export function postProcessNotifications(
  ...
  notifications.forEach((notification) => {
    ...
    if (Object.keys(projectNames).length != 0)
      notification.projectName = projectNames[notification.projectId]?.name
    ...
    if (!prepareNotifications.length) {
      ...
      prepareNotifications[prepareNotifications.length - 1].array[0].type ===
        notification.type &&
      prepareNotifications[prepareNotifications.length - 1].array[0]
        .projectId === notification.projectId
    ) {
      prepareNotifications[prepareNotifications.length - 1].array.push(
        notification,
      )
    } else {
      prepareNotifications.push({
...
function applyPageChanges(notification) {
  if (
    ...
    notification.message == 'Continuation of your previous session.'
  ) {
...
export function postProcessNotificationsUser(
  notifications,
  notificationsState,
): any {
  let final = []
  const toBeAddedValues = notifications.map((n) => ({
    ...n,
    savedToStore: new Date().getTime(),
  }))
