...
export function parseKey(key: string): CommentRequest {
  ...
  const toReturn = {
    commentType: parts[0] as CommentType,
    projectId: parts[1],
    commentKey: parts[2],
  }
...
export function commentTypeToString(
  type: CommentType,
...
export function convertTypeToKey(type: string): string {
  switch (type) {
    case 'Attributes':
      return CommentType.ATTRIBUTE
    case 'Labeling tasks':
      return CommentType.LABELING_TASK
    case 'Labels':
      return CommentType.LABEL
    case 'Data slices':
      return CommentType.DATA_SLICE
    case 'Embeddings':
      return CommentType.EMBEDDING
    case 'Heuristics':
      return CommentType.HEURISTIC
    case 'Knowledge bases':
      return CommentType.KNOWLEDGE_BASE
    case 'Organizations':
      return CommentType.ORGANIZATION
    case 'Records':
