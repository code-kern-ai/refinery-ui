...
export default function postProcessExportRecordData(data: any) {
    ...
    if (x.informationSources.length == 0) x.informationSources.push({ id: NONE_IN_PROJECT, name: "None in project" });
    if (x.labelingTasks.length == 0) x.informationSources.push({ id: NONE_IN_PROJECT, name: "None in project" });
    if (x.attributes.length == 0) x.informationSources.push({ id: NONE_IN_PROJECT, name: "None in project" });
    if (x.dataSlices.length == 0) x.dataSlices.push({ id: NONE_IN_PROJECT, name: "None in project" });
    return x;
}
