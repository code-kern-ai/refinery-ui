...
import {
  isoCodes,
  mostRelevant,
} from '../classes/bricks-integrator/language-iso'
...
export function bricksVariableNeedsTaskId(
  variableType: BricksVariableType,
): boolean {
  switch (variableType) {
    case BricksVariableType.EMBEDDING:
    case BricksVariableType.LABEL:
...
export function getHttpBaseLink(config: BricksIntegratorConfig): string {
  if (config.querySourceSelectionRemote) return HTTP_BASE_LINK
    ...
    return `http://localhost:${config.querySourceSelectionLocalStrapiPort}/api/modules/`
}
...
function extendUrl(
  ...
  for (let i = 0; i < splitVal.length; i++) {
    ...
    filter += filterActiveLearnersFromGenerators(
      splitVal,
      i,
      filter,
      executionTypeFilter,
...
function filterActiveLearnersFromGenerators(
  splitVal: string[],
  index: number,
  filter: string,
  executionTypeFilter: string,
...
export function filterMinVersion(data: any[]): any[] {
  ...
  const currentVersion = el.textContent
    .trim()
    .replace('v', '')
    .split('.')
    .map((e) => parseInt(e))
  ...
  return data.filter((e) =>
    refineryCanHandle(currentVersion, e.attributes.minRefineryVersion),
  )
}

function refineryCanHandle(
...
export function getGroupName(groupKey: string): string {
  switch (groupKey) {
    case 'no_api_key':
      return 'No API Key'
    default:
      return capitalizeFirst(groupKey)
  }
}

export function getIsoCodes(
  onlyMostRelevant: boolean = true,
): { code: string; name: string }[] {
  return isoCodes.filter(
    (e) => !onlyMostRelevant || mostRelevant.includes(e.code),
  )
}

export function getIconName(bricksIconName: string): string {
  switch (bricksIconName) {
    case 'ThreeDCubeSphere':
      return '3dCubeSphere'
    default:
      bricksIconName
  }
