...
`,
      ...
      case InformationSourceExamples.AL_EMPTY_EXTRACTION:
        return {
          name: 'MyExtractor',
          code: `from sequencelearn.sequence_tagger import CRFTagger
...
`,
  ...
  static isCodeStillTemplate(code: string): InformationSourceExamples {
    if (InformationSourceCodeLookup.templateEnumArray.length == 0) {
      for (let t of Object.values(InformationSourceExamples).filter((x) =>
        isNaN(Number(x)),
      )) {
        InformationSourceCodeLookup.templateEnumArray.push({
          example: InformationSourceExamples[t],
          value: InformationSourceCodeLookup.getInformationSourceTemplate(
            InformationSourceExamples[t],
          ),
        })
      }
    }
