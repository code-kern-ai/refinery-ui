...
function getDummyNodeByIdForSelection(
  id: DummyNodes,
  ...
  switch (id) {
    case DummyNodes.CODE_TESTER:
      ...
      baseNode.attributes.description =
        'Lets you test random code for the integrator (only available for kern admins)'
    ...
    case DummyNodes.CODE_PARSER:
      ...
      baseNode.attributes.description =
        'Lets you parse random code to the new structure (only available for kern admins)'
      if (extendedIntegrator) {
        baseNode.attributes.availableFor = ['refinery']
        baseNode.attributes.partOfGroup = ['admin']
      }
    ...
    case DummyNodes.REFACTOR_TESTER:
      ...
      baseNode.attributes.description =
        'Lets you test the new structure with dummy data from vader sentiment (only available for kern admins)'
...
export function extendDummyElements(
  finalData: any[],
  extendedIntegrator: boolean,
  user: User,
  isAdmin: boolean,
  ...
  addElementToList(
    finalData,
    getDummyNodeByIdForSelection(DummyNodes.CODE_TESTER, extendedIntegrator),
    user,
  )
  addElementToList(
    finalData,
    getDummyNodeByIdForSelection(DummyNodes.CODE_PARSER, extendedIntegrator),
    user,
  )
...
export function getDummyNodeByIdForApi(id: DummyNodes): any {
  ...
  switch (id) {
    case DummyNodes.CODE_TESTER:
      ...
      baseNode.data.attributes.description =
        'Lets you test random code for the integrator (only available for kern admins)'
      return baseNode
    case DummyNodes.CODE_PARSER:
      ...
      baseNode.data.attributes.integratorInputs = {
        ...
        variables: {
          ...
          MODE: {
            ...
            description:
              'choose "scores" to only get the sentiment scores as floats',
            optional: 'false',
            addInfo: [BricksVariableType.GENERIC_STRING.toLowerCase()],
          },
          MIN_SCORE: {
