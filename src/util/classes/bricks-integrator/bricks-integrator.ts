...
import {
  BricksExpectedLabels,
  BricksIntegratorConfig,
  BricksVariable,
  BricksVariableType,
  IntegratorInput,
  RefineryDataType,
...
import {
  capitalizeFirst,
  capitalizeFirstForClassName,
} from '@/submodules/javascript-functions/case-types-parser'
import {
  getPythonClassName,
  getPythonFunctionName,
  toPythonFunctionName,
} from '@/submodules/javascript-functions/python-functions-parser'
import {
  bricksVariableNeedsTaskId,
  canHaveDefaultValue,
  getChoiceType,
  getEmptyBricksExpectedLabels,
  getEmptyBricksVariable,
} from '../../shared/bricks-integrator-helper'
...
export class BricksCodeParser {
  ...
  public static prepareCode(
    config: BricksIntegratorConfig,
    executionTypeFilter: string,
    nameLookups: string[],
    labelingTaskId: string,
    forIde: string | boolean = false,
    labelingTasks: any[] = null,
  ) {
    this.errors = []
    ...
    if (this.integratorInputRef)
      this.baseCode = config.api.data.data.attributes.sourceCodeRefinery
    ...
    config = this.checkVariableLines(
      config,
      executionTypeFilter,
      labelingTaskId,
      forIde,
    ...
    if (labelingTaskId) {
      this.labelingTaskName = BricksCodeParser.getLabelingTaskAttribute(
        labelingTaskId,
        'name',
        labelingTasks,
      )
    }
    if (config.api.data.data.id == DummyNodes.CODE_PARSER)
      this.parseJsonCode(config, labelingTaskId)
    return config
  }

  public static checkVariableLines(
    config: BricksIntegratorConfig,
    executionTypeFilter: string,
    labelingTaskId: string,
    forIde: string | boolean = false,
  ...
  private static checkAndMatchVariables(
    forIde: string | boolean = false,
    ...
    if (
      ...
      Object.keys(this.integratorInputRef.variables).length
    ) {
      this.errors.push(
        'different number of variable lines in code and integrator input detected',
      )
    }
    if (this.integratorInputRef.outputs) {
      if (labelingTaskId) {
        this.expected.expectedTaskLabels = []
        const existingLabels = BricksCodeParser.getLabels(
          labelingTaskId,
          labelingTasks,
        )
        for (const label of this.integratorInputRef.outputs) {
          ...
          this.expected.expectedTaskLabels.push({
            ...
            backgroundColor:
              existingLabel && existingLabel.color.backgroundColor
                ? existingLabel.color.backgroundColor
                : 'bg-' +
                  (existingLabel ? existingLabel.color : 'gray') +
                  '-100',
            textColor:
              existingLabel && existingLabel.color.textColor
                ? existingLabel.color.textColor
                : 'text-' +
                  (existingLabel ? existingLabel.color : 'gray') +
                  '-700',
            borderColor:
              existingLabel && existingLabel.color.borderColor
                ? existingLabel.color.borderColor
                : 'border-' +
                  (existingLabel ? existingLabel.color : 'gray') +
                  '-400',
          })
        }
        this.expected.expectedTaskLabels.sort(
          (a, b) => -a.exists - -b.exists || a.label.localeCompare(b.label),
        )
        this.expected.labelsToBeCreated =
          this.expected.expectedTaskLabels.filter((x) => !x.exists).length
        this.expected.labelWarning =
          !this.expected.expectedTaskLabels[
            this.expected.expectedTaskLabels.length - 1
          ].exists
        this.expected.canCreateTask =
          BricksCodeParser.getLabelingTaskAttribute(
            labelingTaskId,
            'taskType',
            labelingTasks,
        ...
        if (!this.globalComments.some((x) => x.startsWith('Will return'))) {
          this.globalComments.push(
            'Will return: ["' +
              this.integratorInputRef.outputs.join('", "') +
              '"]',
          )
        }
      }
    ...
    for (const variable of this.variables) {
      if (!(variable.baseName in this.integratorInputRef.variables)) {
        this.errors.push(
          `Variable ${variable.baseName} in code is not defined in integrator input`,
        )
        continue
      }
      ...
      if (inputV.acceptsMultiple)
        variable.canMultipleValues = isStringTrue(inputV.acceptsMultiple)
      if (inputV.defaultValue && inputV.addInfo) {
        if (
          !inputV.addInfo.some(
            (x) => !canHaveDefaultValue(x.toUpperCase() as BricksVariableType),
          ) &&
          canHaveDefaultValue(variable.type)
        ) {
      ...
      if (inputV.selectionType == SelectionType.CHOICE) {
        ...
        if (newType != BricksVariableType.UNKNOWN && newType != variable.type) {
          if (newType == BricksVariableType.GENERIC_CHOICE) {
            if (!inputV.allowedValues) {
              this.errors.push(
                `Variable ${variable.baseName} in code is of type choice, but allowed values are not provided`,
              )
            } else {
        ...
        if (!inputV.allowedValueRange || inputV.allowedValueRange.length != 2) {
          this.errors.push(
            `Variable ${variable.baseName} is of type range but allowedValueRange is not provided`,
          )
          continue
        }
  ...
  private static getFunctionName(executionTypeFilter: string) {
    const parsedName =
      executionTypeFilter == 'activeLearner'
        ? getPythonClassName(this.baseCode)
        : getPythonFunctionName(this.baseCode)
    if (this.integratorInputRef && this.integratorInputRef.name) {
      ...
      if (parsedName != providedName) {
        this.errors.push(
          `Function name in code (${parsedName}) does not match name in integrator input (${providedName})`,
        )
      }
      return providedName
    }
  ...
  public static replaceVariables(
    ...
    let replacedCode = this.replaceFunctionLine(
      this.baseCode,
      executionTypeFilter,
    )
    ...
    config.codeFullyPrepared = this.variables.every(
      (v) =>
        v.optional ||
        (v.values.length > 0 && v.values.every((va) => va != null)),
    )
    config.canAccept =
      config.codeFullyPrepared && !this.nameTaken && this.functionName != ''
    if (config.api.data.data.id == DummyNodes.CODE_PARSER)
      this.parseJsonCode(config, labelingTaskId)
    return config
  }

  private static parseJsonCode(
    config: BricksIntegratorConfig,
    labelingTaskId: string = null,
  ) {
    ...
    const json: any = {
      ...
      refineryDataType: config.prepareJsonAsPythonEnum
        ? 'RefineryDataType.TEXT.value'
        : RefineryDataType.TEXT, //currently fix text since up until now all were interpreted as text
    }
      ...
      for (let comment of this.globalComments) {
        ...
        if (nextUp) {
          ...
          if (labelStringMatch && labelStringMatch.length > 1) {
            json.outputs = labelStringMatch[1]
              .split(',')
    ...
    if (this.globalComments.length > 0) {
      ...
      if (remainingComments.length > 0)
        json.globalComment = remainingComments.join('\n')
    }
      ...
      for (const variable of this.variables) {
        const element: any = {
          selectionType: getSelectionType(
            variable.type,
            config.prepareJsonAsPythonEnum,
          ),
        }
        if (variable.values.length > 0 && variable.values[0] != null)
          element.defaultValue = variable.values[0]
        ...
        const addInfo = getAddInfo(
          variable.type,
          config.prepareJsonAsPythonEnum,
        )
        if (addInfo && addInfo.length > 0) element.addInfo = addInfo
        if (config.prepareJsonRemoveYOUR)
          json.variables[variable.baseName.substring(5)] = element
  ...
  private static parseExpectedLabelsComment(
    comment: string,
    labelingTaskId: string = null,
    labelingTasks: any[] = null,
  ): string {
    ...
    if (labelingTaskId) {
      if (labelStringMatch && labelStringMatch.length > 1) {
        const labels = labelStringMatch[1]
          .split(',')
          .map((x) => x.replace(/\"/g, '').trim())
  ...
  public static activeLabelMapping(
    config: BricksIntegratorConfig,
    executionTypeFilter: string,
    labelingTaskId: string,
    forIde: boolean | string,
    labelingTasks: any[] = null,
  ) {
    this.expected.availableLabels = BricksCodeParser.getLabels(
      labelingTaskId,
      labelingTasks,
    )
  ...
  private static extendCodeForLabelMapping(
    config: BricksIntegratorConfig,
    executionTypeFilter: string,
  ) {
    ...
    const isExtractor =
      config.api.data.data.attributes.moduleType == 'extractor'
    ...
    functionWrapper +=
      '    #this is a wrapper to map the labels according to your specifications\n'
    if (isExtractor) {
      functionWrapper +=
        '    for (result, start, end) in bricks_base_function(record):\n'
    } else {
      functionWrapper += '    result = bricks_base_function(record)\n'
    }
    functionWrapper +=
      (isExtractor ? '    ' : '') + '    if result in my_custom_mapping:\n'
    functionWrapper +=
      (isExtractor ? '    ' : '') +
    ...
    config.preparedCode =
      config.preparedCode.replace(currentFunctionLine, functionWrapper) +
      mappingDict
  }

  private static getCurrentFunctionLine(
    config: BricksIntegratorConfig,
    executionTypeFilter: string,
  ): string {
  ...
  private static getIndexFunctionLine(
    code: string,
    executionTypeFilter: string,
  ): number {
  ...
  private static extendCodeForRecordIde(
    config: BricksIntegratorConfig,
    forIde: string | boolean = false,
  ) {
    if (!forIde) return
  ...
  private static collectGlobalComment(
    labelingTaskId: string = null,
    labelingTasks: any[] = null,
  ): string[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('#')) {
        ...
        if (
          isCommentTrue(tmpLine, BricksVariableComment.TASK_REQUIRED_LABELS)
        ) {
          tmpLine = this.parseExpectedLabelsComment(
            tmpLine,
            labelingTaskId,
            labelingTasks,
          )
        ...
        if (idx > 0) {
          const parts = tmpLine
            .split('[')
            .map((x, i) => (i == 0 ? x : '[' + x).trim())
          commentLines.push(...parts)
        } else commentLines.push(tmpLine)
      }
    }
    if (this.integratorInputRef && this.integratorInputRef.globalComment)
      this.integratorInputRef.globalComment
        .split('\n')
  ...
  private static getValueString(variable: BricksVariable): string {
    ...
    if (realValues.length == 1) {
      const v = this.getPythonVariable(
        realValues[0],
        variable.pythonType,
        variable.type,
      )
    ...
    return (
      ...
      realValues
        .map((x) =>
          this.getPythonVariable(x, variable.pythonType, variable.type),
        )
        .join(',') +
      ']'
    )
  ...
  private static getPythonVariable(
    value: string,
    pythonType: string,
    bricksType: BricksVariableType,
  ) {
    ...
    if (bricksType == BricksVariableType.LOOKUP_LIST)
  ...
  private static collectVariableLinesFromCode(
    config: BricksIntegratorConfig,
  ): string[] {
    if (
      this.integratorInputRef ||
      (config.api.moduleId < 0 && config.extendedIntegratorNewParse)
    ) {
  ...
  private static parseVariableLines(
  ...
  private static parseVariableLine(
    ...
    variable.optional = isCommentTrue(
      variable.comment,
      BricksVariableComment.GLOBAL_OPTIONAL,
    )
  ...
  private static getValues(variable: BricksVariable): any[] {
    ...
    if (
      variable.type.startsWith('GENERIC_') ||
      variable.type == BricksVariableType.REGEX
    ) {
      ...
      if (value.charAt(0) == '[') {
        return value
          ...
          .split(',')
          .map((x) => this.parseValue(x.trimStart(), variable.pythonType))
      } else {
  ...
  private static setAddOptions(variable: BricksVariable) {
    if (variable.type == BricksVariableType.GENERIC_BOOLEAN) {
      ...
      variable.values.forEach((e) =>
        variable.options.colors.push(
          e == 'True' ? '#2563eb' : e == 'False' ? '#ef4444' : null,
        ),
      )
    }
  }
  ...
  private static getVariableType(
    ...
    if (!this.filterTypes) {
      this.filterTypes = enumToArray(BricksVariableType).filter(
        (x) => x != BricksVariableType.UNKNOWN && !x.startsWith('GENERIC'),
      )
    }
    ...
    for (let i = 0; i < types.length; i++) {
      ...
      if (
        !labelingTaskId &&
        bricksVariableNeedsTaskId(type as BricksVariableType)
      )
        continue
    ...
    if (variable.pythonType.includes('int'))
      return BricksVariableType.GENERIC_INT
    else if (variable.pythonType.includes('float'))
      return BricksVariableType.GENERIC_FLOAT
    else if (variable.pythonType.includes('str'))
      return BricksVariableType.GENERIC_STRING
    else if (variable.pythonType.includes('bool'))
      return BricksVariableType.GENERIC_BOOLEAN

    return BricksVariableType.UNKNOWN
  }

  public static checkFunctionNameAndSet(
    name: string,
    config: BricksIntegratorConfig,
    executionTypeFilter: string,
    nameLookups: string[],
    ...
    name =
      executionTypeFilter == 'activeLearner'
        ? capitalizeFirstForClassName(name)
        : toPythonFunctionName(name)
  ...
  private static replaceFunctionLine(
    code: string,
    executionTypeFilter: string,
  ): string {
    ...
    const getPythonName =
      executionTypeFilter == 'activeLearner'
        ? getPythonClassName(splitBase[idxReplace])
        : getPythonFunctionName(splitBase[idxReplace])
    splitBase[idxReplace] = splitBase[idxReplace]?.replace(
      getPythonName,
      this.functionName,
    )
  ...
  public static getLabelingTaskAttribute(
    labelingTaskId: string,
    attribute: string,
    labelingTasks: any[] = null,
  ) {
