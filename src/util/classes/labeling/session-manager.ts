...
export class SessionManager {
  ...
  public static readHuddleDataFromLocal() {
    ...
    if (typeof this.huddleData.checkedAt.local == 'string') {
      this.huddleData.checkedAt.local = new Date(
        this.huddleData.checkedAt.local,
      )
    }
    if (
      this.huddleData.linkData.requestedPos !=
      this.labelingLinkData.requestedPos
    ) {
      //url manual changed
  ...
  private static huddleOutdated(): boolean {
    ...
    if (this.huddleData.linkData.projectId != this.labelingLinkData.projectId)
      return true
    for (const key in this.labelingLinkData) {
      if (key == 'linkLocked') continue
      if (this.labelingLinkData[key] != this.huddleData.linkData[key])
        return true
    }
    if (this.huddleData.checkedAt?.local) {
      if (
        new Date().getTime() - this.huddleData.checkedAt.local.getTime() >
        ONE_DAY
      )
        return true
    }
  ...
  public static prepareLabelingSession(projectId: string, huddleData) {
    ...
    if (pos == null && this.huddleData?.linkData.requestedPos)
      pos = this.huddleData.linkData.requestedPos
    if (pos == null) pos = 0
    if (huddleId == DUMMY_HUDDLE_ID && this.huddleData?.linkData.huddleId)
      huddleId = this.huddleData.linkData.huddleId

    if (
      !this.huddleData ||
      this.huddleData.linkData.huddleId != huddleId ||
  ...
  public static changeLinkLockState(state: boolean) {
    ...
    this.absoluteWarning = state
      ? 'his link is locked, contact your supervisor to request access'
      : null
  ...
  public static jumpToPosition(pos: number) {
    ...
    else if (jumpPos > this.huddleData.recordIds.length)
      jumpPos = this.huddleData.recordIds.length
    ...
    this.nextDisabled =
      !this.huddleData || jumpPos == this.huddleData.recordIds.length
    this.prevDisabled = !this.huddleData || jumpIdx == 0
    this.positionString =
      jumpPos + ' / ' + this.huddleData.recordIds.length + ' records in'
  }
  ...
  public static setCurrentRecordDeleted() {
    this.huddleData.recordIds[this.huddleData.linkData.requestedPos - 1] =
      'deleted'
  }

  public static getSourceId(): string {
    if (!this.huddleData) return null
    if (this.huddleData.linkData.linkType != LabelingLinkType.HEURISTIC)
      return null
