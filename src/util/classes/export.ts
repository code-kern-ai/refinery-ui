...
export class ExportHelper {
    ...
    public static buildExportData(sessionId: string, formGroup: any): string {
        ...
        const exportData = {
            ...
            file_type: this.firstActiveInGroup(ExportEnums.ExportFileType, 'value', formGroup),
            format: this.firstActiveInGroup(ExportEnums.ExportFormat, 'value', formGroup),
        };
    ...
    private static buildExportDataRows(sessionId: string, formGroup: any): any {
        let type = this.firstActiveInGroup(ExportEnums.ExportRowType, 'value', formGroup);
        let id;
    ...
    private static buildExportDataColumns(formGroup: any) {
        return {
            labeling_tasks: this.allActive(ExportEnums.LabelingTasks, 'id', formGroup),
    ...
    private static buildExportDataSources(formGroup: any): any[] {
        ...
        for (let source of active) {
            if (source == LabelSource.INFORMATION_SOURCE) {
                sources.push(...this.allActive(ExportEnums.Heuristics, "id", formGroup).map(v => ({ type: source, id: v })));
            }
