...
export class CommentDataManager {
  public static commentRequests: Map<Object, CommentRequest[]> = new Map<
    CurrentPage,
    CommentRequest[]
  >()
  ...
  public static registerCommentRequests(
    currentPage: CurrentPage,
    requests: CommentRequest[],
  ) {
    let comments = [...requests]
    if (this.commentRequests.has(currentPage))
      comments.push(...this.commentRequests.get(currentPage))
  ...
  public static buildRequestJSON(): string {
    ...
    CommentDataManager.commentRequests.forEach((value, key) => {
      value.forEach((commentRequest) => {
        ...
        if (
          !(key in requestJSON) ||
          commentRequest.commentType == CommentType.RECORD
        ) {
          requestJSON[key] =
            this.buildJsonEntryFromCommentRequest(commentRequest)
        }
      })
    })
  ...
  private static buildJsonEntryFromCommentRequest(
  ...
  private static shouldRequestAddInfo(commentRequest: CommentRequest): boolean {
    const projectId = commentRequest.projectId
      ? commentRequest.projectId
      : CommentDataManager.globalProjectId
    const addInfoKey = commentRequest.commentType + '@' + projectId
    if (
      !this.addInfo[addInfoKey] ||
      commentRequest.commentType == CommentType.RECORD
    )
      return true
  ...
  public static parseCommentData(data) {
    ...
    for (const key in data) {
      ...
      // add structure
      if (!(keyParts.commentType in this.data))
        this.data[keyParts.commentType] = {}
      const projectId = keyParts.projectId
        ? keyParts.projectId
        : CommentDataManager.globalProjectId
      if (!(projectId in this.data[keyParts.commentType]))
        this.data[keyParts.commentType][projectId] = {}
      if (
        keyParts.commentKey &&
        !(keyParts.commentKey in this.data[keyParts.commentType][projectId])
      ) {
      ...
      if (data[key].add_info) {
        ...
        if (!this.addInfo[addInfoKey])
          this.addInfo[addInfoKey] = { values: data[key].add_info }
        else {
          for (const addInfo of data[key].add_info) {
            if (
              !this.addInfo[addInfoKey].values.find((i) => i.id == addInfo.id)
            )
              this.addInfo[addInfoKey].values.push(addInfo)
          }
        }
      }
      if (data[key].data) {
        data[key].data.forEach((e) => {
          if (!(e.xfkey in this.data[e.xftype][projectId]))
            this.data[e.xftype][projectId][e.xfkey] = []
          if (
            !this.data[e.xftype][projectId][e.xfkey].find((c) => c.id == e.id)
          )
    ...
    for (const key in this.data) {
      for (const projectId in this.data[key]) {
        for (const commentKey in this.data[key][projectId]) {
          ...
          if (index >= 0) {
            this.data[key][projectId][commentKey] = arr.filter(
              (c) => !c.markedForDeletion,
            )
            removedSomething = true
            if (removedSomething && this.data[key][projectId][commentKey] == 0)
              delete this.data[key][projectId][commentKey]
          }
        }
        if (
          removedSomething &&
          Object.keys(this.data[key][projectId]).length == 0
        )
          delete this.data[key][projectId]
      }
      if (removedSomething && Object.keys(this.data[key]).length == 0)
  ...
  public static parseToCurrentData(
    allUsers: User[],
      ...
      value.forEach((commentRequest) => {
        if (this.data[commentRequest.commentType]) {
          ...
          if (this.data[commentRequest.commentType][projectId]) {
            if (
              commentRequest.commentKey &&
              this.data[commentRequest.commentType][projectId][
                commentRequest.commentKey
              ]
            ) {
              this.addCommentArrToCurrent(
                this.data[commentRequest.commentType][projectId][
                  commentRequest.commentKey
                ],
              )
            ...
            if (!commentRequest.commentKey) {
              ...
              keys.forEach((key) =>
                this.addCommentArrToCurrent(
                  this.data[commentRequest.commentType][projectId][key],
                ),
              )
            }
          }
  ...
  private static extendCurrentDataWithAddInfo(allUsers: User[]) {
    for (const key in this.currentData) {
      ...
      if (!commentData.creationUser) {
        commentData.creationUser = getUserNameFromId(
          commentData.created_by,
          allUsers,
        )
      }
      if (!commentData.xfkeyAdd) {
        commentData.xfkeyAddName = this.getAddFromId(
          commentData.xfkey,
          commentData.xftype,
          commentData.project_id,
        )
        commentData.xfkeyAdd =
          commentTypeToString(commentData.xftype as CommentType, true) +
          ': ' +
          commentData.xfkeyAddName
      }
    }
  }

  private static getAddFromId(
    xfkey: string,
    xftype: string,
    projectId: string,
  ): string {
  ...
  private static buildCurrentDataOrder(allUsers: User[]) {
    ...
    for (var key in this.currentData) {
      const findCurrentUser = allUsers.find(
        (u) => u.id == this.currentData[key].created_by,
      )
      this.currentData[key] = {
        ...this.currentData[key],
        avatarUri: getUserAvatarUri(findCurrentUser),
      }
      const e = this.currentData[key]
      this.currentDataOrder.push(e)
    }
    this.currentDataOrder.sort(
      (a, b) =>
  ...
  private static buildCommentTypeOptions() {
    ...
    CommentDataManager.commentRequests.forEach((value, key) => {
      value.forEach((commentRequest) => {
        dict[commentRequest.commentType.toString()] = {
          name: commentTypeToString(commentRequest.commentType),
          order: commentTypeOrder(commentRequest.commentType),
        }
      })
    })
  ...
  public static removeCommentFromData(
    commentId: string,
      ...
      for (const projectId in this.data[key]) {
        for (const commentKey in this.data[key][projectId]) {
          ...
          if (onlyFlag && index >= 0) {
            ...
            if (removedSomething && arr.length == 0)
              delete this.data[key][projectId][commentKey]
          }
        }
        if (
  ...
  public static modifyCacheFor(
    ...
    if (addInfoKey in this.addInfo) {
      if (reReQuest) {
        ...
        if (xfkey) {
          const item = this.addInfo[addInfoKey].values.find(
            (v) => v.id == xfkey,
          )
          ...
          this.addInfo[addInfoKey].values.forEach(
            (v) => (v.markedForDeletion = true),
          )
          backRequest = {
            commentType: commentType as CommentType,
            projectId: projectId,
