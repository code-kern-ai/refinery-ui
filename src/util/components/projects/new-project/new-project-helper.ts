...
export function checkWhitelistTokenizer(tokenizer: any[], isManaged: boolean) {
  ...
  tokenizer.sort(
    (a, b) =>
      +a.disabled - +b.disabled || a.configString.localeCompare(b.configString),
  )
  ...
  if (insertPos != -1) {
    ...
    if (isManaged) {
      tokenizer.splice(insertPos, 0, {
        disabled: true,
        name: 'if you need the options below feel free to contact us',
        configString: 'email',
      })
    } else {
      tokenizer.splice(insertPos, 0, {
        disabled: true,
        name: 'add further options on config page',
      })
    }
    tokenizer.splice(insertPos, 0, {
      disabled: true,
