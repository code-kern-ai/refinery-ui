...
export function postProcessCurrentHeuristic(
  heuristic: Heuristic,
  ...
  if (
    ...
    if (regMatch[2] !== prepareHeuristic.name) {
      prepareHeuristic.sourceCodeToDisplay =
        prepareHeuristic.sourceCode.replace(regMatch[2], prepareHeuristic.name)
    }
  } else if (
    prepareHeuristic.informationSourceType ==
    ...
    if (regMatch[2] !== prepareHeuristic.name) {
      prepareHeuristic.sourceCodeToDisplay =
        prepareHeuristic.sourceCode.replace(
          regMatch[0],
          getClassLine(
            prepareHeuristic.name,
            labelingTasks,
...
export function getClassLine(
  className: string = null,
  labelingTasks: LabelingTask[],
  currentLabelingTaskId: string,
): string {
  const taskType = labelingTasks.find(
    (a) => a.id == currentLabelingTaskId,
  ).taskType
  if (!className)
    className =
      taskType == LabelingTaskTaskType.INFORMATION_EXTRACTION
        ? 'ATLExtractor'
        : 'ATLClassifier'
  className +=
    taskType == LabelingTaskTaskType.INFORMATION_EXTRACTION
...
export function postProcessLastTaskLogs(lastTask: any) {
  ...
  payload.logs = parseContainerLogsData(
    payload.logs,
    payload.informationSource.type,
  )
  return payload.logs
}
