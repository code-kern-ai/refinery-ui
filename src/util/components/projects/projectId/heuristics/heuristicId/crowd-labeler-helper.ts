...
export function postProcessCrowdLabeler(
  heuristic: Heuristic,
  ...
  prepareHeuristic.labels = labelingTask.labels
  prepareHeuristic.crowdLabelerSettings = parseCrowdSettings(
    prepareHeuristic.sourceCode,
  )
  if (link) {
    prepareHeuristic.crowdLabelerSettings.accessLink = link.link
    prepareHeuristic.crowdLabelerSettings.accessLinkParsed = buildFullLink(
      link.link,
    )
    prepareHeuristic.crowdLabelerSettings.accessLinkLocked = link.isLocked
    prepareHeuristic.crowdLabelerSettings.isHTTPS =
      window.location.protocol == 'https:'
  }
  return prepareHeuristic
}

export function parseCrowdSettings(
  settingsJson: string,
): CrowdLabelerHeuristicSettings {
...
export function parseToSettingsJson(
  settings: CrowdLabelerHeuristicSettings,
): string {
  const tmp = {
