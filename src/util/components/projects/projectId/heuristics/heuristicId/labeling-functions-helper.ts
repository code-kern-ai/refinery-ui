...
export function embeddingRelevant(
  embedding: Embedding,
  attributes: Attribute[],
  labelingTasks: LabelingTask[],
  currentLabelingTaskId: string,
): boolean {
  if (!embedding) return false
  const attributeType = attributes.find(
    (a) => a.id == embedding.attributeId,
  )?.dataType
  if (attributeType != 'TEXT') return false
  const onlyAttribute =
    labelingTasks.find((lt) => lt.id == currentLabelingTaskId)?.taskType ==
    'MULTICLASS_CLASSIFICATION'
  return (
    (embedding.type == 'ON_ATTRIBUTE' && onlyAttribute) ||
    (embedding.type != 'ON_ATTRIBUTE' && !onlyAttribute)
  )
}
...
export function postProcessSampleRecordsFromBE(sampleRecords: any) {
  ...
  prepareSampleRecords.records = prepareSampleRecords.records.map(
    (record: any) => {
      return {
  ...
  prepareSampleRecords.containerLogs = parseContainerLogsData(
    prepareSampleRecords['containerLogs'],
    InformationSourceType.LABELING_FUNCTION,
  )
  return prepareSampleRecords
}

export function postProcessSampleRecords(
  sampleRecords: any,
  labelingTasks: LabelingTask[],
  currentLabelingTaskId: string,
) {
  ...
  prepareSampleRecords.records.forEach((record: any) => {
    ...
    const taskType = labelingTasks.find(
      (lt) => lt.id == currentLabelingTaskId,
    )?.taskType
    if (taskType == LabelingTaskTaskType.MULTICLASS_CLASSIFICATION) {
      const label =
        record.calculatedLabels.length > 0 ? record.calculatedLabels[1] : '-'
      ...
      if (record.calculatedLabels.length > 0) {
        ...
        for (const key in resultDict) {
          resultDict[key].displayAmount =
            displayAmount || resultDict[key].count > 1
        }
      } else {
        resultDict['-'] = {
          label: '-',
  ...
  prepareSampleRecords.records = extendArrayElementsByUniqueId(
    prepareSampleRecords.records,
  )
  return prepareSampleRecords
}
...
function getColorForLabel(
  ...
  const labelingTaskLabels = labelingTasks.find(
    (lt) => lt.id == currentLabelingTaskId,
  ).labels
  const labelColor = labelingTaskLabels.find((el) => el.name == label)?.color
  return label != '-' && labelColor != undefined
    ? labelColor
    : getColorStruct('gray')
}
