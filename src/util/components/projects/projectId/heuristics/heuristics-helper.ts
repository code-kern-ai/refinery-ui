...
import {
  getColorStruct,
  mapInformationSourceStatsGlobal,
} from './shared-helper'
...
export function postProcessHeuristics(
  heuristics: string,
  projectId: string,
): Heuristic[] {
  ...
  return JSON.parse(heuristics).map((source: Heuristic) => {
    ...
    source.routerLink = getRouterLinkHeuristic(
...
export function getInformationSourceTemplate(
  matching: any,
  ...
  if (type == InformationSourceType.LABELING_FUNCTION) {
    templateKey =
      firstLabelingTaskType == LabelingTaskTaskType.INFORMATION_EXTRACTION
        ? InformationSourceExamples.LF_EMPTY_EXTRACTION
        : InformationSourceExamples.LF_EMPTY_CLASSIFICATION
  } else {
    templateKey =
      firstLabelingTaskType == LabelingTaskTaskType.INFORMATION_EXTRACTION
        ? InformationSourceExamples.AL_EMPTY_EXTRACTION
        : InformationSourceExamples.AL_EMPTY_CLASSIFICATION
    replaceEmbedding = true
  }
  const code =
    InformationSourceCodeLookup.getInformationSourceTemplate(templateKey)
  if (replaceEmbedding) {
...
export function getRouterLinkHeuristic(
...
export function checkSelectedHeuristics(
  heuristics: Heuristic[],
  onlyValid: boolean,
) {
  const selected = heuristics.filter((i) => i.selected).length
  if (onlyValid) {
    const selectedFinished = heuristics.filter(
      (i) => i.selected && ['FINISHED', 'STARTED'].includes(i?.state),
    ).length
...
export function postProcessCurrentWeakSupervisionRun(
  currentWeakSupervision: CurrentWeakSupervision,
): CurrentWeakSupervision {
  ...
  if (currentWeakSupervision.user.firstName) {
    prepareWeakSupervision.displayName =
      currentWeakSupervision.user.firstName[0] +
      '. ' +
      currentWeakSupervision.user.lastName
  } else {
    prepareWeakSupervision.displayName = UNKNOWN_USER
  }
  prepareWeakSupervision.createdAtDisplay = parseUTC(
    currentWeakSupervision.createdAt,
  )
  if (currentWeakSupervision.finishedAt) {
    prepareWeakSupervision.finishedAtDisplay = parseUTC(
      currentWeakSupervision.finishedAt,
    )
  } else {
