...
export function addGroupToSearchElement(searchElement, labelingTasks) {
  if (searchElement.groupElements[0].hasOwnProperty('operator')) {
    ...
    searchElement.groupElements = newElement
    searchElement.nameAdd = labelingTasks.find(
      (item: any) => item.id == saveEl.taskId,
    )?.name
...
export function prefillActiveValues(
  parse: any,
  fullSearchStoreCopy: any,
  usersMap: any,
) {
    ...
    if (el[SearchGroup.DRILL_DOWN]) {
      ...
      el.groupElements.forEach((groupItem: any, index: number) => {
        if (groupItem.hasOwnProperty('operator')) {
          if (groupItem.active) {
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].active = true
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ...
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].searchValue = groupItem.searchValue
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].searchValueBetween = groupItem.searchValueBetween
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].negate = groupItem.negate
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].caseSensitive = groupItem.caseSensitive
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].name = groupItem.name
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
            ...
            fullSearchStoreCopy[SearchGroup.ATTRIBUTES].groupElements[
              index
            ].color = getActiveNegateGroupColor(groupItem)
          }
        } else if (groupItem.hasOwnProperty('users')) {
          groupItem.users.forEach((userItem: any) => {
            if (userItem.active && usersMap[userItem.id]) {
              const findIdx = fullSearchStoreCopy[
                SearchGroup.USER_FILTER
              ].groupElements.users.findIndex(
                (item: any) => item.id == userItem.id,
              )
              fullSearchStoreCopy[SearchGroup.USER_FILTER].groupElements.users[
                findIdx
              ].active = true
              fullSearchStoreCopy[SearchGroup.USER_FILTER].groupElements.users[
                findIdx
              ].negate = userItem.negate
              fullSearchStoreCopy[SearchGroup.USER_FILTER].groupElements.users[
                findIdx
              ].name = usersMap[userItem.id]
              fullSearchStoreCopy[SearchGroup.USER_FILTER].groupElements.users[
                findIdx
              ].id = userItem.id
              fullSearchStoreCopy[SearchGroup.USER_FILTER].groupElements.users[
                findIdx
              ].dataTip = usersMap[userItem.id]
              fullSearchStoreCopy[SearchGroup.USER_FILTER].groupElements.users[
                findIdx
              ].color = getActiveNegateGroupColor(userItem)
            }
          })
          ...
          groupItem.orderBy.forEach((orderByItem: any) => {
            if (orderByItem.active) {
              ...
              ].groupElements.orderBy.findIndex(
                (item: any) => item.id == orderByItem.id,
              )
              if (findIdx == -1) return
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].active = true
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].direction = orderByItem.direction
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].orderByKey =
                orderByItem.orderByKey
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].displayName =
                orderByItem.displayName
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].id = orderByItem.id
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].seedString =
                orderByItem.seedString
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].isAttribute =
                orderByItem.isAttribute
              fullSearchStoreCopy[
                SearchGroup.ORDER_STATEMENTS
              ].groupElements.orderBy[findIdx].color =
                getActiveNegateGroupColor(orderByItem)
            }
          })
          ...
          if (groupItem.hasComments[0].active) {
            ...
            ].groupElements.hasComments.active = true
            fullSearchStoreCopy[
              SearchGroup.COMMENTS
            ].groupElements.hasComments.negate = groupItem.hasComments[0].negate
            fullSearchStoreCopy[
              SearchGroup.COMMENTS
            ].groupElements.hasComments.color = getActiveNegateGroupColor(
              groupItem.hasComments[0],
            )
          }
          ...
          manualLabels.forEach((manualLabel: any, index: number) => {
            if (manualLabel.active) {
              ...
              fullSearchStoreCopy[key].groupElements.manualLabels[
                index
              ].active = true
              fullSearchStoreCopy[key].groupElements.manualLabels[
                index
              ].negate = manualLabel.negate
              fullSearchStoreCopy[key].groupElements.manualLabels[index].name =
                manualLabel.name
              fullSearchStoreCopy[key].groupElements.manualLabels[index].id =
                manualLabel.id
            }
          })
        } else if (groupItem.hasOwnProperty('weakSupervisionLabels')) {
          const weakSupervisionLabels = groupItem.weakSupervisionLabels
          weakSupervisionLabels.forEach(
            (weakSupervisionLabel: any, index: number) => {
              if (weakSupervisionLabel.active) {
                const key = SearchGroup.LABELING_TASKS + '_' + groupItem.taskId
                fullSearchStoreCopy[key].groupElements.weakSupervisionLabels[
                  index
                ].active = true
                fullSearchStoreCopy[key].groupElements.weakSupervisionLabels[
                  index
                ].negate = weakSupervisionLabel.negate
                fullSearchStoreCopy[key].groupElements.weakSupervisionLabels[
                  index
                ].name = weakSupervisionLabel.name
                fullSearchStoreCopy[key].groupElements.weakSupervisionLabels[
                  index
                ].id = weakSupervisionLabel.id
              }
            },
          )
        } else if (groupItem.hasOwnProperty('modelCallbackLabels')) {
          const modelCallbackLabels = groupItem.modelCallbackLabels
          modelCallbackLabels.forEach(
            (modelCallbackLabel: any, index: number) => {
              if (modelCallbackLabel.active) {
                const key = SearchGroup.LABELING_TASKS + '_' + groupItem.taskId
                fullSearchStoreCopy[key].groupElements.modelCallbackLabels[
                  index
                ].active = true
                fullSearchStoreCopy[key].groupElements.modelCallbackLabels[
                  index
                ].negate = modelCallbackLabel.negate
                fullSearchStoreCopy[key].groupElements.modelCallbackLabels[
                  index
                ].name = modelCallbackLabel.name
                fullSearchStoreCopy[key].groupElements.modelCallbackLabels[
                  index
                ].id = modelCallbackLabel.id
              }
            },
          )
        } else if (
          groupItem.hasOwnProperty('informationSources') ||
          groupItem.hasOwnProperty('heuristics')
        ) {
          const heuristics = groupItem.informationSources
          heuristics.forEach((heuristic: any, index: number) => {
            if (heuristic.active) {
              const key = SearchGroup.LABELING_TASKS + '_' + groupItem.taskId
              fullSearchStoreCopy[key].groupElements.heuristics[index].active =
                true
              fullSearchStoreCopy[key].groupElements.heuristics[index].negate =
                heuristic.negate
              fullSearchStoreCopy[key].groupElements.heuristics[index].name =
                heuristic.name
              fullSearchStoreCopy[key].groupElements.heuristics[index].id =
                heuristic.id
            }
          })
        } else if (groupItem.hasOwnProperty('isWithDifferentResults')) {
          const key = SearchGroup.LABELING_TASKS + '_' + groupItem.taskId
          fullSearchStoreCopy[key].groupElements.isWithDifferentResults.active =
            groupItem.isWithDifferentResults.active
          fullSearchStoreCopy[key].groupElements.isWithDifferentResults.taskId =
            groupItem.isWithDifferentResults.taskId
          fullSearchStoreCopy[
            key
          ].groupElements.isWithDifferentResults.taskType =
            groupItem.isWithDifferentResults.taskType
        } else if (groupItem.hasOwnProperty('confidence')) {
          const key = SearchGroup.LABELING_TASKS + '_' + groupItem.taskId
          fullSearchStoreCopy[
            key
          ].groupElements.weakSupervisionConfidence.active =
            groupItem.confidence.active
          fullSearchStoreCopy[
            key
          ].groupElements.weakSupervisionConfidence.upper =
            groupItem.confidence.upper
          fullSearchStoreCopy[
            key
          ].groupElements.weakSupervisionConfidence.lower =
            groupItem.confidence.lower
          fullSearchStoreCopy[
            key
          ].groupElements.weakSupervisionConfidence.negate =
            groupItem.confidence.negate
        }
      })
    }
  })
...
export function checkActiveGroups(group: any, searchGroup: any) {
  for (let [key, value] of Object.entries(group)) {
    ...
    if (Array.isArray(val2.groupElements)) {
      ...
      const findActive = val2.groupElements.users.filter(
        (item: any) => item.active,
      )
      if (findActive.length > 0) searchGroup[key].isOpen = true
    } else if (val2.groupElements.hasOwnProperty('orderBy')) {
      const findActive = val2.groupElements.orderBy.filter(
        (item: any) => item.active,
      )
      if (findActive.length > 0) searchGroup[key].isOpen = true
      ...
      const findActive = val2.groupElements.manualLabels.filter(
        (item: any) => item.active,
      )
      if (findActive.length > 0) searchGroup[key].isOpen = true
    } else if (val2.groupElements.hasOwnProperty('weakSupervisionLabels')) {
      const findActive = val2.groupElements.weakSupervisionLabels.filter(
        (item: any) => item.active,
      )
      if (findActive.length > 0) searchGroup[key].isOpen = true
    } else if (val2.groupElements.hasOwnProperty('modelCallbackLabels')) {
      const findActive = val2.groupElements.modelCallbackLabels.filter(
        (item: any) => item.active,
      )
      if (findActive.length > 0) searchGroup[key].isOpen = true
    } else if (val2.groupElements.hasOwnProperty('heuristics')) {
      const findActive = val2.groupElements.heuristics.filter(
        (item: any) => item.active,
      )
      if (findActive.length > 0) searchGroup[key].isOpen = true
    } else if (
      (val2.groupElements.weakSupervisionConfidence &&
        val2.groupElements.weakSupervisionConfidence.active) ||
      (val2.groupElements.modelCallbackConfidence &&
        val2.groupElements.modelCallbackConfidence.active) ||
      (val2.groupElements.isWithDifferentResults &&
        val2.groupElements.isWithDifferentResults.active)
    ) {
