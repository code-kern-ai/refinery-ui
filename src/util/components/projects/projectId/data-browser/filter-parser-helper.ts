...
import {
  getAttributeType,
  parseFilterElements,
  prepareFilterElements,
  prepareOperator,
} from './search-operators-helper'
...
export function parseFilterToExtended(
  activeSearchParams,
  ...
  for (let searchElement of activeSearchParams) {
    if (searchElement.values.group == SearchGroup.ATTRIBUTES) {
      attributeFilter = buildFilterElementAttribute(
        first,
        searchElement,
        attributes,
        configuration,
      ...
      appendBlackAndWhiteListLabelingTask(
        toReturn,
      ...
      toReturn.push(
        appendBlackAndWhiteListUser(toReturn, searchElement, drillDownVal),
      )
      toReturn = toReturn.filter((el) => el != null)
    } else if (searchElement.values.group == SearchGroup.ORDER_STATEMENTS) {
      orderBy.ORDER_BY = appendActiveOrderBy(
        searchElement.values.orderBy,
        orderBy,
      )
      ...
      toReturn.push(
        appendBlackAndWhiteListComments(toReturn, searchElement, user),
      )
    }
  }
...
function buildFilterElementAttribute(
  first: boolean,
  searchElement: any,
  attributes: Attribute[],
  configuration,
  ...
  if (searchElement.values.name == 'Any Attribute') {
    ...
    for (let i = 1; i < attributes.length; i++) {
      ...
      if (attributes[i].dataType != DataTypeEnum.BOOLEAN) {
        const filterValues = prepareFilterElements(
          searchEl,
          attributes[i].name,
    ...
    let parseArray = []
    if (
      isNaN(parseInt(searchElement.values.searchValue)) ||
      isNaN(parseFloat(searchElement.values.searchValue))
    ) {
    ...
    const attributeType = attributes.find(
      (a) => a.name == searchElement.values.name,
    ).dataType
    const searchEl = jsonCopy(searchElement)
    searchEl.values.operator = searchEl.values.operator.split(' ').join('_')
    filterElement = {
      ...
      VALUES: prepareFilterElements(
        searchEl,
        searchEl.values.name,
        configuration.separator,
        attributeType,
      ),
    }
...
function appendBlackAndWhiteListLabelingTask(
  appendTo,
  searchElement,
  labelingTasks: LabelingTask[],
  drillDownVal: boolean,
  ...
  const labelingTask = labelingTasks.find(
    (l) => l.id == searchElement.values.taskId,
  )
  appendTo.push(
    appendBlackAndWhiteListLabelingTaskForArray(
      appendTo,
  ...
  appendTo.push(
    appendBlackAndWhiteListLabelingTaskForArray(
      appendTo,
      searchElement.values.heuristics,
  ...
  if (labelingTask) {
    appendTo.push(
      appendBlackAndWhiteListLabelingTaskForConfidence(
        appendTo,
        searchElement.values.weakSupervisionConfidence,
        labelingTask.labels.map((l) => l.id),
        true,
      ),
    )
    appendTo.push(
  ...
  if (isMixed.active) {
    let whitelist = {
      ...
      SUBQUERIES: [
        {
          QUERY_TEMPLATE:
            isMixed.taskType == 'MULTICLASS_CLASSIFICATION'
              ? 'SUBQUERY_RLA_DIFFERENT_IS_CLASSIFICATION'
              : 'SUBQUERY_RLA_DIFFERENT_IS_EXTRACTION',
...
function appendBlackAndWhiteListLabelingTaskForArray(
  ...
  if (whitelist.SUBQUERIES.length > 0) return JSON.stringify(whitelist)
  if (blacklist.SUBQUERIES.length > 0) return JSON.stringify(blacklist)
  return null
}
...
function appendBlackAndWhiteListLabelingTaskForConfidence(
  ...
  const source = forWeakSupervision
    ? LabelSource.WEAK_SUPERVISION
    : LabelSource.MODEL_CALLBACK
  let whitelist = {
    SUBQUERY_TYPE: 'WHITELIST',
  ...
  const query = forWeakSupervision
    ? 'SUBQUERY_RLA_CONFIDENCE'
    : 'SUBQUERY_CALLBACK_CONFIDENCE'
...
function appendBlackAndWhiteListUser(appendTo, searchElement, drillDownVal) {
  return appendBlackAndWhiteListUserForArray(
    appendTo,
    searchElement.users,
    drillDownVal,
  )
  ...
  for (const element of values.orderBy) {
    ...
    if (!orderBy.ORDER_BY.includes(key)) {
      ...
      if (key == StaticOrderByKeys.RANDOM) {
        ...
        orderBy.ORDER_DIRECTION.push(element.direction == 1 ? 'ASC' : 'DESC')
      }
    }
  }
...
function appendBlackAndWhiteListComments(
  appendTo: string[],
  searchElement: any,
  user,
): any {
