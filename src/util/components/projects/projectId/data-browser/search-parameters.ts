...
export function updateSearchParameters(
  ...
  searchElement.forEach((p) => {
    ...
    if (
      Array.isArray(p.groupElements) &&
      p.groupElements[0].group == SearchGroup.ATTRIBUTES
    ) {
      for (let i of p.groupElements) {
        if (!i.active) return
        param = createSplittedText(
          updateSearchParamText(
            i,
            attributes,
            separator,
      ...
      if (isOneActive) {
        activeParams.push({
          splittedText: param,
          values: { group: p.groupElements.group },
          users: p.groupElements.users,
        })
      }
      ...
      for (let i of p.groupElements.orderBy) {
        ...
        activeParams.push({
          splittedText: param,
          values: { group: p.groupElements.group, orderBy: p.groupElements },
        })
      }
    } else if (p.groupElements.group == SearchGroup.LABELING_TASKS) {
      if (
        !(
          p.groupElements.active ||
          (p.groupElements['weakSupervisionConfidence'] &&
            p.groupElements['weakSupervisionConfidence'].active) ||
          (p.groupElements['modelCallbackConfidence'] &&
            p.groupElements['modelCallbackConfidence'].active) ||
          p.groupElements['isWithDifferentResults']
        )
      )
        return
      param = createSplittedText(
      ...
      activeParams.push({
        splittedText: param,
        values: { group: p.groupElements.group, values: p.groupElements },
      })
    }
  })
  return activeParams
...
function updateSearchParamText(
  searchElement,
  attributes,
  separator,
  drillDownVal,
  ...
  if (searchElementCopy.group == SearchGroup.ATTRIBUTES) {
    const attributeType = attributes.find(
      (att) => att.name == searchElementCopy.name,
    )?.dataType
    if (searchElementCopy.operator == SearchOperator.BETWEEN) {
      if (attributeType == 'INTEGER' || attributeType == 'FLOAT') {
        searchElementCopy.searchText =
          ...
          '' +
          ' AND ' +
          searchElementCopy.searchValueBetween
      } else {
        searchElementCopy.searchText =
          searchElementCopy.name +
          ' ' +
      ...
      searchElementCopy.operator == SearchOperator.IN ||
      searchElementCopy.operator == 'IN WC' ||
      searchElementCopy.operator == SearchOperator.IN_WC
    ) {
      if (attributeType == 'INTEGER' || attributeType == 'FLOAT') {
        searchElementCopy.searchText =
          searchElementCopy.name +
        ...
        const splitTextBySeparator = searchElementCopy.searchValue
          .split(separator)
          .filter((i) => i)
        searchElementCopy.searchText =
          searchElementCopy.name +
          ...
          searchElementCopy.operator +
          ' (' +
          splitTextBySeparator.map((x) => "'" + x + "'").join(', ') +
          ')'
      }
    } else {
      if (attributeType == 'INTEGER' || attributeType == 'FLOAT') {
    ...
    if (searchElementCopy.negate)
      searchElementCopy.searchText =
        'NOT (' + searchElementCopy.searchText + ')'
    if (separator == '-')
    ...
    searchElementCopy.groupElements.group == SearchGroup.LABELING_TASKS
  ) {
    searchElementCopy.searchText =
      searchElement.nameAdd +
      labelingTaskBuildSearchParamText(
        searchElementCopy.groupElements,
        drillDownVal,
      )
    if (
      labelingTaskBuildSearchParamText(
    ...
    searchElementCopy.searchText = userBuildSearchParamText(
      searchElementCopy.groupElements.users,
      drillDownVal,
    )
  } else if (
    searchElementCopy.groupElements.group == SearchGroup.ORDER_STATEMENTS
  ) {
    searchElementCopy.searchText = orderByBuildSearchParamText(
      searchElementCopy.groupElements,
    )
  } else if (searchElementCopy.groupElements.group == SearchGroup.COMMENTS) {
    searchElementCopy.searchText = commentsBuildSearchParamText(
      searchElementCopy.groupElements,
    )
  }
...
function labelingTaskBuildSearchParamText(values, drillDownVal): string {
  ...
  let tmp = labelingTaskBuildSearchParamTextPart(
    values.manualLabels,
    'M-label',
    drillDownVal,
  )
  ...
  tmp = labelingTaskBuildSearchParamTextPart(
  ...
  if (
    values.weakSupervisionConfidence &&
    values.weakSupervisionConfidence.active
  ) {
    text += (text ? '\nAND ' : '') + 'WS-Confidence '
    if (values.weakSupervisionConfidence.negate) text += 'NOT '
    text +=
      'BETWEEN ' +
      values.weakSupervisionConfidence.lower +
      '% AND ' +
      values.weakSupervisionConfidence.upper +
      '%'
  }
  ...
  if (values.modelCallbackConfidence && values.modelCallbackConfidence.active) {
    ...
    text +=
      'BETWEEN ' +
      values.modelCallbackConfidence.lower +
      '% AND ' +
      values.modelCallbackConfidence.upper +
      '%'
  }
...
function labelingTaskBuildSearchParamTextPart(
  arr: any[],
  blockname: string,
  drillDownVal: boolean,
): string {
  if (!arr) return ''
...
function orderByBuildSearchParamText(values): string {
  ...
  for (const element of values.orderBy) {
    if (element.active) {
      ...
      if (
        element.displayName != getOrderByDisplayName(StaticOrderByKeys.RANDOM)
      ) {
  ...
  switch (searchElement.values.operator) {
    case SearchOperator.EQUAL:
      return {
        ...
        matchCase: searchElement.values.caseSensitive,
      }
