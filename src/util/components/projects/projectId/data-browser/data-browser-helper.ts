...
import {
  ...
  labelSourceToString,
  sliceTypeToString,
} from '@/submodules/javascript-functions/enums/enum-functions'
import {
  LabelSource,
  Slice,
...
export function postProcessDataSlices(dataSlices: DataSlice[]) {
  ...
  prepareDataSlices.forEach((slice) => {
    slice.displayName =
      slice.sliceType != Slice.STATIC_OUTLIER
        ? slice.name
        : parseUTC(slice.createdAt, true)
...
export function updateSliceInfoHelper(
  slice: DataSlice,
  projectId: string,
  users: User[],
) {
  ...
  if (findById) {
    sliceInfo['Created by'] = findById.firstName + ' ' + findById.lastName
  }
  ...
  if (slice.sliceType == Slice.STATIC_DEFAULT) {
    ...
    sliceInfo['Link'] = buildFullLink(
      '/projects/' + projectId + '/labeling/' + slice.id,
    )
  }
  return sliceInfo
}

export function postProcessUsersCount(
  usersCount: any,
  users: User[],
  currentUser: User,
) {
  ...
  users.forEach((user, index) => {
    ...
    const userCopy = { ...user }
    if (prepareUsersCount.counts)
      prepareUsersCount.counts.forEach((e) => {
        sum += e.count
...
export function postProcessRecordsExtended(
...
export function parseRecordData(element, labelingTasks: LabelingTask[]) {
  if (element.rla_data) {
    ...
    for (const rlaLine of element.rla_data) {
      ...
      if (!element.rla_aggregation.hasOwnProperty(rlaAggParts.key)) {
        element.rla_aggregation[rlaAggParts.key] = {
          ...
          isWSRelated: !(
            rlaLine.source_type == LabelSource.INFORMATION_SOURCE &&
            !rlaLine.weak_supervision_id
          ),
          id: rlaAggParts.key,
        }
      }
      ...
      if (
        rlaLine.confidence != null &&
        (rlaLine.source_type == LabelSource.WEAK_SUPERVISION ||
          rlaLine.source_type == LabelSource.MODEL_CALLBACK)
      ) {
        element.rla_aggregation[rlaAggParts.key].confidence.push(
          rlaLine.confidence,
        )
      }
    }
    ...
    for (const key in element.rla_aggregation) {
      ...
      let sum = 0
      for (const confidence of element.rla_aggregation[key].confidence)
        sum += confidence
      element.rla_aggregation[key].confidenceAvg =
        Math.round(
          (sum / element.rla_aggregation[key].confidence.length) * 10000,
        ) /
          100 +
        '%'
    ...
    if (len && len != countWsRelated) {
      if (len - countWsRelated == 1)
        element.wsHint =
          "1 element isn't visible because of your config settings"
      else
        element.wsHint =
          len -
          countWsRelated +
          " elements aren't visible because of your config settings"
    } else element.wsHint = ''
  }
  return element.rla_aggregation
}

export function getRlaAggregationKeyParts(
  rlaLine,
  labelingTasks: LabelingTask[],
) {
  let parts = getTaskAndLabelNameFromLabelId(
    rlaLine.labeling_task_label_id,
    labelingTasks,
  )
  if (rlaLine.source_type == LabelSource.INFORMATION_SOURCE) {
    ...
    rlaLine.source_type == LabelSource.MANUAL &&
    rlaLine.is_gold_star
  ) {
...
export function getTaskAndLabelNameFromLabelId(
  labelId: string,
  labelingTasks: LabelingTask[],
): any {
...
export function getInformationSourceTextById(
  sourceId: string,
  labelingTasks: LabelingTask[],
) {
  for (const labelingTask of labelingTasks) {
    for (const source of labelingTask.informationSources) {
      if (sourceId == source.id)
        return (
          informationSourceTypeToString(source.type, true) + ': ' + source.name
        )
    }
  }
  return 'UNKNOWN'
...
export function postProcessUniqueValues(
  uniqueValues: any,
  attributesSortOrder: any,
) {
