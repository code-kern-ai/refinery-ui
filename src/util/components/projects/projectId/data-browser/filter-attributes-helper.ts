...
export function prepareFilterAttributes(
  ...
  if (embedding.filterAttributes && embedding.filterAttributes.length == 0)
    return null
  return embedding.filterAttributes
}

export function filterAttributesSSGroup(
  filterAttributesSS,
  operatorsDict,
  attributes,
) {
  return {
    ...
    searchValue: '',
    searchValueBetween: '',
    addText: filterAttributesSS
      ? getPlaceholderText(getAttributeType(attributes, filterAttributesSS[0]))
      : '',
  }
}
...
export function prepareAttFilter(
  filterAttributesForm: any,
  attributes: Attribute[],
) {
  const filter = []
  if (filterAttributesForm.length == 0 || !filterAttributesForm[0].name)
  ...
  for (let i = 0; i < filterAttributesForm.length; i++) {
    ...
    if (attribute.operator !== FilterIntegrationOperator.IN) {
      attribute.searchValue = parseSearchValue(
        attribute.name,
    ...
    if (attribute.operator === FilterIntegrationOperator.IN) {
      ...
      attribute.searchValueBetween = parseSearchValue(
        attribute.name,
        attribute.searchValueBetween,
        attributes,
      )
