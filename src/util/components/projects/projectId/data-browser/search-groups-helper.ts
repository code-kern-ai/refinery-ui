...
import {
  SearchGroupItem,
  SearchItemType,
} from '@/src/types/components/projects/projectId/data-browser/search-groups'
...
export function labelingTasksCreateSearchGroup(
  item,
  task: LabelingTask,
  globalSearchGroupCount: number,
) {
  return {
    id: globalSearchGroupCount,
    group: item.group,
    ...
    sortByWeakSupervisionConfidence: getOrderByGroup(
      StaticOrderByKeys.WEAK_SUPERVISION_CONFIDENCE,
      false,
      -1,
    ),
    sortByModelCallbackConfidence: getOrderByGroup(
      StaticOrderByKeys.MODEL_CALLBACK_CONFIDENCE,
      false,
      -1,
    ),
...
function labelingTaskLabelArray(task: LabelingTask) {
  ...
  for (let l of task.labels) {
    ;(noLabelItemExists =
      noLabelItemExists || l.id === 'NO_LABEL' ? true : false),
      array.push({
        id: l.id,
        name: l.name,
...
function labelingTaskHeuristicArray(task: LabelingTask) {
  ...
  for (let l of task.informationSources) {
    if (
      l.type == InformationSourceType.LABELING_FUNCTION ||
      l.type == InformationSourceType.ACTIVE_LEARNING ||
      l.type == InformationSourceType.ZERO_SHOT ||
      l.type == InformationSourceType.CROWD_LABELER ||
      l.type === undefined
    ) {
      array.push({
        id: l.id,
...
export function orderByCreateSearchGroup(
  item,
  globalSearchGroupCount,
  attributesSortOrder,
  attributesDict,
) {
  return {
...
function orderByArray(attributesSortOrder: any[] = [], attributesDict: any) {
  ...
  array.push(
    getOrderByGroup(StaticOrderByKeys.WEAK_SUPERVISION_CONFIDENCE, false, -1),
  )
  array.push(
    getOrderByGroup(StaticOrderByKeys.MODEL_CALLBACK_CONFIDENCE, false, -1),
  )
...
export function generateRandomSeed() {
  ...
  for (var i = 0; i < length; i++) {
    seed += SEED_CHARACTERS.charAt(Math.floor(Math.random() * charactersLength))
  }
  return seed
}
