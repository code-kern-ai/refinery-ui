...
import {
  ALL_USERS_USER_ID,
  GOLD_STAR_USER_ID,
} from './labeling-main-component-helper'
import {
  canDeleteRla,
  filterRlaLabelCondition,
  getCreatedByName,
  getHoverGroupsOverviewTable,
  getLabelSourceOrder,
} from './overview-table-helper'
import {
...
export function buildLabelingRlaData(
  rlas: any,
  user: User,
...
function getLabelDataTip(e: any): string {
  if (e.sourceType == LabelSource.INFORMATION_SOURCE)
    return informationSourceTypeToString(e.informationSource.type, false)
  else if (e.sourceType == LabelSource.WEAK_SUPERVISION)
    return 'Weak supervision - click to use as manual label'
...
function getLabelForDisplay(e: any, showHeuristicConfidence: boolean): string {
  ...
  if (e.sourceType == LabelSource.WEAK_SUPERVISION && e.confidence != null) {
    final +=
      ' - ' + Math.round((e.confidence + Number.EPSILON) * 10000) / 100 + '%'
  } else if (
    showHeuristicConfidence &&
    e.sourceType == LabelSource.INFORMATION_SOURCE &&
    e.confidence != null
  ) {
    final +=
      ' - ' + Math.round((e.confidence + Number.EPSILON) * 10000) / 100 + '%'
...
function getIcon(e: any): string {
  if (e.sourceType == LabelSource.INFORMATION_SOURCE)
    return e.informationSource.type
  else if (e.sourceType != LabelSource.MANUAL) return e.sourceType
  return null
}

export function filterRlaDataForLabeling(
  data: any[],
  settings,
  projectId,
  rlaKey?: string,
...
export function getFirstFitPos(
  takenPositions: any,
  start: number,
  end: number,
...
function checkFit(
  ...
  for (let i = start; i <= end; i++) {
    if (
      takenPositions[i] &&
      takenPositions[i].rlaArray.find((e) => e.orderPos == pos)
    )
      return false
  }
  return true
...
export function findOrderPosItem(
  orderPosElement: any,
  compareItem: any,
): boolean {
...
export function collectSelectionData(
  attributeId: string,
...
export function getGoldInfoForTask(
  task: any,
  user: User,
  fullRlaData: any,
  displayUserId: string,
  userDisplayRole: UserRole,
): { can: boolean; is: boolean } {
  if (
    user &&
    user.role != UserRole.ENGINEER &&
    userDisplayRole != UserRole.ENGINEER
  )
    return { can: false, is: false }
  if (displayUserId == ALL_USERS_USER_ID) return { can: false, is: false }
  if (task.task.taskType == LabelingTaskTaskType.NOT_USEABLE)
    return { can: false, is: false }
  const taskRlaData = fullRlaData.filter(
    (x) => x.sourceTypeKey == LabelSource.MANUAL && x.taskId == task.task.id,
  )
  const goldRlas = taskRlaData.filter((x) => x.rla.isGoldStar)
  if (displayUserId == GOLD_STAR_USER_ID)
    return { can: goldRlas.length > 0, is: goldRlas.length > 0 }

  const userRlas = taskRlaData.filter(
    (x) => x.createdBy == displayUserId && !x.rla.isGoldStar,
  )
  if (userRlas.length == 0) return { can: false, is: false }
  const otherRlas = taskRlaData.filter(
    (x) => x.createdBy != displayUserId && !x.rla.isGoldStar,
  )
  ...
  return {
    can: !allMatch,
    is: goldRlas.length > 0 && allRlasMatch(goldRlas, userRlas),
  }
}
...
function rlaIsEqual(rlaA: any, rlaB: any): boolean {
  ...
  if (
    rlaA.returnType == InformationSourceReturnType.YIELD &&
    (rlaA.tokenStartIdx != rlaB.tokenStartIdx ||
      rlaA.tokenEndIdx != rlaB.tokenEndIdx)
  )
    return false
...
export function parseSelectionData(): any[] {
  ...
  if (
    attributeIdStart == null ||
    attributeIdEnd == null ||
    attributeIdStart != attributeIdEnd
  )
    return [false]
...
function getSelectionElement(
  ...
  while (steps < maxSteps) {
    if (
      ...
      element.getAttribute('data-tokenidx') != null
    )
      return element
...
export function checkCanEditLabels(
  user: User,
  userDisplayRole: UserRole,
  displayUserId: string,
): boolean {
  ...
  return (
    (displayUserId == GOLD_STAR_USER_ID &&
      userDisplayRole == UserRole.ENGINEER) ||
    (displayUserId == ALL_USERS_USER_ID &&
      userDisplayRole == UserRole.ENGINEER) ||
