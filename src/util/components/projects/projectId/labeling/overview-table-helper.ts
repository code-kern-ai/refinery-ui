...
import {
  ALL_USERS_USER_ID,
  GOLD_STAR_USER_ID,
  getDefaultTaskOverviewLabelSettings,
...
export function buildOverviewTableDisplayArray(
  rlas,
  user: User,
  userDisplayRole,
): TableDisplayData[] {
...
export function getHoverGroupsOverviewTable(data: any): any {
  const all: any = {
    ...
    type:
      'TY_' +
      (data.sourceType == LabelSource.INFORMATION_SOURCE
        ? informationSourceTypeToString(data.informationSource.type, false)
        : labelSourceToString(data.sourceType)),
    label: 'LA_' + data.labelingTaskLabel.id,
    createdBy:
      'CR_' +
      (data.sourceType == LabelSource.INFORMATION_SOURCE
        ? data.informationSource.name
        : data.user.id),
    rlaId: 'ID_' + data.id,
  }
  return {
...
export function getLabelSourceOrder(
  source: LabelSource,
  isType?: InformationSourceType,
): number {
  switch (source) {
    case LabelSource.MANUAL:
      return 0
    ...
    case LabelSource.INFORMATION_SOURCE:
      switch (isType) {
        ...
        case InformationSourceType.ACTIVE_LEARNING:
          return 31
        case InformationSourceType.PRE_COMPUTED:
          return 32
...
export function getCreatedByName(e: any): string {
  if (e.sourceType == LabelSource.INFORMATION_SOURCE)
    return e.informationSource.name
...
export function canDeleteRla(rla, user, userDisplayRole): boolean {
  ...
  if (user.role != userDisplayRole && userDisplayRole != UserRole.ENGINEER)
    return false
...
export function filterRlaDataForUser(
  rlaData: any[],
  user: User,
  ...
  if (rlaKey)
    return rlaData.filter((entry) =>
      filterRlaCondition(entry[rlaKey], user, displayUserId, userDisplayRole),
    )
  return rlaData.filter((rla) =>
    filterRlaCondition(rla, user, displayUserId, userDisplayRole),
  )
}

export function filterRlaCondition(
  rla,
  user,
  displayUserId,
  userDisplayRole,
): boolean {
  if (user.role != UserRole.ENGINEER || userDisplayRole != UserRole.ENGINEER)
    return (
      rla.sourceType == LabelSource.MANUAL && rla.createdBy == displayUserId
    )
...
export function filterRlaLabelCondition(
  rla: any,
  settings,
  projectId,
): boolean {
  ...
  let rlaSettings: LabelingSuiteTaskHeaderLabelSettings =
    taskSettings[rla.labelingTaskLabelId]
  if (!rlaSettings) {
...
export function getShouldHighlightOn(rla: any): string[] {
  const createdBy =
    rla.sourceType == LabelSource.INFORMATION_SOURCE
      ...
      : rla.user.firstName + ' ' + rla.user.lastName
  return [
    rla.id,
    rla.labelingTaskLabel.id,
    rla.sourceType,
    rla.labelingTaskLabel.labelingTask.id,
    createdBy,
  ]
}
