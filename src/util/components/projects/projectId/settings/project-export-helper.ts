...
export function postProcessingFormGroups(
  projectSize: any,
  ...
  projectSize.forEach((element: any) => {
    ...
    if (element.table == ProjectExportGroup.EMBEDDING_TENSORS) {
      hasGdpr = embeddings.some(
        (e: any) =>
          e.name.split('-')[2] == PlatformType.COHERE ||
          e.name.split('-')[2] == PlatformType.OPEN_AI ||
          e.name.split('-')[2] == PlatformType.AZURE,
      )
    }
    let group = {
