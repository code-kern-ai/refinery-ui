...
export function postProcessingEmbeddings(
  embeddings: Embedding[],
  queuedEmbeddings: any[],
): any[] {
  ...
  queuedEmbeddings.forEach((task: any) => {
    preparedEmbeddings.push({
      ...
      custom: false,
      type:
        task.taskInfo['type'] == EmbeddingType.ON_ATTRIBUTE
...
export function postProcessingEmbeddingPlatforms(
  platforms: EmbeddingPlatform[],
...
export function postProcessingRecommendedEncoders(
  attributes: Attribute[],
  tokenizer: string,
  encoderSuggestions: any,
): { [embeddingId: string]: RecommendedEncoder } {
  let embeddingHandles: { [embeddingId: string]: any } = {}
  let encoderSuggestionsCopy = []
  encoderSuggestions = encoderSuggestions.filter(
    (e) => e.tokenizers.includes('all') || e.tokenizers.includes(tokenizer),
  )
  ...
  if (encoderSuggestions)
    encoderSuggestions.forEach((element) => {
      element = { ...element }
      if (
        typeof element.applicability === 'string' ||
        element.applicability instanceof String
      ) {
...
function buildExpectedEmbeddingName(data: any): string {
  ...
  toReturn +=
    '-' +
    (data.granularity.value == EmbeddingType.ON_ATTRIBUTE
      ? 'classification'
      : 'extraction')
  const platform = data.platform
  if (
    platform == PlatformType.HUGGING_FACE ||
    platform == PlatformType.PYTHON
  ) {
    toReturn += '-' + platform + '-' + data.model
  } else if (
    platform == PlatformType.OPEN_AI ||
    platform == PlatformType.COHERE ||
...
function buildEmbeddingNameWithApiToken(data: any) {
  ...
  const apiTokenCut =
    data.apiToken.substring(0, 3) +
    '...' +
    data.apiToken.substring(data.apiToken.length - 4, data.apiToken.length)
  if (
    data.platform == PlatformType.OPEN_AI ||
    data.platform == PlatformType.AZURE
  )
    return platformStr + data.model + '-' + apiTokenCut
...
export function checkIfCreateEmbeddingIsDisabled(
  props: EmbeddingCreationEnabledProps,
) {
  ...
  if (
    ...
    checkFormFields =
      model == null || apiToken == null || apiToken == '' || !termsAccepted
  ...
  const data = {
    ...
    platform: props.embeddingPlatforms.find(
      (p: EmbeddingPlatform) => p.name == platform.name,
    )?.platform,
