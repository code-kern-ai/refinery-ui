...
import {
  AttributeCalculationExamples,
  AttributeCodeLookup,
} from '@/src/util/classes/attribute-calculation'
...
export function postProcessCurrentAttribute(attribute: Attribute): Attribute {
  ...
  prepareAttribute.dataTypeName = DATA_TYPES.find(
    (type) => type.value === attribute?.dataType,
  ).name
  prepareAttribute.visibilityIndex = ATTRIBUTES_VISIBILITY_STATES.findIndex(
    (type) => type.value === attribute?.visibility,
  )
  prepareAttribute.visibilityName = ATTRIBUTES_VISIBILITY_STATES.find(
    (type) => type.value === attribute.visibility,
  ).name
  if (attribute.sourceCode == null) {
    prepareAttribute.sourceCode =
      AttributeCodeLookup.getAttributeCalculationTemplate(
        AttributeCalculationExamples.AC_EMPTY_TEMPLATE,
        prepareAttribute.dataType,
      ).code
    const regMatch: any = getPythonFunctionRegExMatch(
      prepareAttribute.sourceCode,
    )
    prepareAttribute.sourceCodeToDisplay = prepareAttribute.sourceCode.replace(
      regMatch[2],
      prepareAttribute.name,
    )
    ...
    if (regMatch[2] !== prepareAttribute.name) {
      prepareAttribute.sourceCodeToDisplay =
        prepareAttribute.sourceCode.replace(regMatch[2], prepareAttribute.name)
