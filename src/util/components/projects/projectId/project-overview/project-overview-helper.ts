...
export function getDisplayGraphValueArray(): [
  { value: DisplayGraphs; name: string },
] {
  let toReturn = []
...
export function postProcessingStats(
  ...
  projectStats.forEach((element: any) => {
    if (element.source_type == 'INFORMATION_SOURCE') {
      prepareProjectStats.general[element.source_type] =
        element.absolut_labeled +
        ' hitting on current slice\n' +
        element.records_in_slice +
        ' defined in labeling task'
      prepareProjectStats.generalPercent[element.source_type] =
        element.absolut_labeled + ' (' + element.records_in_slice + ')'
    } else {
      prepareProjectStats.general[element.source_type] =
        element.absolut_labeled + ' of ' + element.records_in_slice
      prepareProjectStats.generalStats[element.source_type] =
        percentRoundString(element.percent, 2)
    }
  })
...
export function postProcessLabelDistribution(
  data: string,
): LabelDistribution[] {
...
export function postProcessConfusionMatrix(data: string): any {
  ...
  return JSON.parse(data).map((e) => {
    return {
      ...
      labelIdManual:
        e.label_name_manual == '@@OUTSIDE@@' ? 'Outside' : e.label_name_manual,
      labelIdProgrammatic:
        e.label_name_ws == '@@OUTSIDE@@' ? 'Outside' : e.label_name_ws,
    }
  })
}
