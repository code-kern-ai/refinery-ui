...
export const TOOLTIPS_DICT = {
  ...
  [CurrentPage.PROJECT_SETTINGS]: {
    ...
    PROJECT_SNAPSHOT:
      'Creates a snapshot compressed file of your current project',
    ...
    EMBEDDINGS: {
      ...
      ENGINE:
        'This will be your custom engine name. You can find this in the Azure OpenAI studio in the deployments section',
      URL: 'This will be your custom URL, which looks like this: https://<your-api-base>.openai.azure.com/',
      VERSION:
        'The latest version of the Azure OpenAI service can also be found here.',
      MODEL: 'Choose your model',
      HAS_FILTER_ATTRIBUTES: 'Has filter attributes',
      NO_FILTER_ATTRIBUTES: 'No filter attributes',
      GENERATE_EMBEDDING:
        'Vectorize your attributes. Integration to Hugging Face available',
    ...
    LABELING_TASK: {
      ...
      INFO_RENAME_LABEL:
        'Before renaming a label, check the label occurrences. Renaming a label will not rename the label occurrences automatically. You can change them, or rename the label anyway.',
    },
  },
  ...
  [CurrentPage.ATTRIBUTE_CALCULATION]: {
    ...
    AVAILABLE_LIBRARIES:
      'See available libraries for this attribute calculation',
  ...
  [CurrentPage.HEURISTICS]: {
    ...
    DISABLED_NEW_HEURISTIC:
      'At least one labeling task is needed to create heuristics',
    SELECT_AT_LEAST_ONE_HEURISTIC: 'Please select at least one heuristic',
    SELECT_AT_LEAST_ONE_VALID_HEURISTIC:
      'At least one invalid heuristic is selected',
    WEAK_SUPERVISION:
      'Calculation of denoised labels can take up to a few minutes!',
    ...
    SWITCH_LABELING_TASK:
      'Switching a task will reset the integrator progress!',
  ...
  [CurrentPage.LABELING]: {
    ...
    REACH_END:
      'If you reached the end, you can select new filters in the browser',
    ...
    AUTO_NEXT_RECORD:
      'Jump to the next record after setting a manual classification label',
    HOVER_BACKGROUND: 'Choose a color for the hover effects of the tables',
    LINE_BREAKS:
      'If checked, the attributes in the data-browser and labeling page will be shown with line breaks',
    PRE_WRAP: 'Preserves whitespace and line breaks',
    PRE_LINE:
      'Collapses multiple whitespaces and line breaks into a single space',
    ...
    SHOW_TASK_NAMES:
      'Display the corresponding task names next to the attribute names in the labeling grid',
    LABEL_DISPLAY:
      'Reduce the display size of classification labels by ordering them into less rows',
  ...
  [CurrentPage.EDIT_RECORDS]: {
    ...
    DIFFERENT_RECORDS:
      'If you want different records, you can select new filters in the browser',
