...
import {
  ...
  buildRegexps,
  rebuildText,
} from '@/src/util/shared/highlight-helper'
...
export default function Highlight(props: any) {
  ...
  function buildEverything() {
    ...
    if (props.regex)
      Array.isArray(props.regex)
        ? finalRegex.push(...props.regex)
        : finalRegex.push(props.regex)
    if (props.searchFor)
      finalRegex.push(...buildRegexps(props.searchFor, props.matchCase))
    if (props.searchForExtended) {
      for (const search of props.searchForExtended) {
        if (search.regex) finalRegex.push(search.regex)
        else if (search.searchFor)
          finalRegex.push(buildRegex(search.searchFor, search.matchCase))
      }
    }
    ...
    <span>
      {parts &&
        parts.map((part, index) => (
          <span
            key={part.id}
            className={`${part.isMatch ? highlightClass : null} ${addClassString}`}
          >
            {part.text}
          </span>
        ))}
