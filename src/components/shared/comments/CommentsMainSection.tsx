...
export default function CommentsMainSection(props: CommentMainSectionProps) {
  ...
  const [positionComment, setPositionComment] =
    useLocalStorage<CommentPosition>(
      'positionComment',
      'comments',
      undefined,
  ...
  const deleteComment = useCallback(
    (commentId: string) => {
      deleteCommentMut({
        variables: { commentId: commentId, projectId: projectId },
      }).then((res) => {
        setCommentTextsArray(
          commentTextsArray.filter(
            (commentText: string) =>
              commentText != commentTextsArray[commentId],
          ),
        )
  ...
  const saveComment = useCallback(
    (type: string, commentId: string, comment: string, isPrivate: boolean) => {
      createCommentMut({
        variables: {
          projectId: projectId,
          comment: comment,
          xftype: convertTypeToKey(type),
          xfkey: commentId,
          isPrivate: isPrivate,
        },
      }).then(() => {
  ...
  return (
    <div className="relative z-30" role="dialog" aria-modal="true">
      ...
      <div className={`absolute inset-0 overflow-hidden`}>
        {props.open && (
          <div
            className={`pointer-events-auto fixed inset-y-0 top-16 flex w-96 max-w-full border-x ${positionComment == CommentPosition.RIGHT ? 'right-0' : 'left-20'}`}
          ...
          >
            <div className="flex h-full flex-col overflow-y-auto bg-white shadow-xl">
              <div className="flex flex-row items-center px-4 pt-4">
                {positionComment == CommentPosition.RIGHT ? (
                  <Tooltip
                    content={TOOLTIPS_DICT.GENERAL.MOVE_COMMENT_LEFT}
                    color="invert"
                  >
                    <div
                      className="cursor-pointer"
                      onClick={flipCommentPosition}
                    >
                      <IconArrowLeft
                        size={24}
                        strokeWidth={2}
                  ...
                  <Tooltip
                    content={TOOLTIPS_DICT.GENERAL.MOVE_COMMENT_RIGHT}
                    color="invert"
                  >
                    <div
                      className="cursor-pointer"
                ...
                <div
                  className="m-auto flex cursor-pointer text-base font-bold leading-6 text-gray-900"
                  onClick={() => openAllComments(!allOpen)}
                >
                  <Tooltip
                    content={
                      allOpen
                        ? TOOLTIPS_DICT.GENERAL.CLOSE_ALL
                        : TOOLTIPS_DICT.GENERAL.OPEN_ALL
                    }
                    contentColor="invert"
                    hideArrow={true}
                    placement="bottom"
              ...
              <DisplayComments
                ...
                openComments={openCommentsArray}
                editComments={editCommentsArray}
                commentTexts={commentTextsArray}
                ...
                handleCommentTextChange={(value: string, index: number) =>
                  handleCommentTextChange(value, index)
                }
              ...
              <CommentCreation
                saveComment={(
                  ...
                  comment: string,
                  isPrivate: boolean,
                ) => saveComment(type, commentId, comment, isPrivate)}
                closeCommentCreation={props.toggleOpen}
                isOpen={props.open}
              />
            </div>
          </div>
