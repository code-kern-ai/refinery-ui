...
export default function Comments() {
  ...
  const handleWebsocketNotificationGlobal = useCallback(
    (msgParts: string[]) => {
      ...
      if (msgParts[1] == 'comment_deleted') {
        ...
        somethingToRerequest =
          somethingToRerequest ||
          CommentDataManager.isCommentUpdateInterestingForMe(msgParts)
        if (somethingToRerequest) {
          //create helper addon
          const backRequest: CommentRequest = {
            commentType: msgParts[4] as CommentType,
            projectId: msgParts[2],
            commentKey: msgParts[5],
            commentId: msgParts[3],
          }
        ...
        //create helper addon
        const backRequest: CommentRequest = {
          commentType: msgParts[3] as CommentType,
          projectId: msgParts[2],
          commentKey: msgParts[4],
          commentId: msgParts[5],
        }
        const key = commentRequestToKey(backRequest)
        CommentDataManager.addCommentRequests[key] = backRequest
      }
  ...
  const handleWebsocketNotification = useCallback(
    (msgParts: string[]) => {
      ...
      if (['label_created', 'label_deleted'].includes(msgParts[1])) {
        somethingToRerequest = CommentDataManager.modifyCacheFor(
          CommentType.LABEL,
          msgParts[0],
          msgParts[2],
          msgParts[1] == 'label_created',
        )
      } else if (msgParts[1] == 'attributes_updated') {
        somethingToRerequest = CommentDataManager.modifyCacheFor(
          CommentType.ATTRIBUTE,
          msgParts[0],
          null,
          true,
        )
        ...
        somethingToRerequest = CommentDataManager.modifyCacheFor(
          CommentType.LABELING_TASK,
          msgParts[0],
          msgParts[2],
          msgParts[1] != 'labeling_task_deleted',
        )
      } else if (
        [
          'data_slice_created',
        ...
        somethingToRerequest = CommentDataManager.modifyCacheFor(
          CommentType.DATA_SLICE,
          msgParts[0],
          msgParts[2],
          msgParts[1] != 'data_slice_deleted',
        )
      } else if (
        [
        ...
        somethingToRerequest = CommentDataManager.modifyCacheFor(
          CommentType.HEURISTIC,
          msgParts[0],
          msgParts[2] == 'all' ? null : msgParts[2],
          msgParts[1] != 'information_source_deleted',
        )
      } else if (
        [
        ...
        somethingToRerequest = CommentDataManager.modifyCacheFor(
          CommentType.KNOWLEDGE_BASE,
          msgParts[0],
          msgParts[2],
          msgParts[1] != 'knowledge_base_deleted',
        )
      }
      if (somethingToRerequest) {
        const requestJsonString = CommentDataManager.buildRequestJSON()
  ...
  return (
    <>
      <button
        className="mr-6 inline-block cursor-pointer"
        onClick={toggleModal}
      >
        <Tooltip
          content={TOOLTIPS_DICT.GENERAL.COMMENTS}
          color="invert"
          placement="bottom"
        >
