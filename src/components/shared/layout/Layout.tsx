...
export default function Layout({ children }) {
  ...
  const [refetchNotificationsByUser] = useLazyQuery(NOTIFICATIONS_BY_USER, {
    fetchPolicy: 'network-only',
  })
  const [refetchAdminMessages] = useLazyQuery(GET_ALL_ACTIVE_ADMIN_MESSAGES, {
    fetchPolicy: 'network-only',
  })
      ...
      if (notificationsState.length > 0) {
        setNotificationsState(
          notificationsState.filter(
            (notification) =>
              new Date().getTime() - notification.savedToStore < 5000,
          ),
        )
      } else {
  ...
  function refetchNotificationsAndProcess() {
    refetchNotificationsByUser().then((res) => {
      setNotificationsState(
        postProcessNotificationsUser(
          res['data']['notificationsByUserId'],
          notificationsState,
        ),
      )
    })
  ...
  function refetchAdminMessagesAndProcess() {
    refetchAdminMessages().then((res) => {
      setActiveAdminMessages(
        postProcessAdminMessages(res['data']['allActiveAdminMessages']),
      )
    })
  }
  ...
  return (
    <>
      <div
        ...
        style={{
          width: windowWidth < MIN_WIDTH ? MIN_WIDTH + 'px' : '100%',
          overflowX: windowWidth < MIN_WIDTH ? 'auto' : 'hidden',
        }}
      >
        <Sidebar />
        <div className="flex h-full w-full flex-1 flex-col">
          <Header />
          <div className="block h-full w-full flex-grow bg-gray-100">
            <main>{children}</main>
          </div>
        </div>
      </div>
      <div
        className="absolute bottom-0 left-24 z-50 mb-7 flex cursor-pointer flex-col content-start"
        id="notifications"
      >
        {notificationsState.map((notification, index) => (
          <Fragment key={index}>
            {notification.level === NotificationLevel.INFO && (
              <div onClick={() => dispatch(setNotificationId(notification.id))}>
      ...
      <AdminMessages
        ...
        setActiveAdminMessages={(activeAdminMessages) =>
          setActiveAdminMessages(activeAdminMessages)
        }
      />
