...
export default function GroupDisplay(props: GroupDisplayProps) {
    ...
    function flipControlValue(control: any, type: ExportEnums, isCheckBox: boolean) {
        ...
        if (isCheckBox || !activeStateCtrl) control['active'] = !activeStateCtrl;
        if (type == ExportEnums.ExportPreset && activeStateCtrl) {
            props.setPresetValues(control['value']);
        }
        if (isEnumRadioGroup(type)) {
            const parent = props.formGroup.get(type);
            for (const key in parent) {
                if (key == control['value']) parent[key]['active'] = true;
                else parent[key]['active'] = false;
            }
        }
        props.updateFormGroup(props.formGroup, props.type);
    }

    return (<>
        {props.formGroup && props.formGroup.get(props.type) && <div className={`contents text-left ${props.hiddenCheckCtrl ? (!props.hiddenCheckCtrl ? 'hidden' : null) : null}`}>
