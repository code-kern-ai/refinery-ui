...
import {
  prepareColumnsData,
  prepareTableData,
} from '@/src/util/shared/record-table-helper'
...
export default function RecordTable(props: RecordTableProps) {
  ...
  const [preparedColumnsData, setPreparedColumnsData] = useState(
    props.columnsData ?? [],
  )
        ...
        <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
          <table className="min-w-full divide-y  divide-gray-300 border">
            <thead className="bg-gray-50">
              <tr>
                {preparedColumnsData.map((column) => (
                  <th
                    key={column.order}
                    scope="col"
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {preparedTableData &&
                Object.values(preparedTableData).map((data, index) => (
                  <tr
                    key={data.id}
                    className={`${index % 2 != 0 ? 'bg-gray-50' : 'bg-white'} ${!data.isWSRelated && configuration.weakSupervisionRelated ? 'hidden' : ''}`}
                  >
                    {preparedColumnsData.map((column) => (
                      <td
                        key={column.order}
                      ...
                      >
                        {column.field !== 'label' ? (
                          ...
                          <span
                            className={`inline-flex items-center rounded-md border px-2.5 py-0.5 text-sm font-medium ${data.color.backgroundColor} ${data.color.textColor} ${data.color.borderColor} ${data.color.hoverColor}`}
                          >
                            {data[column.field]}
                          </span>
                        )}
