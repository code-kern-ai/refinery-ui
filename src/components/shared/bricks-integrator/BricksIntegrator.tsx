...
import {
  buildSearchUrl,
  filterMinVersion,
  getEmptyBricksIntegratorConfig,
  getGroupName,
  getHttpBaseLink,
  getHttpBaseLinkExample,
} from '@/src/util/shared/bricks-integrator-helper'
import {
  PASS_ME,
  caesarCipher,
...
import {
  GROUPS_TO_REMOVE,
  extendDummyElements,
  getDummyNodeByIdForApi,
} from '@/src/util/classes/bricks-integrator/dummy-nodes'
import {
  getBricksIntegrator,
  postBricksIntegrator,
} from '@/src/services/base/data-fetch'
...
export default function BricksIntegrator(_props: BricksIntegratorProps) {
  ...
  function initConfig() {
    ...
    if (localStrapiConfig) {
      const unCiphered = JSON.parse(
        caesarCipher(localStrapiConfig, PASS_ME, true),
      )
    ...
    switch (configCopy.page) {
      ...
      case IntegratorPage.INPUT_EXAMPLE:
        if (configCopy.api.moduleId == -1) {
          configCopy.canAccept =
            !!configCopy.api.data.data.attributes.sourceCode
        } else configCopy.canAccept = !!configCopy.api.data
        break
      case IntegratorPage.INTEGRATION:
        configCopy.canAccept =
          configCopy.codeFullyPrepared &&
          !BricksCodeParser.nameTaken &&
          BricksCodeParser.functionName != ''
        break
    }
    dispatch(setBricksIntegrator(configCopy))
  }
  ...
  function requestSearch() {
    const configCopy = jsonCopy(config)
    configCopy.search.lastRequestUrl = buildSearchUrl(
      configCopy,
      props.moduleTypeFilter,
      props.executionTypeFilter,
    )
    if (configCopy.search.currentRequest)
      configCopy.search.currentRequest.unsubscribe()
    ...
    getBricksIntegrator(
      configCopy.search.lastRequestUrl,
      options?.headers,
      (data) => {
        if (!data) {
        ...
        configCopy.extendedIntegrator = data.data.some(
          (e) => e.attributes.partOfGroup,
        ) //new field in integrator
        configCopy.search.currentRequest = null
        let finalData
        if (configCopy.extendedIntegrator) {
          finalData = data.data.map((integratorData) => {
            ...
            if (integratorData.attributes.partOfGroup) {
              integratorDataCopy.attributes.partOfGroup = JSON.parse(
                integratorDataCopy.attributes.partOfGroup,
              )
              integratorDataCopy.attributes.partOfGroup = removeArrayFromArray(
                integratorDataCopy.attributes.partOfGroup,
                GROUPS_TO_REMOVE,
              )
            }
            if (integratorData.attributes.availableFor)
              integratorDataCopy.attributes.availableFor = JSON.parse(
                integratorDataCopy.attributes.availableFor,
              )
            if (integratorData.attributes.integratorInputs)
              integratorDataCopy.attributes.integratorInputs = JSON.parse(
                integratorDataCopy.attributes.integratorInputs,
              )
        ...
        if (props.executionTypeFilter) {
          ...
          finalData = finalData.filter((e) =>
            toFilter.includes(e.attributes.executionType),
          )
        }
        finalData = filterMinVersion(finalData)
        extendDummyElements(
          finalData,
          configCopy.extendedIntegrator,
          user,
          isAdmin,
        )
  ...
  function filterForExtendedIntegrator(
    data: any[],
    configCopy?: BricksIntegratorConfig,
  ): any[] {
    addFilterPartOfGroup('all', configCopy)
    for (let e of data) {
      if (!e.attributes.partOfGroup) continue
      if (
        e.attributes.executionType &&
        e.attributes.executionType != 'activeLearner'
      )
        e.attributes.partOfGroup.push(e.attributes.executionType)
      e.attributes.partOfGroup.forEach((group) =>
        addFilterPartOfGroup(group, configCopy),
      )
    }
    if (Object.keys(config.groupFilterOptions.filterValues).length < 2)
      return data

    return data.filter((e) =>
      e.attributes.availableFor
        ? e.attributes.availableFor.includes('refinery')
        : true,
    )
  }

  function addFilterPartOfGroup(
    key: string,
    configCopy?: BricksIntegratorConfig,
    ...
    configCopy.groupFilterOptions.filterValues[key] = {
      key: key,
      name: name,
      active: key == 'all',
      countInGroup: -1,
  ...
  function filterGroup(configCopy: BricksIntegratorConfig) {
    configCopy.search.results.forEach(
      (e) => (e.groupVisible = filterForGroup(e, configCopy)),
    )
  ...
  function filterForGroup(e: BricksSearchData, configCopy: any): boolean {
    ...
    return activeGroups.every((group) =>
      e.attributes.partOfGroup.includes(group),
    )
  }

  function requestSearchDebounce(
    value: string,
    configCopy?: BricksIntegratorConfig,
  ) {
    ...
    configCopy.search.results.forEach(
      (e) =>
        (e.searchVisible =
          e.id.toString().startsWith(searchFor) ||
          e.attributes.name.toLowerCase().includes(searchFor) ||
          e.attributes.description.toLowerCase().includes(searchFor)),
    )
    configCopy.search.nothingMatches = !configCopy.search.results.find(
      (e) => e.searchVisible && e.groupVisible,
    )
  ...
  function countGroupEntries(configCopy?: BricksIntegratorConfig) {
    ...
    for (const key in data)
      if (data[key].countInGroup > 0 || data[key].active)
        configCopy.groupFilterOptions.filterValuesArray.push(data[key])
    configCopy.groupFilterOptions.filterValuesArray.sort(
      (a, b) => b.countInGroup - a.countInGroup || a.name.localeCompare(b.name),
    )
    dispatch(setBricksIntegrator(configCopy))
  }
      ...
      switch (configCopy.page) {
        ...
        case IntegratorPage.INPUT_EXAMPLE:
          ...
          if (configCopy.api.moduleId < 0)
            configCopy = BricksCodeParser.prepareCode(
              configCopy,
              props.executionTypeFilter,
              props.nameLookups,
  ...
  function requestDataFromApi(configCopy?: BricksIntegratorConfig) {
    ...
    configCopy.api.requestUrl =
      getHttpBaseLink(configCopy) + configCopy.api.moduleId
    ...
    getBricksIntegrator(
      ...
      (c) => {
        if (!c.data.attributes.integratorInputs) configCopy.api.data = c
          ...
          configCopy.api.data.data.attributes.partOfGroup = JSON.parse(
            c.data.attributes.partOfGroup,
          )
          configCopy.api.data.data.attributes.partOfGroup =
            removeArrayFromArray(
              configCopy.api.data.data.attributes.partOfGroup,
              GROUPS_TO_REMOVE,
            )
          configCopy.api.data.data.attributes.partOfGroupText =
            configCopy.api.data.data.attributes.partOfGroup
              .map((x) => getGroupName(x))
              .join(', ')
          configCopy.api.data.data.attributes.availableFor = JSON.parse(
            c.data.attributes.availableFor,
          )
          configCopy.api.data.data.attributes.integratorInputs = JSON.parse(
            c.data.attributes.integratorInputs,
          )
        }
        configCopy.api.data.data.attributes.bricksLink =
          'https://bricks.kern.ai/' +
          c.data.attributes.moduleType +
          's/' +
          c.data.id
        configCopy.api.data.data.attributes.issueLink =
          'https://github.com/code-kern-ai/bricks/issues/' +
          c.data.attributes.issueId
        configCopy.api.requesting = false
        configCopy.example.requestData =
          configCopy.api.data.data.attributes.inputExample
        configCopy = BricksCodeParser.prepareCode(
          configCopy,
          props.executionTypeFilter,
          props.nameLookups,
  ...
  function requestExample() {
    ...
    if (
      !configCopy.example.requestData ||
      configCopy.example.requestData.trim().length == 0
    ) {
    ...
    configCopy.example.requestUrl +=
      configCopy.api.data.data.attributes.moduleType +
      's/' +
      configCopy.api.data.data.attributes.endpoint
  ...
  function selectDifferentTask(taskId: string) {
    ...
    const currentTaskType = BricksCodeParser.getLabelingTaskAttribute(
      props.labelingTaskId,
      'taskType',
      labelingTasks,
    )
    const newTaskType = BricksCodeParser.getLabelingTaskAttribute(
      taskId,
      'taskType',
      labelingTasks,
    )
    if (currentTaskType != newTaskType) {
      ...
      const configCopy = BricksCodeParser.prepareCode(
        jsonCopy(config),
        props.executionTypeFilter,
        props.nameLookups,
        taskId,
    ...
    dispatch(
      setLabelsBricksIntegrator(
        BricksCodeParser.getLabels(taskId, labelingTasks),
      ),
    )
  ...
  return (
    <div className="flex items-center">
      <Tooltip
        content={TOOLTIPS_DICT.GENERAL.OPEN_BRICKS_INTEGRATOR}
        placement="left"
        color="invert"
      >
        <button
          onClick={openBricksIntegrator}
          id="bricks-integrator-open-button"
