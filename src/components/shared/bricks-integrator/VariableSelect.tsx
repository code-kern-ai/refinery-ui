import {
  selectBricksIntegratorAttributes,
  selectBricksIntegratorEmbeddings,
  selectBricksIntegratorLabels,
  selectBricksIntegratorLanguages,
  selectBricksIntegratorLookupLists,
  setAttributesBricksIntegrator,
  setEmbeddingsBricksIntegrator,
  setLabelingTasksBricksIntegrator,
  setLabelsBricksIntegrator,
  setLanguagesBricksIntegrator,
  setLookupListsBricksIntegrator,
...
import {
  BricksVariableComment,
  isCommentTrue,
} from '@/src/util/classes/bricks-integrator/comment-lookup'
...
export default function VariableSelect(props: VariableSelectProps) {
  ...
  useEffect(() => {
    ...
    setAllowedValues(
      props.variable.type,
      props.variable.comment,
      props.labelingTaskId,
    )
  }, [props.variable, labelingTasks])

  function setAllowedValues(
    forType: BricksVariableType,
    comment: string,
    labelingTaskId: string,
  ) {
    switch (forType) {
      case BricksVariableType.LANGUAGE:
        const allLanguages = isCommentTrue(
          comment,
          BricksVariableComment.LANGUAGE_ALL,
        )
        refetchLanguagesAndProcess(!allLanguages)
        break
      case BricksVariableType.ATTRIBUTE:
        if (
          isCommentTrue(comment, BricksVariableComment.ATTRIBUTE_ONLY_TEXT_LIKE)
        ) {
          refetchAttributesAndProcess(['TEXT', 'CATEGORY'])
          return
        } else if (
          isCommentTrue(comment, BricksVariableComment.ATTRIBUTE_ONLY_TEXT)
        ) {
      ...
      case BricksVariableType.LABELING_TASK:
        ...
        if (
          isCommentTrue(
            comment,
            BricksVariableComment.LABELING_TASK_ONLY_CLASSIFICATION,
          )
        )
          typeFilter = 'MULTICLASS_CLASSIFICATION'
        else if (
          isCommentTrue(
            comment,
            BricksVariableComment.LABELING_TASK_ONLY_EXTRACTION,
          )
        )
          typeFilter = 'INFORMATION_EXTRACTION'
        refetchLabelingTasksAndProcess(typeFilter)
        break
      case BricksVariableType.LABEL:
        if (!labelingTaskId) {
          console.log(
            "no labeling task id given -> can't collect allowed labels",
          )
          return
        }
        return dispatch(
          setLabelsBricksIntegrator(
            BricksCodeParser.getLabels(labelingTaskId, labelingTasks),
          ),
        )
  ...
  function refetchAttributesAndProcess(
    typeFilter: string[] = ['TEXT'],
    stateFilter: string[] = ['UPLOADED', 'USABLE', 'AUTOMATICALLY_CREATED'],
  ) {
    refetchAttributes({
      variables: { projectId: projectId, stateFilter: ['ALL'] },
    }).then((res) => {
      const attributes = postProcessingAttributes(
        res.data['attributesByProjectId'],
      )
      ...
      if (filtered.length == 0)
        return dispatch(
          setAttributesBricksIntegrator(['No useable attributes']),
        )
  ...
  function refetchEmbeddingsAndProcess(labelingTaskId: string | null = null) {
    refetchEmbeddings({ variables: { projectId: projectId } }).then((res) => {
      const embeddings = res.data['projectByProjectId']['embeddings'][
        'edges'
      ].map((e) => e['node'])
      ...
      if (!labelingTaskId)
        return dispatch(setEmbeddingsBricksIntegrator(embeddings.map((x) => x))) //copy of array not of values
      const task = labelingTasks.find((lt) => lt.id == labelingTaskId)
      if (!task)
        return dispatch(
          setEmbeddingsBricksIntegrator(['No useable embeddings']),
        )
      const onlyAttribute =
        task.taskType == LabelingTaskTaskType.MULTICLASS_CLASSIFICATION
      let filtered = embeddings.filter(
        (e) =>
          (e.type == LabelingTaskTarget.ON_ATTRIBUTE && onlyAttribute) ||
          (e.type != LabelingTaskTarget.ON_ATTRIBUTE && !onlyAttribute),
      )
      if (filtered && filtered.length > 0)
        return dispatch(setEmbeddingsBricksIntegrator(filtered))
      else
        return dispatch(
          setEmbeddingsBricksIntegrator(['No useable embeddings']),
        )
  ...
  function refetchLookupListsAndProcess() {
    refetchLookupLists({ variables: { projectId: projectId } }).then((res) => {
      ...
      if (lookupLists.length > 0)
        return dispatch(setLookupListsBricksIntegrator(lookupLists))
      else
        return dispatch(
          setLookupListsBricksIntegrator(['No useable lookup lists']),
        )
    })
  }
  ...
  function refetchLabelingTasksAndProcess(typeFilter: string) {
    refetchLabelingTasksByProjectId({
      ...
      if (filtered.length == 0)
        return dispatch(
          setLabelingTasksBricksIntegrator(['No useable labeling tasks']),
        )
  ...
  const handleWebsocketNotification = useCallback(
    (msgParts: string[]) => {
      if (
        msgParts[1] == 'attributes_updated' ||
        (msgParts[1] == 'calculate_attribute' && msgParts[2] == 'created')
      ) {
        refetchAttributesAndProcess()
      } else if (
        [
        ...
        msgParts[1] == 'embedding' &&
        msgParts[3] == 'state' &&
        msgParts[4] == 'FINISHED'
      ) {
        refetchEmbeddingsAndProcess()
      } else if (
        ['knowledge_base_deleted', 'knowledge_base_created'].includes(
          msgParts[1],
        )
      ) {
        refetchLookupListsAndProcess()
  ...
  useWebsocket(
    ...
    projectId,
    CurrentPageSubKey.VARIABLE_SELECTION,
  )

  return (
    <>
      {props.variable &&
        props.variable.values.map((v, index) => (
          <div
            key={index}
            className="col-start-2 my-2 flex flex-row flex-nowrap items-center gap-x-2"
          >
            {props.variable.type == BricksVariableType.ATTRIBUTE && (
              <Dropdown2
                options={attributes}
                buttonName={
                  props.variable.values[index]
                    ? props.variable.values[index]
                    : 'Select attribute'
                }
            ...
            {props.variable.type == BricksVariableType.GENERIC_CHOICE && (
              <Dropdown2
                options={props.variable.allowedValues}
                buttonName={
                  props.variable.values[index]
                    ? props.variable.values[index]
                    : 'Select option'
            ...
            {props.variable.type == BricksVariableType.REGEX && (
              <input
                type="text"
                value={props.variable.values[index]}
                id={'REGEX_' + props.index + '_' + index}
            ...
            {props.variable.type == BricksVariableType.GENERIC_STRING && (
              <input
                type="text"
                value={props.variable.values[index] ?? ''}
                id={'GENERIC_STRING_' + props.index + '_' + index}
                onChange={(e) => changeInputValue(index, e.target.value)}
                style={{ minWidth: '10rem', maxWidth: '25rem' }}
                className={`placeholder-italic h-9 rounded-md border border-gray-300 pl-4 text-sm leading-8 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100 ${props.variable.values[index]?.length > 20 ? 'w-60' : 'w-50'}`}
              />
            )}
            ...
            {props.variable.type == BricksVariableType.GENERIC_INT && (
              <input
                ...
                step="1"
                value={props.variable.values[index] ?? ''}
                id={'GENERIC_INT_' + props.index + '_' + index}
                onChange={(e) => changeInputValue(index, e.target.value)}
                className={`placeholder-italic h-9 w-20 rounded-md border border-gray-300 pl-4 text-sm leading-8 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100`}
              />
            )}
            ...
            {props.variable.type == BricksVariableType.GENERIC_FLOAT && (
              <input
                ...
                step="0.01"
                value={props.variable.values[index] ?? ''}
                id={'GENERIC_FLOAT_' + props.index + '_' + index}
                onChange={(e) => changeInputValue(index, e.target.value)}
                className={`placeholder-italic h-9 w-20 rounded-md border border-gray-300 pl-4 text-sm leading-8 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100`}
              />
            )}

            {props.variable.type == BricksVariableType.GENERIC_BOOLEAN && (
              <div
                className="mr-2 h-4 w-4 cursor-pointer rounded border border-gray-300 hover:bg-gray-200"
                id={'GENERIC_BOOLEAN_' + props.index + '_' + index}
                style={{
                  backgroundColor: props.variable.options.colors[index],
                  borderColor: props.variable.options.colors[index],
                }}
            ...
            {props.variable.type == BricksVariableType.GENERIC_RANGE && (
              <div className="flex flex-row flex-nowrap items-center gap-x-2">
                <input
                  ...
                  step="0.1"
                  value={props.variable.values[index]}
                  id={'GENERIC_RANGE_' + props.index + '_' + index}
                  min={props.variable.allowedValues[0]}
                  max={props.variable.allowedValues[1]}
                  onChange={(e) => changeInputValue(index, e.target.value)}
                  style={{ minWidth: '10rem', maxWidth: '25rem' }}
                  className={`placeholder-italic h-9 rounded-md border border-gray-300 pl-4 text-sm leading-8 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100`}
                />
                <output>{props.variable.values[index]}</output>
              </div>
            )}

            {props.variable.values.length > 1 && (
              <div
                onClick={() => {
                  const propsCopy = { ...props }
                ...
                className="btn hover:border-transparent hover:bg-transparent border-transparent bg-transparent btn-xs cursor-pointer px-0"
              >
                <IconTrash className="h-4 w-4 text-gray-900" />
              </div>
            )}

            {props.variable.canMultipleValues &&
              index == props.variable.values.length - 1 && (
                <label
                  onClick={() => {
                    const propsCopy = { ...props }
                  ...
                  className="inline-flex cursor-pointer items-center rounded border border-gray-200 bg-white px-0.5 py-0.5 text-xs font-medium text-gray-700 shadow-sm focus:outline-none"
                >
