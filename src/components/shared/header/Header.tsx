...
export default function Header() {
  ...
  const [refetchNotifications] = useLazyQuery(NOTIFICATIONS, {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      <div className="relative z-10 flex h-16 flex-shrink-0 items-center justify-between border-b border-gray-200 bg-white shadow-sm">
        {isDemo ? (
          <div className="pointer-events-none absolute bottom-0 left-0 right-0 top-0 flex items-center justify-center">
            <span className="inline-flex items-center rounded bg-red-100 px-2 py-0.5 font-medium text-red-800">
              <IconPlayCard className="mr-2 h-6 w-6" />
        ...
        <div className="flex items-center">
          ...
          currentPage == CurrentPage.USERS ? (
            <div className="ml-4 text-sm font-normal text-gray-500">
              ...
              {organizationInactive && (
                <>
                  ...
                  !{' '}
                  {!isManaged
                    ? 'Switch to your main account to continue labeling...'
          ...
          {currentPage == CurrentPage.NEW_PROJECT && (
            <div className="ml-4 text-sm font-normal text-gray-500">
          ...
          {!(
            ...
            currentPage == CurrentPage.USERS ||
            currentPage == CurrentPage.NEW_PROJECT ||
            currentPage == CurrentPage.MODELS_DOWNLOAD ||
            currentPage == CurrentPage.CONFIG
          ) && (
            <div className="ml-4 text-sm font-normal text-gray-700">
              {project?.name} -{' '}
              <span className="font-normal text-gray-500">
                {project?.numDataScaleUploaded} records
              </span>
            </div>
          )}
          {currentPage == CurrentPage.MODELS_DOWNLOAD && (
            ...
            <div className="ml-4 text-sm font-normal text-gray-500">
              {!organizationInactive && user && (
                <>
                  ...
                  <span className="font-bold text-gray-900">here</span>.
                </>
              )}
              {organizationInactive && (
                <>
        ...
        <div className="flex flex-row flex-nowrap items-center">
          <div className="flex items-center justify-center">
            <a href="/refinery/projects" className="mr-6 flex">
              <Tooltip
                placement="bottom"
                trigger="hover"
                color="invert"
                content={TOOLTIPS_DICT.GENERAL.PROJECTS}
              >
            ...
            <div className="flex items-center justify-center">
              ...
              >
                <Tooltip
                  ...
                  trigger="hover"
                  color="invert"
                  content={TOOLTIPS_DICT.GENERAL.USERS}
                >
                  <IconHexagons className="h-6 w-6" />
                </Tooltip>
              </a>
            </div>
          ) : (
          ...
          {user?.role == UserRole.ENGINEER && (
            <div className="flex items-center justify-center">
              <button
                ...
                onClick={openModalAndRefetchNotifications}
              >
                <Tooltip
                  content={TOOLTIPS_DICT.GENERAL.NOTIFICATION_CENTER}
                  placement="bottom"
                  color="invert"
                >
          ...
          {user?.role == UserRole.ENGINEER ? (
            <div className="flex items-center justify-center">
              <div className={`${style.widget} ${style.widgetLg}`}>
                <a
                  className={style.btnx}
                  href="https://github.com/code-kern-ai/refinery"
                  rel="noopener"
                  target="_blank"
                  aria-label="Star code-kern-ai/refinery on GitHub"
                >
                  <svg
                    viewBox="0 0 16 16"
                    width="16"
