...
export default function SyncRecordsModal(props: SyncRecordsModalProps) {
  ...
  const syncChanges = useCallback(() => {
    ...
    syncChangesMut({
      variables: { projectId: projectId, changes: JSON.stringify(changes) },
    })
      .then((res) => {
        ...
        if (tmp?.ok) {
          ...
          dispatch(
            setModalStates(ModalEnum.SYNC_RECORDS, {
              syncModalAmount: Object.keys(erdDataCopy.cachedRecordChanges)
                .length,
            }),
          )
        } else {
  ...
  useEffect(() => {
    setAcceptButton({
      ...ACCEPT_BUTTON,
      disabled: syncRecordsModal.syncModalAmount == 0 || props.erdData.syncing,
      emitFunction: syncChanges,
    })
  }, [syncRecordsModal])
  ...
  function removeFromCache(key: string) {
    ...
    const r1 = erdDataCopy.displayRecords.find(
    ...
    if (subKey != undefined)
      r1.data[attributeName][subKey] = r2.data[attributeName][subKey]
    else r1.data[attributeName] = r2.data[attributeName]

    delete erdDataCopy.cachedRecordChanges[key]
    dispatch(
      setModalStates(ModalEnum.SYNC_RECORDS, {
        syncModalAmount: Object.keys(erdDataCopy.cachedRecordChanges).length,
      }),
    )
    props.setErdData(erdDataCopy)
  }
  ...
  return (
    <Modal modalName={ModalEnum.SYNC_RECORDS} acceptButton={acceptButton}>
      {props.erdData && (
        <>
          <h1 className="mb-2 inline-flex items-center gap-x-1 text-center text-lg text-gray-900">
            Info
            {props.erdData.syncing ? (
              <LoadingIcon color="blue" />
            ) : (
              <IconInfoCircle className="text-blue-400" />
            )}
          </h1>
          <div className="mb-2 flex flex-col gap-y-2 text-sm font-medium text-gray-700">
            <div>
              {' '}
              {syncRecordsModal.syncModalAmount} changes to be synchronized:{' '}
            </div>
            <div
              className="grid gap-px rounded-lg bg-gray-200 p-px"
              style={{
                gridTemplateColumns:
                  'minmax(auto,100px) max-content minmax(auto,300px) minmax(auto,300px) 50px',
              }}
            >
              <span className="inline-flex h-full w-full items-center justify-center bg-white font-bold">
                Record ({props.erdData.data?.attributes[0].name})
              </span>
              <span className="inline-flex h-full w-full items-center justify-center bg-white font-bold">
                Attribute
              </span>
              ...
              <span className="h-full w-full bg-white"></span>

              {Object.entries(props.erdData.cachedRecordChanges).map(
                ([key, value]: any) => (
                  <Fragment key={key}>
                    <span className="inline-flex h-full w-full items-center justify-center bg-white p-1">
                      {value.display.record}
                    </span>
                    <span className="inline-flex h-full w-full items-center justify-center bg-white p-1">
                      {value.attributeName}
                      {value.display.subKeyAdd}
                    </span>
                    <span className="inline-flex h-full w-full items-center justify-center bg-red-200 p-1">
                      {value.display.oldValue}
                    </span>
                    <span className="inline-flex h-full w-full items-center justify-center bg-green-200 p-1">
                      {value.newValue}
                    </span>
                    <div
                      className="inline-flex h-full w-full cursor-pointer items-center justify-center bg-white p-1"
                      onClick={() => removeFromCache(key)}
                    >
                      <IconTrash className="h-6 w-6 text-red-700" stroke={2} />
                    </div>
                  </Fragment>
                ),
              )}
          ...
          {props.erdData.errors && (
            <div className="relative flex flex-col rounded border border-red-400 bg-red-100 px-4 py-3 text-red-700">
              ...
              {props.erdData.errors.map((error, index) => (
                <div
                  key={error}
                  className="m-auto overflow-x-auto text-sm"
