...
import {
  buildAccessKey,
  createDefaultEditRecordComponentData,
} from '@/src/util/components/projects/projectId/edit-records-helper'
...
export default function EditRecords() {
  ...
  useEffect(() => {
    ...
    erdDataCopy.navBar.positionString =
      erdDataCopy.data.records.length + ' records in'
  ...
  useEffect(() => {
    function handleBeforeUnload(event) {
      const hasUnsavedChanges =
        Object.keys(erdData.cachedRecordChanges).length > 0
  ...
  const alertUser = useCallback(
    (msg: string) => {
      ...
      alert(
        'Settings were changed (msgId: ' + msg + ')\nPage will be reloaded.',
      )
    ...
    <>
      {projectId && (
        <div className="flex h-screen flex-col bg-white">
          <NavBarTopEditRecords
            erdData={erdData}
            setErdData={(erdData) => setErdData(erdData)}
          />
          <div
            className={`grid flex-grow auto-rows-max items-start gap-2 overflow-y-auto px-2 pb-8 pt-2 ${erdData.columnClass}`}
            ...
            {erdData.data && (
              <>
                {erdData.displayRecords.map((record) => (
                  <div
                    key={record.id}
                    className={`relative w-full scroll-mt-8 items-center space-x-3 overflow-hidden rounded-lg border bg-white shadow ${erdData.displayRecords?.length > 2 ? style.item : ''} `}
                    onDoubleClick={() => {
                      const erdDataCopy = { ...erdData }
                      erdDataCopy.editRecordId =
                        erdDataCopy.editRecordId == record.id ? null : record.id
                      setErdData(erdDataCopy)
                    }}
                    id={
                      record.id == erdData.data.selectedRecordId
                        ? 'flash-it'
                        : null
                    }
                  >
                    <div className="px-4 py-5 sm:p-6">
                      {erdData.data.attributes.map((attribute) => (
                        <Fragment key={attribute.id}>
                          <div>
                            ...
                            <div className="overflow-anywhere mb-4 flex text-sm text-gray-800">
                              {attribute.dataType ==
                              DataTypeEnum.EMBEDDING_LIST ? (
                                <div className="flex w-full flex-col gap-y-1 divide-y">
                                  {record.data[attribute.name].map(
                                    (item, subKey) => (
                                ...
                                <>
                                  ...
                                  !attribute.isPrimaryKey ? (
                                    ...
                                    <>
                                      {record.data[attribute.name] != null &&
                                      record.data[attribute.name] !== '' ? (
                                        <span className="relative whitespace-pre-wrap">
                                          ...
                                          {erdData.cachedRecordChanges[
                                            ...
                                            <div className="absolute -left-5 top-0 text-yellow-500">
                                              <Tooltip
                                                content={
                                                  TOOLTIPS_DICT.EDIT_RECORDS
                                                    .CACHED_VALUES
                                                }
                                                color="invert"
                                                placement="right"
                      ...
                      <div className="absolute right-2 top-2 flex flex-row flex-nowrap items-center gap-x-2">
                        {record.rla_data && (
                          <Tooltip
                            content={
                              TOOLTIPS_DICT.EDIT_RECORDS.LABEL_ASSOCIATIONS
                            }
                            color="invert"
                            placement="left"
                            className="cursor-auto"
                      ...
                      <div
                        className="absolute left-1 top-1 cursor-pointer p-1"
                        onClick={() => {
                          const erdDataCopy = { ...erdData }
                      ...
                      >
                        {record.id == erdData.editRecordId ? (
                          <Tooltip
                            content={TOOLTIPS_DICT.EDIT_RECORDS.STOP_EDIT}
                            color="invert"
                            placement="right"
                          >
                          ...
                          <Tooltip
                            content={TOOLTIPS_DICT.EDIT_RECORDS.EDIT_RECORD}
                            color="invert"
                            placement="right"
                          >
                            <IconBallpen size={18} />
