...
export function HeuristicsOverview() {
  ...
  const [refetchHeuristics] = useLazyQuery(GET_HEURISTICS_OVERVIEW_DATA, {
    fetchPolicy: 'network-only',
  })
  ...
  function refetchHeuristicsAndProcess() {
    refetchHeuristics({ variables: { projectId: projectId } }).then((res) => {
      const heuristics = postProcessHeuristics(
        res['data']['informationSourcesOverviewData'],
        projectId,
      )
  ...
  function refetchEmbeddingsAndProcess() {
    refetchEmbeddings({ variables: { projectId: projectId } }).then((res) => {
      const embeddingsFinal = postProcessingEmbeddings(
    ...
    (msgParts: string[]) => {
      if (
        ...
        [
          ...
          'payload_failed',
          'payload_created',
          'payload_update_statistics',
          'weak_supervision_started',
    ...
    projectId && (
      <div className="flex h-full flex-1 flex-col bg-gray-100 p-4">
        <div className="-mr-4 h-full w-full">
          <HeuristicsHeader
            refetch={refetchHeuristicsAndProcess}
            filterList={(labelingTask: LabelingTask) =>
              setFilteredList(
                labelingTask != null
                  ? heuristics.filter(
                      (heuristic) =>
                        heuristic.labelingTaskId === labelingTask.id,
                    )
                  : heuristics,
              )
            }
          ...
          {heuristics && heuristics.length == 0 ? (
            <div>
              <div className="mt-8 font-normal text-gray-500">
                <p className="text-xl leading-7">
                  Seems like your project has no heuristic yet.
                </p>
                <p className="mt-3 text-base leading-6">
                  You can create one from the button New heuristic in the bar
                  above. Also, we got some quick-links from our{' '}
                  <a href="https://docs.kern.ai/" target="_blank">
              ...
              <div className="mt-8 grid grid-cols-1 gap-8 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-3 text-base font-normal leading-6 text-gray-500">
                    Think of the as noisy, imperfect label signals. They can be
                    labeling functions, active learning models or something else
                    that you can integrate. The general interface is that they
                    have some record as input and produce some label-related
                    output (e.g. the label name for classification tasks).
                  </div>
                  <div className="mt-3 cursor-pointer text-base font-semibold leading-6 text-green-800 hover:text-green-500">
                    <a
                      href="https://docs.kern.ai/refinery/heuristics#labeling-functions"
                      target="_blank"
                    >
                      Read about labeling functions
                    </a>
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-3 text-base font-normal leading-6 text-gray-500">
                    You don&apos;t need to re-invent the wheel in some cases, as
                    labeling functions often share some of their structure. We
                    have a public GitHub repository containing some template
                    functions, so you can look into this.
                  </div>
                  <div className="mt-3 cursor-pointer text-base font-semibold leading-6 text-green-800 hover:text-green-500">
                    <a
                      href="https://github.com/code-kern-ai/template-functions"
                      target="_blank"
                    >
                ...
                <div>
                  ...
                  <div className="mt-3 text-base font-normal leading-6 text-gray-500">
                    As you label data manually (and build embeddings), you can
                    make great use of active learning to build implicit
                    heuristics. They are straightforward to be implemented, as
                    we autogenerate the coding for it. Still, you can dive
                    deeper into their mechanics in our docs.
                  </div>
                  <div className="mt-3 cursor-pointer text-base font-semibold leading-6 text-green-800 hover:text-green-500">
                    <a
                      href="https://docs.kern.ai/refinery/heuristics#active-learners"
                      target="_blank"
                    >
                      Read about active learning
                    </a>
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-3 text-base font-normal leading-6 text-gray-500">
                    Zero-shot models are really great heuristics, as they infer
                    predictions from the label names (and some context). You
                    might want to use them for your project, but be aware that
                    pulling them can require some time, since they contain
                    enormous amounts of parameters.
                  </div>
                  <div className="mt-3 cursor-pointer text-base font-semibold leading-6 text-green-800 hover:text-green-500">
                    <a
                      href="https://docs.kern.ai/refinery/heuristics#zero-shot-classifiers"
                      target="_blank"
                    >
                      Read about zero-shot
                    </a>
                  </div>
                </div>
              </div>
            ...
            <>
              ...
              >
                ...
                >
                  {heuristics.length > 0 && filteredList.length == 0 && (
                    <span className="mt-4 text-base font-normal leading-6 text-gray-500">
                      No heuristics for this labeling task
                    </span>
                  )}
                  <GridCards
