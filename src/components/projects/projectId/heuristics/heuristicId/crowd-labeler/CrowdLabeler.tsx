...
export default function CrowdLabeler() {
  ...
  const [refetchDataSlicesMut] = useLazyQuery(DATA_SLICES, {
    fetchPolicy: 'network-only',
  })
  const [refetchComments] = useLazyQuery(REQUEST_COMMENTS, {
    fetchPolicy: 'no-cache',
  })
  const [refetchAccessLink] = useLazyQuery(GET_ACCESS_LINK, {
    fetchPolicy: 'network-only',
  })
  ...
  useEffect(() => {
    ...
    refetchDataSlicesMut({
      variables: { projectId: projectId, sliceType: 'STATIC_DEFAULT' },
    }).then((res) => {
  ...
  function refetchCurrentHeuristicAndProcess() {
    refetchCurrentHeuristic({
      ...
      const currentHeuristic: any = postProcessCrowdLabeler(
  ...
  function refetchAccessLinkAndProcess(currentHeuristic: any) {
    ...
    refetchAccessLink({
      ...
      const currentHeuristicUpdated = postProcessCrowdLabeler(
        currentHeuristic,
        labelingTasks,
        link,
      )
    ...
    <HeuristicsLayout>
      {currentHeuristic && (
        <div>
            ...
            <>
              <div className="overflow-hidden bg-white">
                <div className="relative py-8 pr-4">
                  ...
                  <div className="mt-8 max-w-7xl pl-4 lg:grid lg:grid-cols-2 lg:gap-8">
                    ...
                    <div className="mt-8 lg:mt-0">
                      <div className="max-w-prose text-base lg:max-w-none">
                        <p className="text-lg text-gray-500">
                          You can create slice-specific annotation tasks for
                          your annotators. If you have no annotators yet, you
                          can reach out to the support, so we can add some to
                          your organization.
                        </p>
                      </div>
                      <div className="prose prose-indigo mt-5 text-gray-500 lg:col-start-1 lg:row-start-1 lg:max-w-none">
                        <p>
                          Annotators will receive an annotator/slice-specific
                          URL, which shows them records of the respective data
                          slice. They can label through this slice, but their
                          labels won&apos;t be set as your manual reference
                          labels. Instead, their output will be considered a
                          heuristic just like a labeling function.
                        </p>
                        <p>To get started, please ensure the following:</p>
                        <ul role="list">
                          <li>
                            Create static data slices that you want to have
                            labeled manually. These can be e.g. records with low
                            weak supervision confidence.
                          </li>
                          <li>
                            Contact the support on how many annotators you
                            require. We will create those slots for you.
                          </li>
                          <li>
                            Revisit this page afterwards. You will be able to
                            configure the heuristic.
                          </li>
                        </ul>
                        <p>
                          You can then either send the link to an annotator you
                          want to work with, or let us manage that.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <DangerZone
            elementType={DangerZoneEnum.CROWD_LABELER}
