...
export default function LabelingInfoTableModal(
  props: LabelingInfoTableModalProps,
) {
  ...
  return (
    <Modal modalName={ModalEnum.LABELING_INFO_TABLE}>
      <h1 className="text-center text-lg font-bold text-gray-900">Info</h1>
      {props.dataToDisplay[0] && (
        <div className="flex flex-col items-center">
          <div className="flex flex-row items-center gap-x-2">
            <div className="my-2 mb-2 text-center text-gray-500">
              Every label stores a lot of data.
              <br />
              <br />
              This can be the label type, the corresponding task, creation user
              or heuristic as well as the label itself. Now to get a better
              overview over every label placed on a record we group the labels
              by these different criteria.
              <br />
              <br />
              This means there is a group for e.g. type{' '}
              <span
                className={`underline ${shouldHighLight(tmpHighlightIds, props.dataToDisplay[0].shouldHighlightOn) || (hoverGroupsDict[props.dataToDisplay[0].label.id] && hoverGroupsDict[props.dataToDisplay[0].label.id][LabelingPageParts.TABLE_MODAL]) ? settings.main.hoverGroupBackgroundColorClass : ''}`}
                onMouseEnter={() =>
                  onMouseEnter([props.dataToDisplay[0].sourceTypeKey])
                }
                onMouseLeave={() => onMouseLeave()}
              >
                {props.dataToDisplay[0].sourceType}
              </span>
              . If you hover the element everything with the same type will be
              highlighted. <br />
              If you hover over a task (e.g.{' '}
              <span
                className={`underline ${shouldHighLight(tmpHighlightIds, props.dataToDisplay[0].shouldHighlightOn) || (hoverGroupsDict[props.dataToDisplay[0].label.id] && hoverGroupsDict[props.dataToDisplay[0].label.id][LabelingPageParts.TABLE_MODAL]) ? settings.main.hoverGroupBackgroundColorClass : ''}`}
                onMouseEnter={() =>
                  onMouseEnter([props.dataToDisplay[0].taskId])
                }
                onMouseLeave={() => onMouseLeave()}
              >
                {props.dataToDisplay[0].taskName}
              </span>
              ) everything with the same task will be highlighted. The same goes
              for the other groups.
              <br />
              Note that the magnifying glass highlights only the specific label
              entry.
              <br />
              <br />
              Feel free to try it out in the table below and focus on the
              background.
            </div>
          </div>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              <tr className="bg-white">
                ...
                <td
                  ...
                  onMouseLeave={() => onMouseLeave()}
                  className={`whitespace-nowrap px-3 py-2 text-sm text-gray-500 ${shouldHighLight(tmpHighlightIds, props.dataToDisplay[0].shouldHighlightOn) || (hoverGroupsDict[props.dataToDisplay[0].label.id] && hoverGroupsDict[props.dataToDisplay[0].label.id][LabelingPageParts.TABLE_MODAL]) ? settings.main.hoverGroupBackgroundColorClass : ''}`}
                >
                  {props.dataToDisplay[0].taskName}
                </td>
                <td
                  onMouseEnter={() =>
                    onMouseEnter([props.dataToDisplay[0].user.id])
                  }
                  onMouseLeave={() => onMouseLeave()}
                  className={`whitespace-nowrap px-3 py-2 text-sm text-gray-500 ${shouldHighLight(tmpHighlightIds, props.dataToDisplay[0].shouldHighlightOn) || (hoverGroupsDict[props.dataToDisplay[0].label.id] && hoverGroupsDict[props.dataToDisplay[0].label.id][LabelingPageParts.TABLE_MODAL]) ? settings.main.hoverGroupBackgroundColorClass : ''}`}
                >
                  {props.dataToDisplay[0].createdBy}
                </td>
                <td
                  onMouseEnter={() =>
                    onMouseEnter(
                      [props.dataToDisplay[0].label.id],
                      props.dataToDisplay[0].label.id,
                    )
                  }
                  onMouseLeave={() =>
                    onMouseLeave(props.dataToDisplay[0].label.id)
                  }
                ...
                <td
                  onMouseEnter={() =>
                    onMouseEnter([props.dataToDisplay[0].rla.id])
                  }
                  onMouseLeave={() => onMouseLeave()}
