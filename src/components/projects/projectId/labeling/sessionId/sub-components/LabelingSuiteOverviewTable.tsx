...
import {
  HeaderHover,
  TableDisplayData,
} from '@/src/types/components/projects/projectId/labeling/overview-table'
import { ModalEnum } from '@/src/types/shared/modal'
import { LabelingSuiteManager } from '@/src/util/classes/labeling/manager'
import {
  buildOverviewTableDisplayArray,
  filterRlaDataForUser,
  filterRlaLabelCondition,
  getEmptyHeaderHover,
  rlasHaveHeuristicData,
} from '@/src/util/components/projects/projectId/labeling/overview-table-helper'
...
function shouldHighLight(
  ...
  comparedIds: string[],
  additionalComparedIds?: string[],
) {
  if (additionalComparedIds) {
    return (
      tmpHighlightIds.some((id) => comparedIds.includes(id)) ||
      tmpHighlightIds.some((id) => additionalComparedIds.includes(id))
    )
  }
...
export default function LabelingSuiteOverviewTable() {
  ...
  const [headerHover, setHeaderHover] = useState<HeaderHover>(
    getEmptyHeaderHover(),
  )
    ...
    if (!settings.overviewTable.showHeuristics) {
      if (rlaKey)
        filtered = filtered.filter(
          (entry) => entry[rlaKey].sourceType != LabelSource.INFORMATION_SOURCE,
        )
      else
        filtered = filtered.filter(
          (rla) => rla.sourceType != LabelSource.INFORMATION_SOURCE,
        )
    }
    if (settings.overviewTable.includeLabelDisplaySettings) {
      if (rlaKey)
        filtered = filtered.filter((entry) =>
          filterRlaLabelCondition(entry[rlaKey], settings, projectId),
        )
      else
        filtered = filtered.filter((rla) =>
          filterRlaLabelCondition(rla, settings, projectId),
        )
    }
    return filtered
  }
          ...
          <div className="flex flex-col p-4">
                ...
                <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                  <table className="min-w-full divide-y divide-gray-300 border">
                    <thead className="bg-gray-50">
                      <tr>
                        <th
                          scope="col"
                          onMouseEnter={() =>
                            onMouseEnter(headerHover.typeCollection)
                          }
                          onMouseLeave={onMouseLeave}
                          className={`py-2 pl-4 pr-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500 sm:pl-6 ${shouldHighLight(tmpHighlightIds, headerHover.typeCollection, headerHover.typeCollection) ? headerHover.class : ''}`}
                        >
                          Type
                        </th>
                        <th
                          scope="col"
                          onMouseEnter={() =>
                            onMouseEnter(headerHover.taskCollection)
                          }
                          onMouseLeave={onMouseLeave}
                          className={`px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500 ${shouldHighLight(tmpHighlightIds, headerHover.taskCollection, headerHover.taskCollection) ? headerHover.class : ''}`}
                        >
                          Task
                        </th>
                        <th
                          scope="col"
                          onMouseEnter={() =>
                            onMouseEnter(headerHover.createdByCollection)
                          }
                          onMouseLeave={onMouseLeave}
                          className={`px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500 ${shouldHighLight(tmpHighlightIds, headerHover.createdByCollection, headerHover.createdByCollection) ? headerHover.class : ''}`}
                        >
                          Created by
                        </th>
                        <th
                          scope="col"
                          onMouseEnter={() =>
                            onMouseEnter(headerHover.labelCollection)
                          }
                          onMouseLeave={onMouseLeave}
                          className={`px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-gray-500 ${shouldHighLight(tmpHighlightIds, headerHover.labelCollection, headerHover.labelCollection) ? headerHover.class : ''}`}
                        >
                          Label
                        </th>
                        <th scope="col"></th>
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {dataToDisplay.map((ovItem, index) => (
                        <tr
                          key={ovItem.rla.id}
                        ...
                        >
                          <td
                            onMouseEnter={() =>
                              onMouseEnter([ovItem.sourceTypeKey])
                            }
                            onMouseLeave={onMouseLeave}
                          ...
                          <td
                            onMouseEnter={() => onMouseEnter([ovItem.taskId])}
                            onMouseLeave={onMouseLeave}
                          ...
                          <td
                            onMouseEnter={() =>
                              onMouseEnter([ovItem.label.id], ovItem.label.id)
                            }
                            onMouseLeave={onMouseLeave}
                          ...
                          >
                            <span
                              className={`relative inline-flex cursor-default items-center rounded-md border px-2.5 py-0.5 text-sm font-medium ${ovItem.label.backgroundColor} ${ovItem.label.textColor} ${ovItem.label.borderColor}`}
                            >
                              {ovItem.label.name}
                              <div
                            ...
                            {ovItem.label.value && (
                              <div className="ml-2">{ovItem.label.value}</div>
                            )}
                          </td>
                          <td
                            onMouseEnter={() => onMouseEnter([ovItem.rla.id])}
                            onMouseLeave={onMouseLeave}
                          ...
                          >
                            {ovItem.canBeDeleted && (
                              ...
                              >
                                <IconTrash className="h-6 w-6 cursor-pointer text-red-700" />
                              </div>
                            )}
                          </td>
                        </tr>
            ...
            <div>
              <p className="text-sm font-normal text-gray-500">
                You can hover over the table entries to highlight everything{' '}
                <span
                  className="cursor-pointer underline"
                  onClick={() =>
                    dispatch(openModal(ModalEnum.LABELING_INFO_TABLE))
                  }
                >
                  related
                </span>{' '}
          ...
          {!settings.showHeuristics &&
            ...
            userDisplayRole == UserRole.ENGINEER && (
              <p className="p-5 text-gray-500">
                Heuristics display is currently disabled
              </p>
            )}
        </>
      )}
