...
const ExtractionDisplay = forwardRef<HTMLInputElement, ExtractionDisplayProps>(
  function ExtractionDisplay(props, ref) {
    ...
    return (
      <>
        {props.tokenLookup && (
          <div ref={ref} className="flex flex-row flex-wrap items-start">
            {props.tokenLookup[props.attributeId] &&
              props.tokenLookup[props.attributeId].token &&
              props.tokenLookup[props.attributeId].token.map((token) => (
                <div
                  key={token.idx}
                  className={`relative z-10 ${token.countLineBreaks > 0 && settings.main.lineBreaks != LineBreaksType.NORMAL ? 'w-full' : ''}`}
                  style={{
                    marginBottom:
                      props.tokenLookup[props.attributeId][token.idx]
                        ?.tokenMarginBottom,
                  }}
                  data-tokenidx={token.idx}
                  data-attributeid={props.attributeId}
                >
                  {token.countLineBreaks == 0 ? (
                    <>
                      {token.type ? (
                        <Tooltip
                          content={'spaCy type: ' + token.type}
                          color="invert"
                          placement="top"
                          onKeyDown={(e: any) => {
                            e.preventDefault()
                        ...
                        >
                          <TokenValue
                            ...
                            setSelected={(e) =>
                              props.setSelected(token.idx, token.idx, e)
                            }
                          />
                        </Tooltip>
                      ) : (
                      ...
                      {props.tokenLookup[props.attributeId][token.idx] && (
                        <>
                          {props.tokenLookup[props.attributeId][
                            token.idx
                          ].rlaArray.map((rlaItem: any) => (
                            <div
                              key={rlaItem.orderPos}
                              className={`absolute left-0 right-0 top-0 flex items-end`}
                              style={{
                                bottom: rlaItem.bottomPos,
                                zIndex:
                                  shouldHighlightOn(tmpHighlightIds, [
                                    LabelSourceHover.MANUAL,
                                    rlaItem.rla.id,
                                    rlaItem.rla.createdBy,
                                    rlaItem.rla.labelingTaskLabel.labelingTask
                                      .id,
                                  ]) ||
                                  (hoverGroupsDict[rlaItem.labelId] &&
                                    hoverGroupsDict[rlaItem.labelId][
                                      LabelingPageParts.MANUAL
                                    ]) ||
                                  (hoverGroupsDict[rlaItem.labelId] &&
                                    hoverGroupsDict[rlaItem.labelId][
                                      LabelingPageParts.WEAK_SUPERVISION
                                    ]) ||
                                  hoverBoxDict[rlaItem.rla.id]
                                    ? 1
                                    : 0,
                              }}
                    ...
                    <>
                      {settings.main.lineBreaks != LineBreaksType.NORMAL ? (
                        <>
                          {token.countLineBreaksArray.map((item, index) => (
                            <div key={item}>
                              <br />
                            </div>
                          ))}
...
function TokenValue(props: any) {
  ...
  const isSelected = useMemo(() => {
    ...
    return (
      props.token.idx >= activeTokenSelection.tokenStart &&
      props.token.idx <= activeTokenSelection.tokenEnd
    )
  ...
  return (
    <>
      {props.token && props.token.value != '\n' && (
        <label
          onClick={(e) => props.setSelected(e)}
          className={`relative rounded-lg text-sm font-normal leading-5 text-gray-500 hover:bg-gray-200 ${!props.token.nextCloser ? 'pr-1' : ''}`}
          data-tokenidx={props.token.idx}
          data-attributeid={props.attributeId}
          style={{
            backgroundColor: isSelected ? '#3399FF' : null,
