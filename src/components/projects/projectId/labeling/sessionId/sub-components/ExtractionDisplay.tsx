            ...
            {props.tokenLookup[props.attributeId] && props.tokenLookup[props.attributeId].token && props.tokenLookup[props.attributeId].token.map((token) => (<div key={token.idx} className={`relative z-10 ${token.countLineBreaks > 0 && settings.main.lineBreaks != LineBreaksType.NORMAL ? 'w-full' : ''}`}
                ...
                {token.countLineBreaks == 0 ? (<>
                    ...
                    {props.tokenLookup[props.attributeId][token.idx] && <>
                        {props.tokenLookup[props.attributeId][token.idx].rlaArray.map((rlaItem) => (<div key={rlaItem.orderPos} className={`absolute left-0 right-0 top-0 flex items-end`} style={{ bottom: rlaItem.bottomPos, zIndex: -2 }}
                            onMouseEnter={(e: any) => {
                                const hoverBoxDictCopy = { ...hoverBoxDict };
                                hoverBoxDictCopy[rlaItem.rla.id] = true;
                                setHoverGroupDict(hoverBoxDictCopy);
                                e.target.style.zIndex = -1;
                            }} onMouseLeave={(e: any) => {
                                const hoverBoxDictCopy = { ...hoverBoxDict };
                                for (const key in hoverBoxDictCopy) {
                                    hoverBoxDictCopy[key] = false;
                                }
                                setHoverGroupDict(hoverBoxDictCopy);
                            ...
                            <div className={`h-px flex items-end w-full relative ${props.labelLookup[rlaItem.labelId].color.backgroundColor} ${props.labelLookup[rlaItem.labelId].color.textColor} ${props.labelLookup[rlaItem.labelId].color.borderColor} ${hoverGroupsDict[rlaItem.labelId][LabelingPageParts.MANUAL] || hoverGroupsDict[rlaItem.labelId][LabelingPageParts.WEAK_SUPERVISION] || hoverBoxDict[rlaItem.rla.id] ? 'heightHover' : ''}`}
                                ...
                                }} onMouseEnter={(e: any) => e.target.zIndex = -1}>
                                {rlaItem.isFirst && rlaItem.canBeDeleted && <div onMouseEnter={(e: any) => e.target.zIndex = -2} className="pl-1 cursor-pointer absolute" style={{ bottom: '-11px', left: '-21px' }} onClick={() => deleteRecordLabelAssociation(rlaItem.rla.id)}>
