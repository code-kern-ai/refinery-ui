...
import {
  AVAILABLE_LABELING_LINKS,
  GET_RECORD_LABEL_ASSOCIATIONS,
  GET_TOKENIZED_RECORD,
  LINK_LOCKED,
  REQUEST_HUDDLE_DATA,
} from '@/src/services/gql/queries/labeling'
...
export default function LabelingMainComponent() {
  ...
  const [refetchHuddleData] = useLazyQuery(REQUEST_HUDDLE_DATA, {
    fetchPolicy: 'no-cache',
  })
  const [refetchAvailableLinks] = useLazyQuery(AVAILABLE_LABELING_LINKS, {
    fetchPolicy: 'no-cache',
  })
  const [refetchTokenizedRecord] = useLazyQuery(GET_TOKENIZED_RECORD, {
    fetchPolicy: 'no-cache',
  })
  const [refetchRecordByRecordId] = useLazyQuery(GET_RECORD_BY_RECORD_ID, {
    fetchPolicy: 'no-cache',
  })
  const [refetchRla] = useLazyQuery(GET_RECORD_LABEL_ASSOCIATIONS, {
  ...
  const [refetchLinkLocked] = useLazyQuery(LINK_LOCKED, {
    fetchPolicy: 'no-cache',
  })
    ...
    if (tmp) {
      ...
      for (let key in settingsCopy.task) {
        if (
          ...
          key != 'show' &&
          key != 'isCollapsed' &&
          key != 'alwaysShowQuickButtons'
        )
          delete settingsCopy.task[key]
      }
  ...
  useEffect(() => {
    ...
    refetchLinkLocked({
      variables: { projectId: projectId, linkRoute: router.asPath },
    }).then((result) => {
      const lockedLink = result['data']['linkLocked']
      if (lockedLink) {
        setAbsoluteWarning(
          'This link is locked, contact your supervisor to request access',
        )
  ...
  useEffect(() => {
    ...
    if (!huddleData)
      requestHuddleData(router.query.sessionId as string) //nothing stored in local storage or outdated
    else {
  ...
  useEffect(() => {
    if (!projectId || allUsers.length == 0 || !router.query.sessionId || !user)
      return
  ...
  useEffect(() => {
    ...
    combineLatest([
      refetchTokenizedRecord({
        variables: { recordId: SessionManager.currentRecordId },
      }),
      refetchRecordByRecordId({
        variables: { projectId, recordId: SessionManager.currentRecordId },
      }),
      refetchRla({
        variables: { projectId, recordId: SessionManager.currentRecordId },
      }),
      ...
      const rlas = rla['data']?.['recordByRecordId']?.[
        'recordLabelAssociations'
      ]['edges'].map((e) => e.node)
      dispatch(
        updateRecordRequests(
          'rla',
          prepareRLADataForRole(rlas, user, userDisplayId, userDisplayRole),
        ),
      )
    })
  ...
  useEffect(() => {
    if (!rlas || !user) return
    const [usersIcons, showUsersIcons] = UserManager.prepareUserIcons(
      rlas,
      user,
      users,
    )
  ...
  function setUpCommentsRequestsAndFetch() {
    ...
    requests.push({
      commentType: CommentType.RECORD,
      projectId: projectId,
      commentKey: SessionManager.currentRecordId,
    })
  ...
  function previousRecord() {
    ...
    CommentDataManager.unregisterCommentRequests(CurrentPage.LABELING)
    CommentDataManager.registerCommentRequests(CurrentPage.LABELING, [
      {
        commentType: CommentType.RECORD,
        projectId: projectId,
        commentKey: SessionManager.currentRecordId,
  ...
  function requestHuddleData(huddleId: string) {
    refetchHuddleData({
      variables: {
        projectId: projectId,
        huddleId: huddleId,
        huddleType: SessionManager.labelingLinkData.linkType,
      },
    }).then((result) => {
      ...
      localStorage.setItem(
        'huddleData',
        JSON.stringify(SessionManager.huddleData),
      )
      ...
      dispatch(setDisplayUserRole(user.role))
      router.push(
        `/projects/${projectId}/labeling/${SessionManager.huddleData.linkData.huddleId}?pos=${SessionManager.huddleData.linkData.requestedPos}&type=${SessionManager.huddleData.linkData.linkType}`,
      )
    })
  }
  ...
  function refetchLabelingTasksAndProcess() {
    refetchLabelingTasksByProjectId({
      ...
      const labelingTasksProcessed =
        postProcessLabelingTasksSchema(labelingTasks)
      dispatch(
        setLabelingTasksAll(
          prepareTasksForRole(labelingTasksProcessed, userDisplayRole),
        ),
      )
    })
  }

  function prepareTasksForRole(
    taskData: LabelingTask[],
    userDisplayRole,
  ): LabelingTask[] {
    if (
  ...
  const handleWebsocketNotification = useCallback(
    (msgParts: string[]) => {
      ...
      if (
        ...
        (msgParts[1] == 'calculate_attribute' &&
          ['created', 'updated'].includes(msgParts[2]))
      ) {
        refetchAttributesAndProcess()
        ...
        if (msgParts[2] == recordId) {
          refetchRla({ variables: { projectId, recordId: recordId } }).then(
            (result) => {
              const rlas = result['data']?.['recordByRecordId']?.[
                'recordLabelAssociations'
              ]['edges'].map((e) => e.node)
              dispatch(
                updateRecordRequests(
        ...
        ['access_link_changed', 'access_link_removed'].includes(msgParts[1])
      ) {
        if (
          router.pathname.includes(msgParts[3]) &&
          SessionManager.labelingLinkData
        ) {
          //python "True" string
          SessionManager.labelingLinkData.linkLocked =
            !msgParts[4] || msgParts[4] === 'True'
          location.reload()
        }
      } else if (
        [
  ...
  return (
    <div
      className={`flex h-full flex-col bg-white ${LabelingSuiteManager.somethingLoading ? style.wait : ''}`}
    >
      <NavigationBarTop
        absoluteWarning={absoluteWarning}
      ...
      {!lockedLink ? (
        ...
        <div className="relative flex h-16 flex-shrink-0 items-center justify-between bg-white shadow-sm"></div>
      )}
    </div>
  )
}
