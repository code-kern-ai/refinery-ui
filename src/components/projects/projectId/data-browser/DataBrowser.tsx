...
import {
  DATA_SLICES,
  GET_RECORD_COMMENTS,
  GET_UNIQUE_VALUES_BY_ATTRIBUTES,
...
import {
  ...
  postProcessUniqueValues,
  postProcessUsersCount,
...
export default function DataBrowser() {
  ...
  const [refetchUsersCount] = useLazyQuery(GET_ORGANIZATION_USERS_WITH_COUNT, {
  ...
  const [refetchRecordComments] = useLazyQuery(GET_RECORD_COMMENTS, {
    fetchPolicy: 'no-cache',
  })
  const [refetchComments] = useLazyQuery(REQUEST_COMMENTS, {
    fetchPolicy: 'no-cache',
  })
  const [refetchUniqueValues] = useLazyQuery(GET_UNIQUE_VALUES_BY_ATTRIBUTES, {
    fetchPolicy: 'no-cache',
  })
  ...
  useEffect(() => {
    ...
    refetchExtendedRecord({
      ...
      const parsedRecordData = postProcessRecordsExtended(
  ...
  function refetchDataSlicesAndProcess(dataSliceId?: string) {
    refetchDataSlices({ variables: { projectId: projectId } }).then((res) => {
      ...
      if (dataSliceId) {
        const findSlice = res.data.dataSlices.find(
          (slice) => slice.id == dataSliceId,
        )
  ...
  function refetchUsersCountAndProcess() {
    refetchUsersCount({ variables: { projectId: projectId } }).then((res) => {
      dispatch(
        setUsersMapCount(
          postProcessUsersCount(
            res.data['allUsersWithRecordCount'],
            users,
            user,
          ),
        ),
  ...
  function refetchRecordCommentsAndProcess(parsedRecordData: any) {
    ...
    refetchRecordComments({
      variables: { projectId: projectId, recordIds: currentRecordIds },
    }).then((res) => {
  ...
  function getNextRecords() {
    setSearchRequest({
      offset: searchRequest.offset + searchRequest.limit,
      limit: searchRequest.limit,
    })
  }
  ...
  function refetchUniqueValuesAndProcess() {
    refetchUniqueValues({ variables: { projectId: projectId } }).then((res) => {
      dispatch(
        setUniqueValuesDict(
          postProcessUniqueValues(
            res.data['uniqueValuesByAttributes'],
            attributes,
          ),
        ),
      )
  ...
  const handleWebsocketNotification = useCallback(
    (msgParts: string[]) => {
      if (
        ...
        ['calculate_attribute', 'attributes_updated'].includes(msgParts[1])
      ) {
        refetchAttributesAndProcess()
      } else if (
