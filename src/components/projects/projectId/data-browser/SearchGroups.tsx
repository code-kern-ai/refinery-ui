...
import {
  attributeCreateSearchGroup,
  commentsCreateSearchGroup,
  generateRandomSeed,
  getBasicGroupItems,
  getBasicSearchGroup,
  getBasicSearchItem,
  labelingTasksCreateSearchGroup,
  orderByCreateSearchGroup,
  userCreateSearchGroup,
} from '@/src/util/components/projects/projectId/data-browser/search-groups-helper'
import {
  SearchGroup,
  Slice,
  StaticOrderByKeys,
...
import {
  checkDecimalPatterns,
  getAttributeType,
  getSearchOperatorTooltip,
} from '@/src/util/components/projects/projectId/data-browser/search-operators-helper'
...
import {
  ...
  selectConfiguration,
  selectDataSlicesAll,
  selectFullSearchStore,
  selectIsTextHighlightNeeded,
  selectRecords,
  selectSearchGroupsStore,
  selectSimilaritySearch,
  selectTextHighlight,
  selectUniqueValuesDict,
  selectUsersCount,
  setActiveDataSlice,
  ...
  setRecordsInDisplay,
  setSearchGroupsStore,
  setSearchRecordsExtended,
  setTextHighlight,
...
import {
  GET_RECORDS_BY_STATIC_SLICE,
  GET_STATIC_DATA_SLICE_CURRENT_COUNT,
  SEARCH_RECORDS_EXTENDED,
} from '@/src/services/gql/queries/data-browser'
import {
  getActiveNegateGroupColor,
  postProcessRecordsExtended,
} from '@/src/util/components/projects/projectId/data-browser/data-browser-helper'
import { parseFilterToExtended } from '@/src/util/components/projects/projectId/data-browser/filter-parser-helper'
import {
  getRegexFromFilter,
  updateSearchParameters,
} from '@/src/util/components/projects/projectId/data-browser/search-parameters'
...
import {
  checkActiveGroups,
  prefillActiveValues,
} from '@/src/util/components/projects/projectId/data-browser/prefill-values-helper'
...
export default function SearchGroups() {
  ...
  const [searchGroupsOrder, setSearchGroupsOrder] = useState<
  ...
  const [currentWeakSupervisionRun, setCurrentWeakSupervisionRun] =
    useState(null)
  const [selectedHeuristicsWS, setSelectedHeuristicsWS] = useState<string[]>([])

  const [refetchExtendedRecord] = useLazyQuery(SEARCH_RECORDS_EXTENDED, {
    fetchPolicy: 'no-cache',
  })
  const [refetchCurrentWeakSupervision] = useLazyQuery(
    GET_CURRENT_WEAK_SUPERVISION_RUN,
    { fetchPolicy: 'network-only' },
  )
  const [refetchRecordsStatic] = useLazyQuery(GET_RECORDS_BY_STATIC_SLICE, {
    fetchPolicy: 'network-only',
  })
  const [refetchStaticSliceCurrentCount] = useLazyQuery(
    GET_STATIC_DATA_SLICE_CURRENT_COUNT,
    { fetchPolicy: 'network-only' },
  )
  ...
  useEffect(() => {
    ...
    const visibleAttributes = attributes.filter(
      (a) => a.visibility == AttributeVisibility.DO_NOT_HIDE,
    )
    attributesSort.push({
      name: 'Any Attribute',
      key: null,
  ...
  useEffect(() => {
    ...
    if (
      !fullSearchStore ||
      fullSearchStore[SearchGroup.DRILL_DOWN] == undefined
    )
      return
    ...
    if (activeSlice && activeSlice.static) {
      refetchRecordsStatic({
        ...
        dispatch(
          setSearchRecordsExtended(
            postProcessRecordsExtended(
              res.data['recordsByStaticSlice'],
              labelingTasks,
            ),
          ),
        )
        refetchStaticSliceCurrentCount({
          variables: { projectId: projectId, sliceId: activeSlice.id },
        }).then((res) => {
          if (!res.data) {
            dispatch(
              updateAdditionalDataState('staticDataSliceCurrentCount', null),
            )
            return
          }
          dispatch(
            updateAdditionalDataState(
              'staticDataSliceCurrentCount',
              res['data']['staticDataSlicesCurrentCount'],
            ),
          )
        })
      })
        ...
        refetchExtendedRecord({
          variables: {
            ...
            filterData: parseFilterToExtended(
              ...
              labelingTasks,
              user,
              fullSearchStore[SearchGroup.DRILL_DOWN],
            ),
            offset: 0,
            limit: 20,
          },
          ...
          dispatch(
            setSearchRecordsExtended(
              postProcessRecordsExtended(
                res.data['searchRecordsExtended'],
                labelingTasks,
              ),
            ),
          )
  ...
  useEffect(() => {
    if (
      ...
      return
    if (
      activeSlice.sliceType == Slice.STATIC_DEFAULT ||
    ...
    const activeParams = updateSearchParameters(
      Object.values(JSON.parse(findDataSLice.filterRaw)),
      attributes,
      configuration.separator,
      jsonCopy(fullSearchStore),
      searchGroupsStore,
      labelingTasks,
    )
  ...
  useEffect(() => {
    ...
    setSelectedHeuristicsWS(
      currentWeakSupervisionRun.selectedInformationSources.split(','),
    )
  ...
  function prepareSearchGroups() {
    ...
    // Attributes
    const searchGroupAttributes = getBasicSearchGroup(
    ...
    for (let baseItem of getBasicGroupItems(
      ...
      fullSearchCopy[SearchGroup.ATTRIBUTES].groupElements.push(
        attributeCreateSearchGroup(baseItem, ++GLOBAL_SEARCH_GROUP_COUNT),
      )
    }

    // User Filter
    const searchGroupUserFilter = getBasicSearchGroup(
      SearchGroup.USER_FILTER,
      GROUP_SORT_ORDER + 200,
    )
    ...
    for (let baseItem of getBasicGroupItems(
      ...
      fullSearchCopy[SearchGroup.USER_FILTER].groupElements =
        userCreateSearchGroup(baseItem, ++GLOBAL_SEARCH_GROUP_COUNT, usersMap)
    }

    // Labeling Tasks
    let count = 0
    for (let task of labelingTasks) {
      const searchGroupLabelingTasks = getBasicSearchGroup(
        SearchGroup.LABELING_TASKS,
        GROUP_SORT_ORDER + 300 + count,
        task.name,
        task.id,
      )
      searchGroupsCopy[SearchGroup.LABELING_TASKS + '_' + task.id] =
        searchGroupLabelingTasks
      fullSearchCopy[SearchGroup.LABELING_TASKS + '_' + task.id] = {
        groupElements: [],
      }
      for (let baseItem of getBasicGroupItems(
        searchGroupLabelingTasks.group,
        searchGroupLabelingTasks.key,
      )) {
        fullSearchCopy[
          SearchGroup.LABELING_TASKS + '_' + task.id
        ].groupElements = labelingTasksCreateSearchGroup(
          baseItem,
          task,
          ++GLOBAL_SEARCH_GROUP_COUNT,
        )
      }
      count++
    }
    ...
    // comments
    const searchGroupComments = getBasicSearchGroup(
      SearchGroup.COMMENTS,
      GROUP_SORT_ORDER + 500,
    )
    fullSearchCopy[SearchGroup.COMMENTS] = { groupElements: [] }
    searchGroupsCopy[SearchGroup.COMMENTS] = searchGroupComments
    for (let baseItem of getBasicGroupItems(
      searchGroupComments.group,
      searchGroupComments.key,
    )) {
      fullSearchCopy[SearchGroup.COMMENTS].groupElements =
        commentsCreateSearchGroup(baseItem, ++GLOBAL_SEARCH_GROUP_COUNT)
    }
    ...
    for (let [key, value] of Object.entries(searchGroupsCopy)) {
      ...
      if (findEl == undefined) {
        searchGroupsOrderCopy.push({
          order: (value as any).sortOrder,
          key: key,
        })
      }
    }
  ...
  function prepareNewFormGroups(
    filterRaw: any,
    usersMap: any,
    searchGroupsCopy,
  ) {
    const parse = JSON.parse(filterRaw)
    const labelingTasksInSlice = Object.keys(parse).filter((el) =>
      el.includes(SearchGroup.LABELING_TASKS),
    )
    const tasksIds = labelingTasksInSlice.map((el) => el.split('_')[2])
    const fullSearchStoreCopy = parse.hasOwnProperty(SearchGroup.CATEGORY)
      ? parse
      : jsonCopy(fullSearchStore)
    labelingTasks.forEach((task, index) => {
      const searchGroupLabelingTasks = getBasicSearchGroup(
        SearchGroup.LABELING_TASKS,
        GROUP_SORT_ORDER + 300 + index,
        task.name,
        task.id,
      )
      if (!tasksIds.includes(task.id)) {
        fullSearchStoreCopy[SearchGroup.LABELING_TASKS + '_' + task.id] = {
          groupElements: [],
        }
        for (let baseItem of getBasicGroupItems(
          searchGroupLabelingTasks.group,
    ...
    if (parse.hasOwnProperty(SearchGroup.CATEGORY)) {
      ...
      const prefilledValues = prefillActiveValues(
        parse,
        fullSearchStoreCopy,
        usersMap,
      )
      dispatch(setFullSearchStore(prefilledValues))
      const openGroups = checkActiveGroups(
        fullSearchStoreCopy,
        searchGroupsCopy,
      )
      dispatch(setSearchGroupsStore(openGroups))
  ...
  function setActiveNegateGroup(
    ...
    if (!groupItemCopy['active']) groupItemCopy['active'] = true
    ...
    if (group.key == SearchGroup.USER_FILTER) {
      ...
      // for the labeling tasks because of the different structure
      fullSearchCopy[group.key].groupElements['heuristics'][index] =
        groupItemCopy
  ...
  function getOperatorDropdownValues(
    i?: number,
    value?: any,
    fullSearchCopyParam?: any,
  ) {
    const operatorsCopy = []
    const tooltipsCopy = []
    const fullSearchCopy = fullSearchCopyParam
      ? jsonCopy(fullSearchCopyParam)
      : jsonCopy(fullSearchStore)
    const formControlsIdx =
      fullSearchCopy[SearchGroup.ATTRIBUTES].groupElements[i]
    ...
    if (attributeType !== DataTypeEnum.BOOLEAN) {
      ...
      if (formControlsIdx) {
        ...
        formControlsIdx['addText'] =
          attributeType == DataTypeEnum.INTEGER
            ? 'Enter any number'
            : attributeType == DataTypeEnum.FLOAT
              ? 'Enter any float'
              : 'Enter any string'
      }
    } else {
  ...
  function addSearchGroupItem(groupItem, groupKey) {
    ...
    fullSearchCopy[groupKey].groupElements.push(
      attributeCreateSearchGroup(item, ++GLOBAL_SEARCH_GROUP_COUNT),
    )
    dispatch(setFullSearchStore(fullSearchCopy))
    updateSearchParams(fullSearchCopy)
  }
  ...
  function selectValueDropdown(value: any, i: number, field: string, key: any) {
    ...
    if (field == 'name') {
      ...
      if (
        attributeType == DataTypeEnum.BOOLEAN &&
  ...
  function updateLabelsFullSearch(
    label: any,
    groupKey: string,
    labelsKey: string,
  ) {
    ...
    fullSearchCopy[groupKey].nameAdd = labelingTasks.find(
      (el) => el.id == groupKey.split('_')[2],
    ).name
  ...
  function changeConfidence(
    ...
    group['active'] = true
    fullSearchCopy[key].nameAdd = labelingTasks.find(
      (el) => el.id == key.split('_')[2],
    ).name
    dispatch(setFullSearchStore(fullSearchCopy))
    updateSearchParams(fullSearchCopy)
  }
  ...
  function updateIsDifferent(groupKey: string, group: any) {
    const fullSearchCopy = jsonCopy(fullSearchStore)
    fullSearchCopy[groupKey].groupElements['isWithDifferentResults'].active =
      !fullSearchCopy[groupKey].groupElements['isWithDifferentResults'].active
    fullSearchCopy[groupKey].groupElements['isWithDifferentResults'].color =
      getActiveNegateGroupColor(
        fullSearchCopy[groupKey].groupElements['isWithDifferentResults'],
      )
    fullSearchCopy[groupKey].nameAdd = labelingTasks.find(
      (el) => el.id == groupKey.split('_')[2],
    ).name
  ...
  function setSortFormControl(index, group) {
    const fullSearchCopy = jsonCopy(fullSearchStore)
    const formControlsIdx =
      fullSearchCopy[group.key].groupElements['orderBy'][index]
    const findActive = fullSearchCopy[group.key].groupElements['orderBy'].find(
      (el) => el['active'] == true,
    )
    if (findActive && findActive != formControlsIdx)
      findActive['active'] = false
  ...
  function setRandomSeedGroup(value?: string) {
    const fullSearchCopy = jsonCopy(fullSearchStore)
    const formControlsIdx = fullSearchCopy[
      SearchGroup.ORDER_STATEMENTS
    ].groupElements['orderBy'].find(
      (el) => el['orderByKey'] == StaticOrderByKeys.RANDOM,
    )
  ...
  function updateSearchParams(fullSearchCopy) {
    const activeParams = updateSearchParameters(
      Object.values(fullSearchCopy),
      attributes,
      configuration.separator,
      jsonCopy(fullSearchCopy),
      searchGroupsStore,
    )
    dispatch(setActiveSearchParams(activeParams))
    dispatch(updateAdditionalDataState('clearFullSearch', false))
    dispatch(setRecordsInDisplay(false))
    if (activeSlice && activeSlice.static)
      dispatch(updateAdditionalDataState('displayOutdatedWarning', true))
  }
        ...
        if (searchElement.values.group == SearchGroup.ATTRIBUTES) {
          if (
            ...
            searchElement.values.name ==
              attributesDict[attributesSortOrder[i].key].name
          ) {
            if (typeof searchElement.values.searchValue != 'string') {
              searchElement.values.searchValue =
                searchElement.values.searchValue.toString()
            }
      ...
      for (let searchElement of activeSearchParams) {
        if (searchElement.values.group == SearchGroup.ATTRIBUTES) {
          if (getRegexFromFilter(searchElement) == null) {
            ...
            if (
              searchElement.values.name == 'Any Attribute' ||
              searchElement.values.name == attributesDict[attributeKey].name
            ) {
              if (
  ...
  function refetchCurrentWeakSupervisionAndProcess() {
    refetchCurrentWeakSupervision({ variables: { projectId: projectId } }).then(
      (res) => {
        if (res == null) {
          setCurrentWeakSupervisionRun({ state: Status.NOT_YET_RUN })
        } else {
          setCurrentWeakSupervisionRun(
            postProcessCurrentWeakSupervisionRun(
              res['data']['currentWeakSupervisionRun'],
            ),
          )
        }
      },
  ...
  return (
    <>
      {searchGroupsStore &&
        searchGroupsOrder.map((group) => (
          <div key={group.key} className="mt-4">
            <div
              onClick={() => toggleGroupMenu(group.key)}
              className={`flex cursor-pointer flex-row items-center rounded-md hover:bg-gray-50`}
            >
              <div
                className={`w-7 ${searchGroupsStore[group.key].isOpen ? style.rotateTransform : null} ${style.transitionTransform}`}
              >
              ...
              <div className="flex flex-col">
                ...
                >
                  ...
                  {searchGroupsStore[group.key].nameAdd != '' && (
                ...
                >
                  ...
                  {searchGroupsStore[group.key].nameAdd != '' && (
                    <label className="ml-1 font-normal">
                      {searchGroupsStore[group.key].nameAdd}
                    </label>
                  )}
                </div>
            ...
            <div
              className={`${style.transitionAll} ml-4`}
              style={{
                maxHeight: searchGroupsStore[group.key].isOpen
                  ? '1200px'
                  : '0px',
                display: searchGroupsStore[group.key].isOpen ? 'block' : 'none',
              }}
            >
              <form>
                {searchGroupsStore[group.key].group ==
                  SearchGroup.ATTRIBUTES && (
                  <div className="mx-2 contents">
                    {fullSearchStore[group.key].groupElements.map(
                      (groupItem, index) => (
                        <div key={groupItem.id}>
                          <div className="my-2 flex flex-row items-center rounded-md hover:bg-gray-50">
                            <div className="flex flex-col">
                              <div
                                onClick={() =>
                                  setActiveNegateGroup(groupItem, index, group)
                                }
                                style={{
                                  backgroundColor: groupItem.color,
                                  borderColor: groupItem.color,
                                }}
                                className="ml-2 mr-2 h-4 w-4 cursor-pointer rounded border border-gray-300 hover:bg-gray-200"
                              ></div>
                              {fullSearchStore[group.key].groupElements.length >
                                1 && (
                                <div
                                  onClick={() =>
                                    removeSearchGroupItem(group.key, index)
                                  }
                                  className="hover:border-transparent hover:bg-transparent border-transparent bg-transparent mt-2 flex cursor-pointer justify-center px-0"
                                >
                                  <IconTrash className="h-4 w-4 cursor-pointer text-gray-900" />
                                </div>
                              )}
                            </div>
                            <div className="mr-2.5 mt-2 flex flex-grow  flex-col ">
                              <div className="flex flex-grow flex-row flex-wrap gap-1">
                                <div
                                  style={{
                                    width:
                                      groupItem.operator != '' ? '49%' : '100%',
                                  }}
                                >
                                  <Dropdown2
                                    options={attributesSortOrder}
                                    buttonName={groupItem.name}
                                ...
                                <div style={{ width: '49%' }}>
                                  {groupItem.operator != '' && (
                                    <Dropdown2
                                      options={operatorsDropdown}
                                      buttonName={groupItem.operator}
                              ...
                              {uniqueValuesDict[groupItem['name']] &&
                              groupItem['operator'] != '' &&
                              groupItem['operator'] != 'BETWEEN' &&
                              groupItem['operator'] != 'IN' &&
                              groupItem['operator'] != 'IN WC' ? (
                                <div className="my-2">
                                  <Dropdown2
                                    options={
                                      uniqueValuesDict[groupItem['name']]
                                ...
                                <div className="my-2 flex flex-grow flex-row items-center">
                                  {groupItem['operator'] != '' && (
                                    <input
                                      placeholder={groupItem['addText']}
                                      value={groupItem['searchValue']}
                                      onChange={(e) =>
                                        selectValueDropdown(
                                          e.target.value,
                                          index,
                                      ...
                                      onKeyDown={(e) =>
                                        checkIfDecimals(e, index, group.key)
                                      }
                                  ...
                                  {groupItem['operator'] ==
                                    ...
                                    <input
                                      placeholder={groupItem['addText']}
                                      value={groupItem['searchValueBetween']}
                                      onChange={(e) =>
                                        selectValueDropdown(
                                          e.target.value,
                                          index,
                              ...
                              {(groupItem['operator'] == 'BEGINS WITH' ||
                                groupItem['operator'] == 'ENDS WITH' ||
                                groupItem['operator'] ==
                                  SearchOperator.CONTAINS ||
                                groupItem['operator'] == 'IN WC') &&
                                saveAttributeType != DataTypeEnum.INTEGER &&
                                saveAttributeType != DataTypeEnum.FLOAT && (
                                  <label
                                    htmlFor="caseSensitive"
                                    className="flex cursor-pointer items-center pb-2 text-xs text-gray-500"
                                  >
                                    <input
                                      name="caseSensitive"
                          ...
                          <div className="flex w-full justify-center">
                            {index ==
                              ...
                              <span
                                onClick={() =>
                                  addSearchGroupItem(groupItem, group.key)
                                }
                                className="cursor-pointer rounded-md bg-gray-100 p-1 text-gray-800 hover:bg-gray-300"
                              >
                ...
                {searchGroupsStore[group.key].group ==
                  ...
                  <div className="mt-4 flex flex-row items-center">
                    <div className="flex flex-grow">
                      <div className="flex flex-col">
                        {fullSearchStore[group.key].groupElements['users'] &&
                          fullSearchStore[group.key].groupElements['users'].map(
                            (groupItem, index) => (
                              <div key={groupItem.id} className="my-1">
                                <div className="form-control flex flex-row flex-nowrap items-center">
                                  ...
                                  {usersMap && usersMap[groupItem['id']] && (
                                    <div>
                                      <Tooltip
                                        content={groupItem['dataTip']}
                                        placement="right"
                                        color="invert"
                                      >
                                        <IconInfoCircle
                ...
                {searchGroupsStore[group.key].group ==
                  SearchGroup.LABELING_TASKS && (
                  <div className="mt-4 flex flex-row items-center">
                    {fullSearchStore[group.key] && (
                      <div className="flex flex-grow flex-col">
                        <div>Manually labeled</div>
                        {fullSearchStore[group.key].groupElements[
                          'manualLabels'
                        ] &&
                        fullSearchStore[group.key].groupElements['manualLabels']
                          .length == 0 ? (
                          <ButtonLabelsDisabled />
                        ) : (
                          <Dropdown2
                            options={
                              fullSearchStore[group.key].groupElements[
                        ...
                        fullSearchStore[group.key].groupElements[
                          ...
                          <Dropdown2
                            ...
                            buttonName={
                              weakSupervisionLabels.length == 0
                                ? 'None selected'
                                : weakSupervisionLabels.join(',')
                            }
                            hasCheckboxesThreeStates={true}
                            selectedOption={(option: any) => {
                              const labels = [
                        ...
                        <div className="mt-1 min-w-0 flex-grow">
                          <div className="flex flex-row items-center whitespace-nowrap">
                            <span className="font-dmMono mr-0.5 text-sm">
                              CONFIDENCE BETWEEN
                            </span>
                            <input
                              disabled={
                              ...
                              className="placeholder-italic h-8 w-11 rounded-md border border-gray-300 pl-2 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
                            />
                            <span className="font-dmMono mx-0.5 text-sm">
                              % AND
                            </span>
                            <input
                              disabled={
                        ...
                        fullSearchStore[group.key].groupElements[
                          ...
                          <Dropdown2
                            ...
                            buttonName={
                              modelCallBacksLabels.length == 0
                                ? 'None selected'
                                : modelCallBacksLabels.join(',')
                            }
                            hasCheckboxesThreeStates={true}
                            selectedOption={(option: any) => {
                              const labels = [
                                ...modelCallBacksLabels,
                                option.name,
                              ]
                        ...
                        fullSearchStore[group.key].groupElements['heuristics']
                          .length == 0 ? (
                          <div className="text-sm text-gray-400">
                            No heuristics associated with this task
                          </div>
                        ) : (
                          <div className="flex flex-col">
                            <div
                              className="flex cursor-pointer items-center border-b border-t border-b-gray-300 border-t-gray-300"
                              onClick={() =>
                                updateIsDifferent(
                                  group.key,
                                  fullSearchStore[group.key].groupElements[
                                    'isWithDifferentResults'
                                  ],
                                )
                              }
                            >
                              <div
                                ...
                                className="ml-2 mr-2 h-4 w-4 cursor-pointer rounded border border-gray-300 hover:bg-gray-200"
                              ></div>
                              <span className="w-full truncate pl-2 text-sm">
                ...
                {searchGroupsStore[group.key].group ==
                  SearchGroup.ORDER_STATEMENTS && (
                  <div className="mt-4">
                    {fullSearchStore[group.key].groupElements['orderBy'] &&
                      fullSearchStore[group.key].groupElements['orderBy'].map(
                        (groupItem, index) => (
                          <div key={groupItem.id}>
                            <div className="form-control class mb-2">
                              {groupItem['orderByKey'] !=
                              StaticOrderByKeys.RANDOM ? (
                                <div className="mb-2 flex items-center">
                                  ...
                                  >
                                    <div
                                      className={`cursor-pointer p-0 ${groupItem['direction'] == 1 ? style.rotateTransform : null}`}
                                    >
                                      <div className="ml-2 mr-2 flex h-4 w-4 cursor-pointer items-center justify-center rounded border border-gray-300 hover:bg-gray-200">
                                        {groupItem['active'] != 0 && (
                                          <IconArrowDown className="h-3 w-3 text-gray-500" />
                                        )}
                                      </div>
                                    </div>
                                    <span className="ml-2 w-full truncate text-sm">
                                      {groupItem['displayName']}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="mr-2 flex flex-row items-center">
                                  ...
                                  <div className="flex rounded-md shadow-sm">
                                    <span className="ml-2 inline-flex items-center rounded-l-md border border-r-0 border-gray-300 bg-gray-50 px-2.5 text-sm text-gray-500 sm:text-sm">
                                      Seed
                                      <div
                                        className="ml-2 cursor-pointer"
                                        onClick={() => setRandomSeedGroup()}
                                      >
                                        <IconArrowsRandom />
                                      </div>
                                    </span>
                                    <input
                                      value={groupItem['seedString']}
                                      onChange={(e) =>
                                        setRandomSeedGroup(e.target.value)
                                      }
                                      className="placeholder-italic h-8 w-36 rounded-r-md border border-gray-300 pl-4 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100"
                                    />
                                  </div>
                                </div>
                              )}
                                ...
                                >
                                  <div
                                    style={{
                                      backgroundColor:
                                        fullSearchStore[group.key]
                                          .groupElements['hasComments'].color,
                                      borderColor:
                                        fullSearchStore[group.key]
                                          .groupElements['hasComments'].color,
                                    }}
                ...
                {searchGroupsStore[group.key].group != SearchGroup.ATTRIBUTES &&
                  searchGroupsStore[group.key].group !=
                    SearchGroup.USER_FILTER &&
                  searchGroupsStore[group.key].group !=
                    SearchGroup.LABELING_TASKS &&
                  searchGroupsStore[group.key].group !=
                    SearchGroup.ORDER_STATEMENTS &&
                  searchGroupsStore[group.key].group !=
                    SearchGroup.COMMENTS && <p>{'Default :('}</p>}
              </form>
            </div>
          </div>
        ))}
      ...
      >
        {fullSearchStore && (
          <div className="flex flex-row items-center">
            <Tooltip
              content={TOOLTIPS_DICT.DATA_BROWSER.CONNECT}
              color="invert"
              placement="right"
              className="cursor-auto"
            >
              <div className="filtersUnderline mr-2 cursor-help underline">
                Connect by
              </div>
            </Tooltip>
            <div className="flex items-center">
              <input
                type="radio"
                id="radio-drill-down-inactive"
                name="radio-drill-down-inactive"
                onChange={() => handleDrillDown(false)}
                checked={!fullSearchStore[SearchGroup.DRILL_DOWN]}
                className="h-4 w-4 border-gray-200 text-blue-600 focus:ring-blue-500"
              />
              <label
                htmlFor="radio-drill-down-inactive"
                className="label font-dmMono cursor-pointer p-1 pr-2 text-sm"
              >
                OR
              </label>
            </div>
            <div className="mr-3 flex items-center">
              <input
                type="radio"
                id="radio-drill-down-active"
                name="radio-drill-down-active"
                onChange={() => handleDrillDown(true)}
                checked={fullSearchStore[SearchGroup.DRILL_DOWN]}
                className="h-4 w-4 border-gray-200 text-blue-600 focus:ring-blue-500"
              />
              <label
                htmlFor="radio-drill-down-active"
                className="label font-dmMono cursor-pointer p-1 pr-2 text-sm"
              >
                AND
              </label>
...
function ButtonLabelsDisabled() {
  return (
    <button
      disabled={true}
      className="inline-flex w-80 cursor-not-allowed items-center rounded-md border border-gray-300 bg-white px-4 py-1.5 text-xs font-semibold text-gray-700 shadow-sm focus:ring-offset-2 focus:ring-offset-gray-400"
    >
      <div className="mr-4 min-w-0 truncate">
