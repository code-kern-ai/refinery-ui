...
export default function RecordCommentsModal() {
  ...
  return (
    <Modal modalName={ModalEnum.RECORD_COMMENTS}>
      <h1 className="mb-2 text-center text-lg text-gray-900">
        Record comments
      </h1>
      {modalComments &&
        modalComments.commentsData &&
        modalComments.commentsData.map((comment, index) => (
          <div className="break-words" key={comment.comment}>
            <div className="flex flex-grow justify-between gap-8">
              <p style={{ minWidth: '100px' }} className="font-bold">
                Comment {index + 1}
              </p>
