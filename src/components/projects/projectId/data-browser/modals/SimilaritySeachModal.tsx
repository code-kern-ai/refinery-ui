...
import {
  filterAttributesSSGroup,
  getPlaceholderText,
  prepareAttFilter,
  prepareFilterAttributes,
} from '@/src/util/components/projects/projectId/data-browser/filter-attributes-helper'
import {
  checkDecimalPatterns,
  getAttributeType,
  getFilterIntegrationOperatorTooltip,
...
const SECOND_ACCEPT_BUTTON = {
  buttonCaption: 'Search without filter',
  useButton: true,
  disabled: false,
}
...
export default function SimilaritySearchModal() {
  ...
  const [operatorsDict, setOperatorsDict] = useState<{
    [key: string]: string[]
  }>({})
  ...
  const [refetchSimilarRecords] = useLazyQuery(SEARCH_SIMILAR_RECORDS, {
  ...
  function getSimilarRecords(hasFilter: boolean = false) {
    refetchSimilarRecords({
      variables: {
        ...
        attFilter: hasFilter
          ? prepareAttFilter(filterAttributesForm, attributes)
          : null,
        recordSubKey: null,
      },
    }).then((res) => {
      dispatch(
        setSearchRecordsExtended(
          postProcessRecordsExtended(
            res.data['searchRecordsBySimilarity'],
            labelingTasks,
          ),
        ),
      )
  ...
  useEffect(() => {
    setAcceptButton({
      ...ACCEPT_BUTTON,
      emitFunction: requestSimilaritySearch,
      disabled: selectedEmbedding == null,
    })
    setSecondAcceptButton({
      ...SECOND_ACCEPT_BUTTON,
      emitFunction: requestSimilaritySearchWithoutFilter,
      useButton: filterAttributesSS,
      disabled: selectedEmbedding == null,
    })
  }, [modalSS, selectedEmbedding, filterAttributesSS])

  const [acceptButton, setAcceptButton] = useState<ModalButton>(ACCEPT_BUTTON)
  const [secondAcceptButton, setSecondAcceptButton] =
    useState<ModalButton>(SECOND_ACCEPT_BUTTON)

  useEffect(() => {
    if (!embeddings || !selectedEmbedding) return
    setFilterAttributesSS(
      prepareFilterAttributes(embeddings, selectedEmbedding.name),
    )
    ...
    filterAttributesSS.forEach((attribute: string) => {
      ...
      if (attributeType !== DataTypeEnum.INTEGER) {
        operators = operators.filter(
          (operator) => operator !== FilterIntegrationOperator.BETWEEN,
        )
        tooltips = tooltips.filter(
          (tooltip) =>
            tooltip !==
            getFilterIntegrationOperatorTooltip(
              FilterIntegrationOperator.BETWEEN,
            ),
        )
      }
  ...
  return (
    <Modal
      modalName={ModalEnum.SIMILARITY_SEARCH}
    ...
    >
      <div className="flex flex-grow justify-center text-lg font-medium leading-6 text-gray-900">
        Select embedding for similarity search{' '}
      </div>
      ...
      <Dropdown2
        ...
        selectedOption={(value) => setSelectedEmbedding(value)}
        dropdownClasses="my-2"
      />

      {filterAttributesSS && filterAttributesSS.length > 0 && (
        <div className="mt-4 flex flex-col justify-start">
          <div className="text-md bg-white pr-2 font-medium text-gray-900">
            Filter attributes
          </div>
          <div className="text-sm text-gray-400">
            You can filter similarity search based on the attributes selected
      ...
      <div className="mx-2 contents">
        {filterAttributesForm &&
          filterAttributesForm.map((form, index) => (
            <div key={form.id} className="mx-2 contents">
              <div className="my-2 flex flex-row items-center rounded-md hover:bg-gray-50">
                <div className="flex flex-col">
                  {filterAttributesForm.length > 1 && (
                    <div
                      onClick={() => removeFilterAttributesSS(index)}
                ...
                <div className="mr-2.5 mt-2 flex flex-grow  flex-col ">
                  <div className="flex flex-grow flex-row flex-wrap gap-1">
                    <div style={{ width: '50%' }}>
                      <Dropdown2
                        options={filterAttributesSS}
                        buttonName={form.name}
                        backgroundColors={colorsAttributes}
                        selectedOption={(option: any) =>
                          setFilterDropdownVal(option, index, 'name')
                        }
                    ...
                    <div style={{ width: '49%' }}>
                      <Dropdown2
                        options={operatorsDict[form.name]}
                        buttonName={form.operator}
                        tooltipsArray={tooltipsDict[form.operator]}
                        tooltipArrayPlacement="left"
                        selectedOption={(option: any) =>
                          setFilterDropdownVal(option, index, 'operator')
                        }
                        fontClass="font-dmMono"
                      />
                    </div>
                  </div>
                  {uniqueValuesDict[form['name']] &&
                  form['operator'] != '' &&
                  form['operator'] == 'EQUAL' ? (
                    <div className="mt-2 w-full">
                      <Dropdown2
                        options={uniqueValuesDict[form['name']]}
                        buttonName={
                          form['searchValue']
                            ? form['searchValue']
                            : 'Select value'
                        ...
                        selectedOption={(option: string) =>
                          setFilterDropdownVal(option, index, 'searchValue')
                        }
                        fontClass="font-dmMono"
                      />
                    </div>
                    ...
                    <div className="my-2 flex flex-grow flex-row items-center">
                      {form.operator != '' && (
                        <input
                          placeholder={form.addText}
                          value={form.searchValue}
                          onChange={(e) =>
                            setFilterDropdownVal(
                              e.target.value,
                              index,
                              'searchValue',
                            )
                      ...
                      {form.operator == SearchOperator.BETWEEN && (
                        <span className="mx-1 text-sm text-gray-500">AND</span>
                      )}
                      {form.operator == SearchOperator.BETWEEN && (
                        <input
                          placeholder={form.addText}
                          value={form.searchValueBetween}
                          onChange={(e) =>
                            setFilterDropdownVal(
                              e.target.value,
                              index,
