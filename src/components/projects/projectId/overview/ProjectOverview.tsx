...
import {
  ...
  postProcessConfusionMatrix,
  postProcessLabelDistribution,
  postProcessingStats,
} from '@/src/util/components/projects/projectId/project-overview/project-overview-helper'
...
import {
  ...
  GET_GENERAL_PROJECT_STATS,
  GET_INTER_ANNOTATOR_BY_PROJECT_ID,
  GET_LABEL_DISTRIBUTION,
  IS_RATS_TOKENIZAION_STILL_RUNNING,
} from '@/src/services/gql/queries/project-overview'
import {
...
import {
  ...
  setOverviewFilters,
} from '@/src/reduxStore/states/tmp'
...
} from '@/src/util/components/projects/projectId/project-overview/charts-helper'
...
export default function ProjectOverview() {
  ...
  const [refetchProjectStats] = useLazyQuery(GET_GENERAL_PROJECT_STATS, {
  ...
  const [refetchDataSlices] = useLazyQuery(DATA_SLICES, {
    fetchPolicy: 'network-only',
  })
  const [refetchLabelDistribution] = useLazyQuery(GET_LABEL_DISTRIBUTION, {
    fetchPolicy: 'no-cache',
  })
  const [refetchConfidenceDistribution] = useLazyQuery(
    GET_CONFIDENCE_DISTRIBUTION,
    { fetchPolicy: 'no-cache' },
  )
  const [refetchConfusionMatrix] = useLazyQuery(GET_CONFUSION_MATRIX, {
    fetchPolicy: 'no-cache',
  })
  const [refetchRatsTokenization] = useLazyQuery(
    IS_RATS_TOKENIZAION_STILL_RUNNING,
    { fetchPolicy: 'no-cache' },
  )
  const [refetchInterAnnotator] = useLazyQuery(
    GET_INTER_ANNOTATOR_BY_PROJECT_ID,
    { fetchPolicy: 'no-cache' },
  )
  ...
  function prepareInterAnnotator() {
    ...
    if (tmp) {
      ...
      if (parsedValues['interAnnotatorAllUsers'] != undefined)
        interAnnotatorFinal.allUsers = parsedValues['interAnnotatorAllUsers']
      if (parsedValues['interAnnotatorGoldUser'] != undefined)
        interAnnotatorFinal.goldUser = parsedValues['interAnnotatorGoldUser']
      if (parsedValues['interAnnotatorDataSlice'])
        interAnnotatorFinal.dataSlice = parsedValues['interAnnotatorDataSlice']
      if (parsedValues['labelingTasksTarget'])
        overviewFiltersCopy.targetAttribute =
          parsedValues['labelingTasksTarget']
      if (parsedValues['labelingTasks'])
        overviewFiltersCopy.labelingTask = parsedValues['labelingTask']
      if (parsedValues['displayGraphs'])
        overviewFiltersCopy.graphTypeEnum = parsedValues['graphTypeEnum']
      if (parsedValues['dataSlice'])
        overviewFiltersCopy.dataSlice = parsedValues['dataSlice']
    }
  ...
  function setDisplayNERConfusion() {
    ...
    if (labelingTaskTaskType != LabelingTaskTaskType.INFORMATION_EXTRACTION) {
      ...
      refetchRatsTokenization({ variables: { projectId: projectId } }).then(
        (res) => {
  ...
  function getLabelDistributions() {
    ...
    const dataSliceId =
      dataSliceFindId == '@@NO_SLICE@@' ? null : dataSliceFindId
    refetchLabelDistribution({
      variables: {
        projectId: projectId,
      ...
      setLabelDistribution(
        postProcessLabelDistribution(res['data']['labelDistribution']),
      )
    })
  }
  ...
  function getConfidenceDistributions() {
    ...
    refetchConfidenceDistribution({
      ...
      setConfidenceDistribution(
        JSON.parse(res['data']['confidenceDistribution']),
      )
    })
  }
  ...
  function getConfusionMatrix() {
    ...
    refetchConfusionMatrix({
      ...
      setConfusionMatrix(
        postProcessConfusionMatrix(res['data']['confusionMatrix']),
      )
    })
  }
  ...
  function getInterAnnotatorMatrix() {
    ...
    values.dataSlice =
      values.dataSlice == '@@NO_SLICE@@' ? null : values.dataSlice
    const labelingTaskId = overviewFilters.labelingTask?.id
    refetchInterAnnotator({
      variables: {
        projectId: projectId,
        labelingTaskId: labelingTaskId,
        includeGoldStar: values.goldUser,
        includeAllOrgUser: values.allUsers,
        onlyOnStaticSlice: values.dataSlice,
      },
    }).then((res) => {
      if (res['data'] == null) return
  ...
  function saveSettingsToLocalStorage() {
    ...
    currentData[projectId] = parseOverviewSettingsToDict(
      interAnnotatorFormGroup,
      overviewFilters,
    )
  ...
  function getProjectStats() {
    ...
    refetchProjectStats({
      ...
      if (res['data'] == null) return
      setProjectStats(
        postProcessingStats(JSON.parse(res['data']['generalProjectStats'])),
      )
    })
  }
  ...
  return (
    <div>
      {projectId != null && (
        <div className="flex h-[calc(100vh-4rem)] min-h-full flex-1 flex-col overflow-y-auto bg-gray-100 px-4 pb-10 pt-4">
          <ProjectOverviewHeader />
          <ProjectOverviewCards projectStats={projectStats} />
          {graphsHaveValues ? (
            <div>
              {(overviewFilters.graphType.value == DisplayGraphs.ALL ||
                overviewFilters.graphType.value ==
                  DisplayGraphs.LABEL_DISTRIBUTION) && (
                <div className="mt-8 grid w-full">
                  <div className="inline-block text-lg font-medium leading-6 text-gray-900">
                    Label distribution
                  </div>
                  <div className="mt-1 inline-block text-sm font-medium leading-5 text-gray-700">
                    See the distribution of your manually labeled and weakly
                    supervised records.
                  </div>
                  <div
                    className={`mt-2 h-full w-full shadow ${style.stats} grid flex-grow place-items-center bg-white`}
                  >
                    {labelDistribution && (
                      <div className="h-full w-full p-5">
                        <LabelDistributionBarChart
                          dataInput={labelDistribution}
                        />
                      </div>
                    )}
                  </div>
              ...
              {(overviewFilters.graphType.value == DisplayGraphs.ALL ||
                overviewFilters.graphType.value ==
                  DisplayGraphs.CONFIDENCE_DISTRIBUTION) && (
                <div className="mt-8 grid w-full">
                  <div className="inline-block text-lg font-medium leading-6 text-gray-900">
                    Confidence distribution
                  </div>
                  <div className="mt-1 inline-block text-sm font-medium leading-5 text-gray-700">
                    See the confidence distribution of your weakly supervised
                    records.
                  </div>
                  <div
                    className={`mt-2 h-full w-full shadow ${style.stats} grid flex-grow place-items-center bg-white`}
                  >
                    {confidenceDistribution && (
                      <div className="h-full w-full p-5">
                        <ConfidenceDistributionBarChart
                          dataInput={confidenceDistribution}
                        />
                      </div>
                    )}
                  </div>
              ...
              {(overviewFilters.graphType.value == DisplayGraphs.ALL ||
                overviewFilters.graphType.value ==
                  DisplayGraphs.CONFUSION_MATRIX) && (
                <div className="mt-8 grid w-full">
                  <div className="inline-block text-lg font-medium leading-6 text-gray-900">
                    Confusion matrix
                  </div>
                  <div className="mt-1 inline-block text-sm font-medium leading-5 text-gray-700">
                    See how often your manually labeled and weakly supervised
                    records agree or disagree.
                  </div>
                  <div
                    className={`mt-2 h-full w-full shadow ${style.stats} grid flex-grow place-items-center bg-white`}
                  >
                    {confusionMatrix && displayConfusion ? (
                      <ConfusionMatrixBarChart dataInput={confusionMatrix} />
                    ) : (
                      <>
                        {displayConfusion ? (
                          <div
                            className={`mb-3 flex items-center ${style.statsTitle}`}
                            style={{ height: '50px' }}
                          >
                            <LoadingIcon />
                          </div>
                          ...
                          <div
                            className={`flex items-center ${style.statsTitle}`}
                            style={{ height: '50px' }}
                          >
                            Tokenization is still running
                          </div>
                        )}
                      </>
                    )}
              ...
              {(overviewFilters.graphType.value == DisplayGraphs.ALL ||
                ...
                isManaged && (
                  <div className="mt-8 grid w-full">
                    ...
                    <div className="mt-1 inline-block text-sm font-medium leading-5 text-gray-700">
                      See how users agree or disagree on redundantly labeled
                      records.
                    </div>
                    {interAnnotatorMatrix && (
                      <div>
                        {interAnnotatorMatrix.countNames >
                        1 + (goldUserRequested ? 1 : 0) ? (
                          <div
                            className={`mt-2 h-full w-full shadow ${style.stats} grid flex-grow place-items-center bg-white`}
                          >
                            {interAnnotatorMatrix && (
                              <div className="h-full w-full p-5">
                                <InterAnnotatorBarChart
                                  dataInput={interAnnotatorMatrix}
                                />
                              </div>
                            )}
                          </div>
                          ...
                          <div className="mt-2">
                            ...
                            {isManaged && (
                              <div
                                className={`mt-2 shadow ${style.stats} grid h-full w-full flex-grow place-items-center bg-white p-5`}
                              >
                                <div className="text-center">
                                  ...
                                  <p className="mt-1 text-sm text-gray-500">
                                    None of your records are labeled redundantly
                                    yet.
                                  </p>
                                </div>
                              </div>
            ...
            <div>
              <div className="mt-8 inline-block text-lg font-medium leading-6 text-gray-900">
                Monitoring
              </div>
              <div className="mt-1 block text-sm font-medium leading-5 text-gray-700">
                Go to the settings page to add a labeling task.
              </div>
              <div
                className={`mt-2 w-full shadow ${style.stats} place-content-center bg-white p-4`}
              >
                <div className={`${style.statsTitle}`}>
                  Add Labels to display charts
                </div>
              </div>
            </div>
          )}
