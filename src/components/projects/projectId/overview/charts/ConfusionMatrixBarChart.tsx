...
export default function ConfusionMatrixBarChart(props: BarChartProps) {
  useEffect(() => {
      ...
      if (elements.length > 0) {
        ...
        divTooltip.html(
          '<span style="font-family: \'DM Sans\', sans-serif;">' +
            elementData.labelIdProgrammatic +
            ' / ' +
            elementData.labelIdManual +
            '</span>',
        )
      }
    }
    ...
    svg
      ...
      .data(data, function (d) {
        return d.labelIdManual + ':' + d.labelIdProgrammatic
      })
      .enter()
      .append('rect')
      .classed('my-selector', true)
      .attr('x', function (d) {
        return x(d.labelIdManual)
      })
      .attr('y', function (d) {
        return y(d.labelIdProgrammatic)
      })
      ...
      .style('opacity', (d) => {
        ...
        return d3.max([d.counts / d3.max(data.map((e) => e.counts)), 0.5])
      })
  ...
  return (
    <>
      <div
        className={`${style.confusionMatrixTooltip} confusionMatrixTooltip`}
      ></div>
