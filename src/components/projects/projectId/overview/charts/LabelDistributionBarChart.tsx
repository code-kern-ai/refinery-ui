...
export default function LabelDistributionBarChart(props: BarChartProps) {
  ...
  useEffect(() => {
    ...
    var z = d3.scaleOrdinal().range(['#E1B53E', '#5EC269'])
    ...
    yAxis
      ...
      .selectAll('.tick')
      ._groups[0].forEach((tick: any) => {
        var positionString = tick.getAttribute('transform')
        var position = parseFloat(
          positionString.substring(12, positionString.length - 1),
        )
    ...
    return (
      ...
      `</div> 
            <div style="display: grid;grid-template-columns: max-content max-content;">
              ...
              <div>` +
      Math.round(elementData.value * 100) / 100 +
      `%</div>
              <div class="font-bold">absolute</div>
              <div>` +
      elementData.absolute / 100 +
      `</div>
            </div>
          </div>
        </span>
