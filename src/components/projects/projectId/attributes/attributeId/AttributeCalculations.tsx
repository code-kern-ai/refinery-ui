...
import {
  ...
  GET_ATTRIBUTE_BY_ATTRIBUTE_ID,
  GET_LABELING_TASKS_BY_PROJECT_ID,
  GET_PROJECT_TOKENIZATION,
...
import {
  ATTRIBUTES_VISIBILITY_STATES,
  DATA_TYPES,
  getTooltipVisibilityState,
...
export default function AttributeCalculation() {
  ...
  const [refetchProjectTokenization] = useLazyQuery(GET_PROJECT_TOKENIZATION, {
    fetchPolicy: 'no-cache',
  })
  const [refetchAttributeByAttributeId] = useLazyQuery(
    GET_ATTRIBUTE_BY_ATTRIBUTE_ID,
    { fetchPolicy: 'no-cache' },
  )
  ...
  useEffect(() => {
    ...
    if (isInitial == null)
      setIsInitial(
        AttributeCodeLookup.isCodeStillTemplate(
          currentAttribute.sourceCode,
  ...
  useEffect(() => {
    ...
    if (!currentAttribute || attributes.length == 0) {
      refetchAttributes({
        ...
        const currentAttribute = postProcessCurrentAttribute(
          attributes.find(
            (attribute) => attribute.id === router.query.attributeId,
          ),
        )
    ...
    if (lookupLists.length == 0) {
      refetchLookupLists({ variables: { projectId: projectId } }).then(
        (res) => {
  ...
  useEffect(() => {
    ...
    setTooltipsArray(
      ATTRIBUTES_VISIBILITY_STATES.map((state) =>
        getTooltipVisibilityState(state.value),
      ),
    )
  }, [attributes])
  ...
  useEffect(() => {
    ...
    if (
      currentAttribute.state == AttributeState.USABLE ||
      currentAttribute.state == AttributeState.RUNNING
    ) {
  ...
  useEffect(() => {
    if (!currentAttribute) return
    if (
      currentAttribute.sourceCodeToDisplay == editorValue ||
      currentAttribute.state == AttributeState.USABLE
    )
      return
  ...
  function changeAttributeName(name: string) {
    ...
    if (name == '') return
    const duplicateNameExists = attributes.find(
    ...
    updateAttributeMut({
      variables: {
        ...
        name: attributeNew.name,
      },
    }).then(() => {
      setCurrentAttribute(postProcessCurrentAttribute(attributeNew))
      setEditorValue(
        attributeNew.sourceCode.replace(
          'def ac(record)',
          'def ' + attributeNew.name + '(record)',
        ),
      )
  ...
  function updateVisibility(option: any) {
    ...
    attributeNew.visibilityIndex = ATTRIBUTES_VISIBILITY_STATES.findIndex(
    ...
    updateAttributeMut({
      variables: {
        projectId: projectId,
        attributeId: currentAttribute.id,
        visibility: attributeNew.visibility,
      },
    }).then(() => {
      setCurrentAttribute(postProcessCurrentAttribute(attributeNew))
      dispatch(updateAttributeById(attributeNew))
  ...
  function updateDataType(option: any) {
    ...
    updateAttributeMut({
      variables: {
        projectId: projectId,
        attributeId: currentAttribute.id,
        dataType: attributeNew.dataType,
      },
    }).then(() => {
      setCurrentAttribute(postProcessCurrentAttribute(attributeNew))
      dispatch(updateAttributeById(attributeNew))
  ...
  function updateSourceCode(value: string, attributeNameParam?: string) {
    ...
    updateAttributeMut({
      variables: {
        ...
        name: attributeNameParam,
      },
    }).then(() => {})
  }

  function checkProjectTokenization() {
    refetchProjectTokenization({ variables: { projectId: projectId } }).then(
      (res) => {
          ...
          <div>
            ...
            >
              ...
              >
                ...
                >
                  <a
                    href={`/refinery/projects/${projectId}/settings`}
                    onClick={(e) => {
                      e.preventDefault()
                  ...
                  <Statuses
                    status={currentAttribute.state}
                    page="attributes"
                    initialCaption="Registered"
                  />
                </div>
              </div>
            </div>
            <div className="w-full">
                  ...
                  <div className="mt-2 flex items-center">
                    <Tooltip
                      ...
                      content={
                        currentAttribute.state == AttributeState.USABLE ||
                        currentAttribute.state == AttributeState.RUNNING
                          ? TOOLTIPS_DICT.ATTRIBUTE_CALCULATION.CANNOT_EDIT_NAME
                          : TOOLTIPS_DICT.ATTRIBUTE_CALCULATION.EDIT_NAME
                      }
                    >
                      <button
                        ...
                        className={`float-left mr-3 block flex-shrink-0 rounded-md border border-gray-300 bg-white px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${currentAttribute.state == AttributeState.USABLE || currentAttribute.state == AttributeState.RUNNING}`}
                      >
                        Edit name
                      </button>
                    </Tooltip>
                    ...
                    >
                      ...
                      currentAttribute.state != AttributeState.RUNNING ? (
                        <input
                          type="text"
                          value={attributeName}
                          onInput={(e: any) => setAttributeName(e.target.value)}
                          onBlur={() => openName(false)}
                          onKeyDown={(e) => {
                            if (e.key == 'Enter') openName(false)
                          }}
                          className="placeholder-italic h-8 rounded-md border border-gray-300 pl-4 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100"
                        />
                      ) : (
              ...
              >
                ...
                <Dropdown2
                  buttonName={currentAttribute.visibilityName}
                  options={ATTRIBUTES_VISIBILITY_STATES}
                  dropdownWidth="w-52"
                  tooltipArrayPlacement="right"
                  tooltipsArray={tooltipsArray}
                  selectedOption={(option: any) => updateVisibility(option)}
                ...
                <div className="flex flex-row items-center">
                  <Tooltip
                    ...
                    content={
                      currentAttribute.state == AttributeState.USABLE ||
                      currentAttribute.state == AttributeState.RUNNING
                        ? TOOLTIPS_DICT.ATTRIBUTE_CALCULATION
                            .CANNOT_EDIT_DATATYPE
                        : TOOLTIPS_DICT.ATTRIBUTE_CALCULATION.EDIT_DATATYPE
                    }
                  >
                    <Dropdown2
                      buttonName={currentAttribute.dataTypeName}
                      options={DATA_TYPES}
                      dropdownWidth="w-52"
                      selectedOption={(option: any) => updateDataType(option)}
                      disabled={currentAttribute.state == AttributeState.USABLE}
                      dropdownClasses="z-30"
                    />
                  </Tooltip>
                  {currentAttribute.dataType == DataTypeEnum.EMBEDDING_LIST && (
                    <div className="ml-3 text-sm text-gray-700">
                      Only useable for similarity search
                    </div>
                  )}
                </div>
              ...
              <div className="my-3 flex flex-row items-center justify-between">
                ...
                <div className="flex flex-row flex-nowrap">
                  ...
                  <Tooltip
                    content={
                      TOOLTIPS_DICT.ATTRIBUTE_CALCULATION.AVAILABLE_LIBRARIES
                    }
                    placement="left"
                    color="invert"
                  >
                    <a
                      href="https://github.com/code-kern-ai/refinery-ac-exec-env/blob/dev/requirements.txt"
                      target="_blank"
                      className="ml-2 rounded-md border border-gray-300 bg-white  px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none"
                    >
                ...
                refetchCurrentAttribute={() => {
                  refetchAttributeByAttributeId({
                    ...
                    if (attribute == null) setCurrentAttribute(null)
                      ...
                      setCurrentAttribute(
                        postProcessCurrentAttribute(attribute),
                      )
                  })
                }}
              />
              ...
              <div className="mt-8">
                ...
                {currentAttribute.progress == 0 &&
                  currentAttribute.state == AttributeState.INITIAL && (
                    <div className="bg-white">
                      <div className="py-6 text-sm font-normal leading-5 text-gray-500">
                        This attribute was not yet run.
                      </div>
                    </div>
                  )}
                {currentAttribute.progress < 1 &&
                  currentAttribute.state == AttributeState.RUNNING && (
                    <div className=" card mb-4 flex-grow overflow-visible rounded-2xl border border-gray-200 bg-white">
                      <div className="card-body p-6">
                        <div className="flex flex-row items-center">
                          <Tooltip
                            content={
                              TOOLTIPS_DICT.ATTRIBUTE_CALCULATION.BEING_EXECUTED
                            }
                            color="invert"
                            placement="right"
                            className="relative z-10 cursor-auto"
                          >
                            <LoadingIcon />
                          </Tooltip>
                          <div className="w-full text-sm font-normal leading-5 text-gray-500">
                            <div className="h-2.5 w-full rounded-full bg-gray-200 dark:bg-gray-700">
                              <div
                                ...
                                style={{
                                  width: currentAttribute.progress + '%',
                                }}
                              ></div>
                            </div>
                          </div>
                ...
                {currentAttribute.state !== AttributeState.RUNNING &&
                  currentAttribute.state !== AttributeState.INITIAL && (
                    <div className="flex flex-row items-center">
                      ...
                      {currentAttribute.state == AttributeState.FAILED && (
                        <Tooltip
                      ...
                      <div className="py-6 text-sm font-normal leading-5 text-gray-500">
                        {currentAttribute.state === 'FAILED'
                          ? 'Attribute calculation ran into errors.'
              ...
              <DangerZone
                elementType={DangerZoneEnum.ATTRIBUTE}
