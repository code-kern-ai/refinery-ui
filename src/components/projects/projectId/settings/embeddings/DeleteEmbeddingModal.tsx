...
export default function DeleteEmbeddingModal() {
  ...
  const modalDeleteEmbedding = useSelector(
    selectModal(ModalEnum.DELETE_EMBEDDING),
  )
  ...
  const deleteEmbedding = useCallback(() => {
    ...
    if (modalDeleteEmbedding.isQueuedElement) {
      refetchDeleteTaskQueue({
        variables: { projectId: projectId, taskId: embeddingId },
      }).then((res) => {
        dispatch(removeFromAllEmbeddingsById(embeddingId))
      })
    } else {
      refetchDeleteEmbedding({
        variables: { projectId: projectId, embeddingId: embeddingId },
      }).then((res) => {
        dispatch(removeFromAllEmbeddingsById(embeddingId))
      })
    }
  ...
  useEffect(() => {
    ...
    abortButtonCopy.buttonCaption = modalDeleteEmbedding.isQueuedElement
      ? 'Dequeue embedding'
      : 'Delete embedding'
  ...
  return (
    <Modal modalName={ModalEnum.DELETE_EMBEDDING} abortButton={abortButton}>
      ...
      <p className="mt-2 text-sm text-gray-500">
        ...
        {modalDeleteEmbedding.isQueuedElement ? 'dequeue' : 'delete'} this
        embedding?
      </p>
      {!modalDeleteEmbedding.isQueuedElement && (
        <p className="mt-2 text-sm text-gray-500">
          This will delete all corresponding tensors!
        </p>
      )}
    </Modal>
  )
