...
import {
  selectEmbeddings,
  selectRecommendedEncodersAll,
  selectRecommendedEncodersDict,
  selectUsableNonTextAttributes,
  selectUseableEmbedableAttributes,
  setAllRecommendedEncodersDict,
...
import {
  DEFAULT_AZURE_TYPE,
  GRANULARITY_TYPES_ARRAY,
  checkIfCreateEmbeddingIsDisabled,
  platformNamesDict,
} from '@/src/util/components/projects/projectId/settings/embeddings-helper'
...
export default function AddNewEmbeddingModal() {
  ...
  const useableEmbedableAttributes = useSelector(
    selectUseableEmbedableAttributes,
  )
  ...
  const embeddingPlatforms = useSelector(
    selectCachedValue(CacheEnum.EMBEDDING_PLATFORMS),
  )
  ...
  const [selectedPlatform, setSelectedPlatform] =
    useState<EmbeddingPlatform>(null)
  const [granularityArray, setGranularityArray] = useState(
    GRANULARITY_TYPES_ARRAY,
  )
  const [filteredAttributesArray, setFilteredAttributesArray] = useState<
    Attribute[]
  >([])
  ...
  function prepareSuggestions() {
    ...
    let suggestionList = embeddingHandlesAll.filter(
      (suggestion: any) => suggestion.platform == platformVal,
    )
    const suggestionListFiltered = suggestionList.map((suggestion: any) => {
      ...
      if (
        (granularity.value == EmbeddingType.ON_ATTRIBUTE &&
          applicability.attribute) ||
        (granularity.value == EmbeddingType.ON_TOKEN && applicability.token)
      ) {
    ...
    suggestionList = suggestionListFiltered.filter(
      (suggestion: any) => !suggestion.forceHidden,
    )
  ...
  function checkIfAttributeHasToken() {
    const attribute = useableEmbedableAttributes.find(
      (a) => a.id == targetAttribute.id,
    )
    if (attribute?.dataType == DataTypeEnum.EMBEDDING_LIST) {
      setGranularityArray(
        GRANULARITY_TYPES_ARRAY.filter(
          (g) => g.value != EmbeddingType.ON_TOKEN,
        ),
      )
    } else {
      checkIfPlatformHasToken()
  ...
  function changePlatformOrGranularity() {
    ...
    if (
      savePlatform == PlatformType.COHERE ||
      savePlatform == PlatformType.OPEN_AI ||
      savePlatform == PlatformType.AZURE
    ) {
      setGranularity(
        GRANULARITY_TYPES_ARRAY.find(
          (g) => g.value == EmbeddingType.ON_ATTRIBUTE,
        ),
      )
  ...
  function checkIfPlatformHasToken() {
    if (!platform) return
    if (
      platform.name == platformNamesDict[PlatformType.COHERE] ||
      platform.name == platformNamesDict[PlatformType.OPEN_AI] ||
      platform.name == platformNamesDict[PlatformType.AZURE]
    ) {
      setGranularityArray(
        GRANULARITY_TYPES_ARRAY.filter(
          (g) => g.value != EmbeddingType.ON_TOKEN,
  ...
  const addEmbedding = useCallback(() => {
    const config: any = {
      ...
      embeddingType:
        granularity.value == EmbeddingType.ON_TOKEN
          ? EmbeddingType.ON_TOKEN
          : EmbeddingType.ON_ATTRIBUTE,
      filterAttributes: filteredAttributes,
    }

    if (
      platform.name == platformNamesDict[PlatformType.HUGGING_FACE] ||
      platform.name == platformNamesDict[PlatformType.PYTHON]
    ) {
      config.model = model
  ...
  useEffect(() => {
    setAcceptButton({
      ...acceptButton,
      emitFunction: addEmbedding,
      disabled: checkIfCreateEmbeddingIsDisabled({
        platform,
        model,
        apiToken,
        termsAccepted,
    ...
    <Modal modalName={ModalEnum.ADD_EMBEDDING} acceptButton={acceptButton}>
      ...
      <div className="mb-2 flex flex-grow justify-center text-sm text-gray-500">
        {useableEmbedableAttributes
          ...
          : 'No usable text attributes to embed exist'}
      </div>
      <form>
        <div
        ...
        >
          <Tooltip
            content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.TARGET_ATTRIBUTE}
            placement="right"
            color="invert"
          >
          ...
          <Dropdown2
            options={useableEmbedableAttributes}
            buttonName={targetAttribute ? targetAttribute.name : 'Choose'}
            selectedOption={(option: any) => setTargetAttribute(option)}
          />

          <Tooltip
            content={
              TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.FILTER_ATTRIBUTES
            }
            placement="right"
            color="invert"
          >
          ...
          <Dropdown2
            options={filteredAttributesArray}
            buttonName={
          ...
          <Tooltip
            content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.PLATFORM}
            placement="right"
            color="invert"
          >
            <span className="card-title label-text mb-0 flex">
          ...
          <Dropdown2
            options={embeddingPlatforms}
            buttonName={platform ? platform.name : 'Choose'}
            selectedOption={(option: any) => {
              setPlatform(option)
          ...
          <Tooltip
            content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.GRANULARITY}
            placement="right"
            color="invert"
          >
            <span className="card-title label-text mb-0 flex">
          ...
          <Dropdown2
            options={granularityArray}
            buttonName={granularity ? granularity.name : 'Choose'}
            selectedOption={(option: any) => setGranularity(option)}
          />
          {platform &&
            platform.name == platformNamesDict[PlatformType.HUGGING_FACE] && (
              <SuggestionsModel
                options={embeddingHandles[targetAttribute]}
                selectedOption={(model: string) => setModel(model)}
              />
            )}
          {platform &&
            platform.name == platformNamesDict[PlatformType.OPEN_AI] && (
              <>
                ...
                />
                <Tooltip
                  content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.API_TOKEN}
                  placement="right"
                  color="invert"
                >
                  <span className="card-title label-text mb-0 flex">
            ...
            platform.name == platformNamesDict[PlatformType.AZURE] && (
              <>
                ...
                {azureEngines.length == 0 ? (
                  <>
                    <Tooltip
                      content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.ENGINE}
                      placement="right"
                      color="invert"
                    >
                      <span className="card-title label-text mb-0 flex">
                    ...
                    <input
                      placeholder="Enter Azure engine"
                  ...
                  <>
                    <SuggestionsAzure
                      options={azureEngines}
                      selectedOption={(option: string) => setEngine(option)}
                      name="Engine"
                      tooltip="This will be your custom engine name. You can find this in the Azure OpenAI studio in the deployments section."
                    />
                  </>
                )}
                ...
                {azureUrls.length == 0 ? (
                  <>
                    <Tooltip
                      content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.URL}
                      placement="right"
                      color="invert"
                    >
                      <span className="card-title label-text mb-0 flex">
                  ...
                  <>
                    <SuggestionsAzure
                      options={azureUrls}
                      selectedOption={(option: string) => setUrl(option)}
                      name="Azure URL"
                      tooltip="This will be your custom URL, which looks like this: https://<your-api-base>.openai.azure.com/"
                    />
                  </>
                )}
                ...
                {azureVersions.length == 0 ? (
                  <>
                    <div className="flex flex-row items-center">
                      ...
                      <Tooltip
                        content={
                          TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.VERSION
                        }
                        placement="right"
                        color="invert"
                      >
                        <a
                          ...
                          target="_blank"
                        >
                          <IconExternalLink className="ml-1 h-5 w-5 cursor-help" />
                        </a>
                      </Tooltip>
                    </div>
                    <input
                      placeholder="Enter Azure version"
                  ...
                  <>
                    <SuggestionsAzure
                      options={azureVersions}
                      selectedOption={(option: string) => setVersion(option)}
                      name="Version"
                      tooltip="The latest version of the Azure OpenAI service can also be found here."
                    />
                  </>
                )}
              </>
        ...
        {platform &&
          (platform.name == platformNamesDict[PlatformType.COHERE] ||
            ...
            <div className="mt-3 text-center">
              <div className="rounded-lg border border-gray-300 p-2.5 text-justify text-xs text-gray-500">
              ...
              <div className="m-3 flex flex-row items-center justify-center">
                ...
                <label
                  ...
                  className="ml-1 cursor-pointer text-sm font-medium text-gray-500"
                >
...
function SuggestionsModel(props: SuggestionsProps) {
  ...
  useEffect(() => {
    ...
    refetchModelsDownload().then((res) => {
      const modelsDownloaded = postProcessingModelsDownload(
        res.data['modelProviderInfo'],
      )
  ...
  return (
    <>
      <Tooltip
        content={TOOLTIPS_DICT.PROJECT_SETTINGS.EMBEDDINGS.MODEL}
        placement="right"
        color="invert"
      >
        <span className="card-title label-text mb-0 flex">
          <span className="filtersUnderline cursor-help underline">Model</span>
        </span>
      </Tooltip>
      <Dropdown2
        ...
        selectedOption={(option: any) =>
          props.selectedOption(option.configString)
        }
        ...
        filteredOptions={(option: string) =>
          setFilteredList(
            props.options.filter((val: any) =>
              val.configString.includes(option),
            ),
          )
        }
      />
...
function SuggestionsAzure(props: SuggestionsProps) {
  ...
  return (
    <>
      <Tooltip content={props.tooltip} placement="right" color="invert">
        <span className="card-title label-text mb-0 flex">
          <span className="filtersUnderline cursor-help underline">
            {props.name}
          </span>
      ...
      <Dropdown2
        ...
        filteredOptions={(option: string) =>
          setFilteredList(
            props.options.filter((val: any) => val.includes(option)),
          )
        }
      />
    </>
