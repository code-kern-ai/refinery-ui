...
export default function CreateNewAttributeModal() {
  ...
  const modalCreateNewAtt = useSelector(
    selectModal(ModalEnum.CREATE_NEW_ATTRIBUTE),
  )
  ...
  useEffect(() => {
    setAcceptButton({
      ...acceptButton,
      emitFunction: createUserAttribute,
      disabled:
        duplicateNameExists ||
        attributeName.trim() == '' ||
        attributeType == null,
  ...
  function handleAttributeName(value: string) {
    ...
    const checkName = attributes.some(
      (attribute) => attribute.name == valueToSave,
    )
  ...
  return (
    <Modal
      modalName={ModalEnum.CREATE_NEW_ATTRIBUTE}
    ...
    >
      ...
      <div className="mb-2 flex flex-grow justify-center text-sm text-gray-500">
        Choose a name for your attribute and pick a datatype you want to use
      </div>
      <div
      ...
      >
        <Tooltip
          content={TOOLTIPS_DICT.PROJECT_SETTINGS.ATTRIBUTE_NAME}
          color="invert"
          placement="right"
        >
          <span className="card-title  label-text mb-0 cursor-help font-normal">
            <span className="filtersUnderline underline">Attribute name</span>
          </span>
        </Tooltip>
        <input
          type="text"
          value={attributeName}
          onChange={(e: any) => handleAttributeName(e.target.value)}
          onKeyDown={(e) => {
            if (e.key == 'Enter') createUserAttribute()
          }}
        ...
        />
        <Tooltip
          content={TOOLTIPS_DICT.PROJECT_SETTINGS.SELECT_ATTRIBUTE_TYPE}
          color="invert"
          placement="right"
        >
          <span className="card-title label-text mb-0 cursor-help font-normal">
            <span className="filtersUnderline underline">Attribute type</span>
          </span>
        </Tooltip>
        <Dropdown2
          buttonName={attributeType ? attributeType.name : 'Select type'}
          options={DATA_TYPES}
          selectedOption={(option: any) => setAttributeType(option)}
        />
      </div>
      {duplicateNameExists && (
        <div className="mt-2 text-xs text-red-700">Attribute name exists</div>
      )}
      {attributeType.name == 'Embedding List' && (
        <div className="mt-2 max-w-2xl rounded-lg border border-gray-300 p-2.5 text-justify text-xs text-gray-500">
          <label className="text-gray-700">
            Embedding lists are special. They can only be used for similarity
            search. If a list entry is matched, the whole record is considered
            matched.
          </label>
        </div>
      )}
