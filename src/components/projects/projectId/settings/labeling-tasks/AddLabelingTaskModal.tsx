...
export default function AddLabelingTaskModal() {
  ...
  const modalAddLabelingTask = useSelector(
    selectModal(ModalEnum.ADD_LABELING_TASK),
  )
  ...
  const addLabelingTask = useCallback(() => {
    ...
    if (targetAttribute.name !== 'Full Record') {
      taskTarget = usableAttributes.find(
        (attribute) => attribute.name == targetAttribute.name,
      ).id
    }
    createLabelingTaskMut({
      variables: {
        projectId: projectId,
        labelingTaskName: taskName,
  ...
  useEffect(() => {
    setAcceptButton({
      ...ACCEPT_BUTTON,
      emitFunction: addLabelingTask,
      disabled:
        taskName == '' ||
        targetAttribute.name == '' ||
        !isTaskNameUnique(labelingTasksSchema, taskName),
    })
  }, [modalAddLabelingTask, targetAttribute, taskName])
  ...
  return (
    <Modal modalName={ModalEnum.ADD_LABELING_TASK} acceptButton={acceptButton}>
      ...
      <div className="mb-2 flex flex-grow justify-center text-sm text-gray-500">
        Afterward, you can select the label task type depending on the target
        type
      </div>
      <div
      ...
      >
        <Tooltip
          content={
            TOOLTIPS_DICT.PROJECT_SETTINGS.LABELING_TASK.TARGET_ATTRIBUTE
          }
          placement="right"
          color="invert"
        >
        ...
        <Tooltip
          content={
            TOOLTIPS_DICT.PROJECT_SETTINGS.LABELING_TASK.NAME_LABELING_TASK
          }
          placement="right"
          color="invert"
        >
          <span className="card-title label-text mb-0 flex">
            <span className="filtersUnderline cursor-help underline">Name</span>
          </span>
        </Tooltip>
        <input
          placeholder="Labeling task name"
          value={taskName}
