...
export default function LookupListsOverview() {
  ...
  function selectLookupLists(checked: boolean) {
    dispatch(
      setCheckedLookupLists(Array(checkedLookupLists.length).fill(checked)),
    )
    prepareSelectionList()
  }
  ...
  return projectId ? (
    <div className="flex h-screen flex-1 flex-col bg-gray-100 p-4">
      <div className="w-full ">
        <div className="block flex-shrink-0 items-center justify-between sm:flex">
          <div className="inline-block text-lg font-medium leading-6 text-gray-900">
            Lookup lists
          </div>
          <div className="grid grid-cols-1 flex-row items-center gap-4 xs:flex xs:gap-0">
            {lookupLists && lookupLists.length > 0 ? (
              <Dropdown2
                ...
                disabledOptions={[
                  false,
                  false,
                  checkedLookupLists.every((checked) => !checked),
                ]}
                selectedOption={(option: any) => executeOption(option)}
              ...
              <Tooltip
                placement="left"
                content={TOOLTIPS_DICT.LOOKUP_LISTS_OVERVIEW.ENABLE_ACTIONS}
                color="invert"
              >
                <button
                  type="button"
            ...
            <div className="flex justify-center overflow-visible">
              <Tooltip
                placement="left"
                content={TOOLTIPS_DICT.LOOKUP_LISTS_OVERVIEW.CREATE_LOOKUP_LIST}
                color="invert"
              >
                <button
            ...
            <div className="flex justify-center overflow-visible">
              <Tooltip
                placement="left"
                content={
                  TOOLTIPS_DICT.LOOKUP_LISTS_OVERVIEW.NAVIGATE_HEURISTICS
                }
                color="invert"
              >
                <a
            ...
            <div className="flex justify-center overflow-visible">
              <Tooltip
                placement="left"
                content={
                  TOOLTIPS_DICT.LOOKUP_LISTS_OVERVIEW.NAVIGATE_MODEL_CALLBACKS
                }
                color="invert"
              >
                <a
          ...
          <div>
            <div className="mt-8 font-normal text-gray-500">
              <p className="text-xl leading-7">
                Seems like your project has no lookup list yet.
              </p>
              <p className="mt-3 text-base leading-6">
                You can create one from the button New list in the bar above.
                Also, we got some quicklinks from our{' '}
            ...
            <div className="mt-8 grid grid-cols-1 gap-8 sm:grid-cols-2">
              <div>
                <div className="text-xl font-semibold leading-7 text-gray-900">
                  Automatically building lookup lists
                </div>
                <div className="mt-3 text-base font-normal leading-6 text-gray-500">
                  When you label for an extraction task, we automatically
                  generate a lookup list with that label name. Also, each
                  labeled span is going to be stored in that list. That way, you
                  can just label your data, and we collect your entities.
                </div>
                <div className="mt-3 cursor-pointer text-base font-semibold leading-6 text-green-800 hover:text-green-500">
                  <a
                    href="https://docs.kern.ai/docs/building-labeling-functions#lookup-lists-for-distant-supervision"
                    target="_blank"
                  >
                    Read about lookup lists
                  </a>
                </div>
              </div>
              <div>
                <div className="text-xl font-semibold leading-7 text-gray-900">
                  Building labeling functions using lookup lists
                </div>
                <div className="mt-3 text-base font-normal leading-6 text-gray-500">
                  Labeling functions can be implemented to look up these lists,
                  such that you don&apos;t need to maintain the function code,
                  but can instead cover the lookup list. There are plenty other
                  template functions, which you can check out in our public
                  GitHub repository.
                </div>
                <div className="mt-3 cursor-pointer text-base font-semibold leading-6 text-green-800 hover:text-green-500">
                  <a
