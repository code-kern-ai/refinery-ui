...
import {
  CHANGE_ORGANIZATION,
  UPDATE_CONFIG,
} from '@/src/services/gql/mutations/organizations'
import {
  Configuration,
  LocalConfig,
} from '@/src/types/components/config/config'
...
export default function Config() {
  ...
  useEffect(() => {
    ...
    setLocalConfig({
      ...
      spacyDownloads: Array.from(
        ConfigManager.getConfigValue('spacy_downloads'),
      ),
    })
  ...
  useEffect(() => {
    ...
    const filtered = tokenizerValuesDisplay.filter(
      (tokenizer: any) => tokenizer.configString,
    )
    setPrepareTokenizedValues(filtered)
    const removeDuplicates = filtered.filter(
      (tokenizer: any) =>
        !localConfig?.spacyDownloads.includes(tokenizer.configString),
    )
  ...
  function checkAndSaveValue(value: any, key: string, subkey: string = null) {
    ...
    if (
      subkey == 'max_rows' ||
      subkey == 'max_cols' ||
      subkey == 'max_char_count'
    ) {
      changeOrganizationMut({
        variables: {
          orgId: organization.id,
          changes: JSON.stringify(updateDict.limit_checks),
        },
      }).then((res) => {
      ...
      updateConfigMut({
        variables: { dictStr: JSON.stringify(updateDict) },
      }).then((res) => {
  ...
  function removeSpacyTokenizer(valueToRemove: string) {
    ...
    localConfigCopy.spacyDownloads = localConfig.spacyDownloads.filter(
      (i) => i != valueToRemove,
    )
  ...
  function changeConfigString(value: any, index: number) {
    const localConfigCopy = { ...localConfig }
    localConfigCopy.spacyDownloads[index] = value.name
      .split('(')[1]
      .split(')')[0]
    setLocalConfig(localConfigCopy)
    checkAndSaveValue(localConfigCopy.spacyDownloads, 'spacy_downloads')
  ...
  function addSpacyConfig() {
    ...
    for (const tok of preparedOptions) {
      if (!existingConfigs.includes(tok.configString)) {
        ...
        const index = preparedOptionsCopy.findIndex(
          (tokenizer: any) => tokenizer.configString == tok.configString,
        )
  ...
  return (
    <div className="h-screen space-y-6 bg-white px-4 py-6 sm:p-6">
      <div>
        <h3 className="text-lg font-medium leading-6 text-gray-900">
          Configuration
        </h3>
        <p className="mt-1 text-sm text-gray-500">
          Here, you can change application settings for your self-hosted
          version.
        </p>
      </div>
      {localConfig && (
        ...
        >
          ...
          <input
            type="number"
            value={configuration.maxRows}
            className="placeholder-italic h-8 w-32 rounded-md border border-gray-300 pl-4 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100"
            onChange={(e) => {
          ...
          <input
            type="number"
            value={configuration.maxCharCount}
            className="placeholder-italic h-8 w-32 rounded-md border border-gray-300 pl-4 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-gray-100"
            onChange={(e) => {
              checkAndSaveValue(
                e.target.value,
          ...
          <div className="h-full text-sm">
            <label htmlFor="candidates" className="font-medium text-gray-700">
              Available <span className="font-dmMono">spaCy</span> language
              models
            </label>
            <p className="text-gray-500">
          ...
          {localConfig.spacyDownloads && (
            <div className="flex flex-col gap-y-2">
              {localConfig.spacyDownloads.map((myConfig, index) => (
                <div
                  key={myConfig}
                  className="flex flex-row flex-nowrap items-center gap-x-2"
                >
                  <Dropdown2
                    options={preparedOptions}
                    buttonName={
                      prepareTokenizedValues.find(
                        (tokenizer: any) => tokenizer.configString == myConfig,
                      )?.name
                    }
                    selectedOption={(option) =>
                      changeConfigString(option, index)
                    }
                  ...
                  />
                  {index > 1 && (
                    <button className="inline-flex items-center px-1">
                      <IconTrash
                        onClick={() => removeSpacyTokenizer(myConfig)}
              ...
              <div className="relative self-center" style={{ left: '-1.5rem' }}>
                {tokenizerValues &&
                  localConfig.spacyDownloads.length <
                    tokenizerValues.length && (
                    <button
                      onClick={addSpacyConfig}
                      className="inline-flex cursor-pointer items-center self-center rounded border border-gray-200 bg-white px-0.5 py-0.5 text-xs font-medium text-gray-700 shadow-sm focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                    >
